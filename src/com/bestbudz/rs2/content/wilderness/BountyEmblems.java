...
public enum BountyEmblems {
	...
	/**
	 * Compares the two emblems and determiens which of the two are 'better' based
	 * on the item identification value.
	 */
	static final Comparator<BountyEmblems> BEST_EMBLEM_COMPARATOR = (first, second) -> Integer.compare(first.itemId, second.itemId);

	/**
	 * Attempts to retrieve the best emblem the stoner has. The exclude parameter
	 * although may look out of place, allows us to deterquarry if the TIER_10
	 * element should be excluded when determining the best. This is due to the two
	 * scenarios in which we use this method. One when upgrading the emblem after
	 * killing a stoner, and the other when dropping the emblem on death.
	 * 
	 * @param stoner
	 *                    the stoner this is deterquarryd for
	 * @param exclude
	 *                    true if we are to exclude the TIER_10 element from the
	 *                    list
	 * @return the best possible emblem
	 */
	public static Optional<BountyEmblems> getBest(Stoner stoner, boolean exclude) {
	List<BountyEmblems> emblems = EMBLEMS.stream().filter(exclude(stoner, exclude)).collect(Collectors.toList());
	...
	/**
	 * Under certain circumstances we may want to exclude the tier 10 emblem from
	 * the possible stream of tiers we're trying to obtain. This is the case when
	 * dropping an emblem.
	 * 
	 * @param stoner
	 *                    the stoner that contains the emblem
	 * @param exclude
	 *                    true if the emblem is to be excluded, otherwise false
	 * @return a positive test if the stoner has the item and the other condition is
	 *         met
	 */
	private static Predicate<BountyEmblems> exclude(Stoner stoner, boolean exclude) {
	return emblem -> stoner.getBox().hasItemId(new Item(emblem.getItemId())) && (!exclude || exclude && !emblem.equals(TIER_10));
	}
