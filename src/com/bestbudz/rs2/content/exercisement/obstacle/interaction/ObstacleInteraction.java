        ...
        new Task(stoner, 1, false) {
          ...
          public void execute() {
            if (nextObstacle != null && stoner.getLocation().equals(nextObstacle.getEnd())) {
              Location delta =
                  Utility.delta(
                      nextObstacle == null ? start : nextObstacle.getStart(),
                      nextObstacle == null ? end : nextObstacle.getEnd());
              stoner
                  .getUpdateFlags()
                  .sendFaceToDirection(
                      nextObstacle.getStart().getX() + delta.getX(),
                      nextObstacle.getStart().getY() + delta.getY());
              stoner.getProfession().lock(4);
              nextObstacle.getType().getInteraction().start(stoner);
              nextObstacle
                  .getType()
                  .getInteraction()
                  .onExecution(stoner, nextObstacle.getStart(), nextObstacle.getEnd());
              ...
              stoner
                  .getUpdateFlags()
                  .sendFaceToDirection(
                      (nextObstacle == null ? start.getX() : nextObstacle.getStart().getX())
                          + delta.getX(),
                      (nextObstacle == null ? start.getY() : nextObstacle.getStart().getY())
                          + delta.getY());
            ...
            if (!started) {
              ...
              if (ordinal > -1 && Utility.random(45) == 0) {
                ...
                stoner.send(
                    new SendMessage("<col=C60DDE>There appears to be a wild Grace mark near you."));
              }
            }
            ...
            if (ordinal > -1) {
              if (ordinal == 0) {
                ...
                stoner
                    ...
                    .set(
                        "EXERCISEMENT_FLAGS",
                        stoner.getAttributes().getInt("EXERCISEMENT_FLAGS") | (1 << ordinal));
              }
            }
