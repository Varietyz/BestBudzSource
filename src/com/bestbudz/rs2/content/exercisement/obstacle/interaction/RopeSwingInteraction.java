...
public interface RopeSwingInteraction extends ObstacleInteraction {
	...
	TaskQueue.queue(new Task(stoner, 1, true, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.CURRENT_ACTION) {
		@Override
		public void execute() {
		if (stoner.getLocation().equals(new Location(start.getX() + xMod, start.getY() + yMod))) {
			stop();
			return;
		}
		...
		TaskQueue.queue(new ForceMoveTask(stoner, 2, new Location(stoner.getX() + xMod, stoner.getY() + yMod), new Location(modX, modY), 751, 28, 66, obj.getFace() == 0 ? 2 : 0));
		}
	});
	}
