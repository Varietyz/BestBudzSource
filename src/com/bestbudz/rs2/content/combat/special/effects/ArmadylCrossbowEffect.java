...
public class ArmadylCrossbowEffect implements CombatEffect {
	...
	public void execute(Stoner attacker, Entity target) {
		if (target.isNpc()) {
			...
			double penetrationMultiplier = 0.5 + (divineFavor * 0.15);
			if (dragonBolts) penetrationMultiplier += 0.2;
			if (enchantedBolts) penetrationMultiplier += 0.1;
			...
			if (divineFavor > 2.0) {
				int guidedShots = (int)(divineFavor - 1.0);
			...
			if (dragonBolts && divineFavor > 1.8) {
				...
				double explosionChance = (divineFavor - 1.8) * 25;
