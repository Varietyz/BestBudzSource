...
public class SaradominGodswordEffect implements CombatEffect {
  ...
  public void execute(Stoner p, Entity e) {
    ...
    long hp = (int) (dmg * 0.5D);
    long pray = (int) (dmg * 0.25D);

    if ((hp > 9) && (p.getGrades()[3] < p.getMaxGrades()[3])) {
      int tmp55_54 = 3;
      long[] tmp55_51 = p.getGrades();
    ...
    else if ((pray == 0) && (hp > 0)) message = "You regenerate " + hp + " Life.";
    else if ((hp == 0) && (pray > 0)) message = "You regenerate " + pray + " Necromance.";
    else {
      return;
    }
