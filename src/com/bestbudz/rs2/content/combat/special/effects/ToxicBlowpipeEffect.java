...
public class ToxicBlowpipeEffect implements CombatEffect {
	...
	public void execute(Stoner attacker, Entity target) {
		...
		long healingAmount = (long)(totalDamage * (0.4 + venomMastery * 0.15));
		...
		if (target.isNpc() && venomMastery > 1.5) {
			Mob victim = com.bestbudz.rs2.entity.World.getNpcs()[target.getIndex()];
			if (victim != null) {
				int venomDamage = (int)(4 + venomMastery * 2);
				int venomDuration = (int)(8 + venomMastery);
		...
		if (venomMastery > 2.0) {
			int rapidFireBonus = (int)((venomMastery - 2.0) * 20);
			attacker.getAttributes().set("rapid_fire_mode", rapidFireBonus);
			attacker.getAttributes().set("rapid_fire_end", System.currentTimeMillis() + 15000);
		...
		if (totalDamage > 30 && venomMastery > 2.2) {
			...
			int cloudDuration = (int)(5 + venomMastery);
		...
		if (venomMastery > 2.5) {
			attacker.getAttributes().set("venom_immunity", System.currentTimeMillis() + 25000);
		...
		if (venomMastery > 3.0 && target.isNpc()) {
			...
			if (victim != null) {
				...
				if (healthPercent < 0.15) {
					double executeChance = (venomMastery - 3.0) * 20;
