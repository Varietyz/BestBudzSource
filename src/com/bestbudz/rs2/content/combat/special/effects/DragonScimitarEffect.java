...
public class DragonScimitarEffect implements CombatEffect {
	...
	if (stoner.getLastDamageDealt() > 0) {
		...
		stoner.send(new SendMessage("You have cancelled " + opponent.getStoner().getUsername() + "'s protection necromance."));
		opponent.getStoner().send(new SendMessage("Your protection necromance's been cancelled by " + stoner.getUsername()));
	}
	}
