...
public class BandosGodswordEffect implements CombatEffect {
	...
	if (e.getGrades()[BGS_DRAIN_IDS[id]] - drain < 0) {
		int diff = drain - e.getGrades()[BGS_DRAIN_IDS[id]];
		e.getGrades()[BGS_DRAIN_IDS[id]] = 0;
		p.getClient().queueOutgoingPacket(new SendMessage("You drain your opponents " + com.bestbudz.rs2.content.profession.Professions.PROFESSION_NAMES[BGS_DRAIN_IDS[id]] + " down to 0."));
		...
		if (id < BGS_DRAIN_IDS.length - 1) {
			...
			short[] tmp202_194 = e.getGrades();
			tmp202_194[tmp202_201] = ((short) (tmp202_194[tmp202_201] - diff));
			p2.getProfession().update(BGS_DRAIN_IDS[id]);
			p.getClient().queueOutgoingPacket(new SendMessage("You drain some of your opponents " + com.bestbudz.rs2.content.profession.Professions.PROFESSION_NAMES[BGS_DRAIN_IDS[id]] + "."));
		}
	} else {
		int tmp277_276 = BGS_DRAIN_IDS[id];
		short[] tmp277_269 = e.getGrades();
