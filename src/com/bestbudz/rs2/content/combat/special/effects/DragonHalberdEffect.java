...
public class DragonHalberdEffect implements CombatEffect {
	...
	public void execute(Stoner attacker, Entity target) {
		if (target.isNpc()) {
			...
			double sweepMultiplier = 0.4 + (dragonicReach * 0.2);
			...
			if (areaDamage > 0) {
				...
				target.getAttributes().set("halberd_sweep_radius", 2);
			...
			if (dragonicReach > 2.0) {
				double techniqueChance = (dragonicReach - 2.0) * 20;
			...
			if (dragonicReach > 2.8) {
				double perfectChance = (dragonicReach - 2.8) * 25;
				if (com.bestbudz.core.util.Utility.random(100) < perfectChance) {
					...
					target.getAttributes().set("perfect_sweep_radius", 3);
