 ...
 * @information Represents an assaulters/victims rolls in sagittarius combat
 */
public class RangeFormulas {

	/**
	 * Calculates the victims aegis to sagittarius assaults
	 * 
	 * @param stoner
	 * @return
	 */
	public static int calculateRangeAegis(Entity defending) {
	...
	if (!defending.isNpc()) {
		...
		if (defending.getBonuses() != null && defending.getGrades() != null) {
			return defending.getGrades()[1] + defending.getBonuses()[9] + (defending.getBonuses()[9] / 2);
		}
		return 0;
	}

	int aegisGrade = defender.getProfession().getGrades()[1];
	...
	/**
	 * 
	 * Calculates the assaulters assault
	 * 
	 * @param c
	 * @return
	 */
	...
	if (!entity.isNpc()) {
		...
		if (entity.getGrades() != null) {
			int rangeGrade = entity.getGrades()[4];
	...
	public static final double[][] SAGITTARIUS_NECROMANCE_MODIFIERS = { { 3.0D, 0.05D }, { 11.0D, 0.1D }, { 19.0D, 0.15D } };
	...
	if ((weapon.getSagittariusDefinition().getType() == SagittariusWeaponDefinition.SagittariusTypes.THROWN) || (weapon.getSagittariusDefinition().getArrows() == null) || (weapon.getSagittariusDefinition().getArrows().length == 0)) {
	...
	for (int i = 0; i < SAGITTARIUS_NECROMANCE_MODIFIERS.length; i++) {
		if (stoner.getNecromance().active(Necromance.values()[(int) SAGITTARIUS_NECROMANCE_MODIFIERS[i][0]])) {
			pBonus += SAGITTARIUS_NECROMANCE_MODIFIERS[i][1];
			break;
		}
	}

	int str = stoner.getProfession().getGrades()[4];
