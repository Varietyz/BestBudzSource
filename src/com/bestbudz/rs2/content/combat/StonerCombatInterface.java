...
public class StonerCombatInterface implements CombatInterface {
	...
	if (stoner.getSpecialAssault().isInitialized() && stoner.getCombat().getCombatType() != CombatTypes.MAGE) {
	...
	if (assaulting.isNpc()) {
		...
		if (MobConstants.MERCENARY_REQUIREMENTS.get(mob.getId()) != null) {
			...
			if (stoner.getProfession().getGrades()[Professions.MERCENARY] < requirement) {
				stoner.send(new SendMessage("You need a mercenary grade of " + requirement + " to assault this monster!"));
				return false;
			}
		}
	}

	if (stoner.getSpecialAssault().isInitialized() && stoner.getCombat().getCombatType() != CombatTypes.MAGE && (!stoner.getController().canUseSpecialAssault(stoner) || !SpecialAssaultHandler.hasSpecialAmount(stoner))) {
	...
	if (type == CombatTypes.MAGE && !stoner.getMage().getSpellCasting().canCast())
		return false;
	...
	if (!stoner.inMultiArea() || !assaulting.inMultiArea()) {
		if (stoner.getCombat().inCombat() && stoner.getCombat().getLastAssaultedBy() != stoner.getCombat().getAssaulting()) {
			stoner.getClient().queueOutgoingPacket(new SendMessage("You are already under assault."));
			return false;
		}

		if (assaulting.getCombat().inCombat() && assaulting.getCombat().getLastAssaultedBy() != stoner && !stoner.getSummoning().isFamiliar(assaulting.getCombat().getLastAssaultedBy())) {
			stoner.getClient().queueOutgoingPacket(new SendMessage("This " + (stoner.getCombat().getAssaulting().isNpc() ? "monster" : "stoner") + " is already under assault."));
			return false;
		}
	...
	if (!assaulting.isNpc()) {
		...
		if (other != null && !stoner.getController().canAssaultStoner(stoner, other))
			return false;
	} else {
		Mob mob = World.getNpcs()[assaulting.getIndex()];
		...
		if (mob != null) {
			if (mob instanceof FamiliarMob) {
				...
				if (mob.getOwner().equals(stoner)) {
					stoner.getClient().queueOutgoingPacket(new SendMessage("You cannot assault your own familiar!"));
					return false;
				}
			}
			...
			if (!MercenaryMonsters.canAssaultMob(stoner, mob))
				return false;

			if (!mob.getDefinition().isAssaultable() || (mob.getOwner() != null && !mob.getOwner().equals(stoner))) {
	...
	} else if (stoner.getCombat().getCombatType() == CombatTypes.MAGE && ItemCheck.hasDFireShield(stoner) && stoner.getMage().isDFireShieldEffect() && damage < 15) {
		return 15;
	}
	...
	if (!stoner.canTakeDamage() || stoner.isImmuneToHit() || stoner.getMage().isTeleporting() && !stoner.getController().isSafe(stoner)) {
		return;
	}
			...
			if (mob != null && MobConstants.isDragon(mob)) {
				if (ItemCheck.isWearingAntiDFireShield(stoner) && (hit.getType() == Hit.HitTypes.MAGE)) {
					...
					if (stoner.hasFireImmunity()) {
						stoner.getClient().queueOutgoingPacket(new SendMessage("You resist all of the dragonfire."));
						hit.setDamage(0);
					} else {
						stoner.getClient().queueOutgoingPacket(new SendMessage("You manage to resist some of the dragonfire."));
						hit.setDamage((int) (hit.getDamage() * 0.3));
					}
				} else if (hit.getType() == Hit.HitTypes.MAGE && stoner.hasSuperFireImmunity()) {
					stoner.getClient().queueOutgoingPacket(new SendMessage("You reset all of the dragonfire."));
					hit.setDamage(0);
				} else if (hit.getType() == Hit.HitTypes.MAGE && stoner.hasFireImmunity()) {
					stoner.getClient().queueOutgoingPacket(new SendMessage("You manage to resist some of the dragonfire."));
					hit.setDamage((int) (hit.getDamage() * 0.5));
				} else if ((hit.getType() == Hit.HitTypes.MAGE)) {
					stoner.getClient().queueOutgoingPacket(new SendMessage("You are horribly burned by the dragonfire."));
				}
			}
		} else {
			Stoner p = World.getStoners()[hit.getAssaulter().getIndex()];

			if (p != null && !stoner.getController().canAssaultStoner(p, stoner) || !stoner.getController().canAssaultStoner(stoner, p)) {
				return;
			}
		}
	...
	if (stoner.getGrades()[3] > 0 && !stoner.isDead()) {
		if (stoner.getEquipment().isWearingItem(11090)) {
			if (stoner.getProfession().getGrades()[3] > 0 && stoner.getProfession().getGrades()[3] <= stoner.getMaxGrades()[3] * 0.20) {
				stoner.send(new SendMessage("The Phoenix necklace of life saves you but was destroyed in the process"));
		...
		if (stoner.getEquipment().isWearingItem(2570) && !stoner.inDuelArena()) {
			if (stoner.getProfession().getGrades()[3] > 0 && stoner.getProfession().getGrades()[3] <= stoner.getMaxGrades()[3] * 0.10) {
				stoner.getEquipment().remove(new Item(2570, 1));
				stoner.getMage().teleport(3085, 3491, 0, TeleportTypes.SPELL_BOOK);
				stoner.send(new SendMessage("The Ring of life has saved you; but was destroyed in the process."));
	...
	if (hit.getAssaulter() != null) {
		...
		if (stoner.getEquipment().getItems()[3] != null && stoner.getEquipment().getItems()[3].getId() == 12006) {
	...
	if (stoner.getCombat().getCombatType() == CombatTypes.SAGITTARIUS && stoner.getSpecialAssault().isInitialized()) {
		Item weapon = stoner.getEquipment().getItems()[3];
	...
	case MAGE:
		if (success) {
			MageEffects.doMageEffects(stoner, assault, stoner.getMage().getSpellCasting().getCurrentSpellId());
		}
	...
	if (stoner.getSpecialAssault().isInitialized() && stoner.getCombat().getCombatType() != CombatTypes.MAGE) {
		...
		// BoltSpecials.checkForBoltSpecial(stoner, entity);
	...
	if (stoner.getCombat().getCombatType() != CombatTypes.MAGE) {
		StonerSounds.sendSoundForId(stoner, stoner.getSpecialAssault().isInitialized(), stoner.getEquipment().getItems()[3] != null ? stoner.getEquipment().getItems()[3].getId() : 0);
	}
	...
	if (stoner.getAttributes().get(PestControlGame.PEST_GAME_KEY) != null) {
		stoner.getAttributes().set(PestControlGame.PEST_DAMAGE_KEY, stoner.getAttributes().get(PestControlGame.PEST_DAMAGE_KEY) != null ? stoner.getAttributes().getInt(PestControlGame.PEST_DAMAGE_KEY) + hit.getDamage() : hit.getDamage());
	}
	...
	else if (stoner.getMelee().isToragEffectActive())
		BarrowsSpecials.doToragEffect(stoner, entity);
