...
public class Sagittarius {
	...
	if (Utility.randomNumber(RangeFormulas.calculateRangeAegis(entity.getCombat().getAssaulting())) > Utility.randomNumber(RangeFormulas.calculateRangeAssault(entity))) {
		success = false;
	} else {
		success = true;
	}

	int damage = entity.getCorrectedDamage(Combat.next(entity.getMaxHit(CombatTypes.SAGITTARIUS) + 1));

	Hit hit = new Hit(entity, (success) || (entity.isIgnoreHitSuccess()) ? damage : 0, Hit.HitTypes.SAGITTARIUS);
