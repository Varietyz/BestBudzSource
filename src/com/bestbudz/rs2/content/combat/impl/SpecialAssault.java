...
public class SpecialAssault {
  ...
  public boolean doInstantGraniteMaulSpecial() {
    return (stoner.getCombat().getAssaulting() != null)
        && (stoner.getCombat().withinDistanceForAssault(stoner.getCombat().getCombatType(), false))
        && (stoner.canAssault());
  }
  ...
  public void toggleSpecial() {
    ...
    if ((weapon.getId() == 4153) && (!initialized) && (doInstantGraniteMaulSpecial())) {
      ...
      if (weapon.getId() == 15241) {
        ...
        if (a > 0) {
          if (initialized) stoner.getCombat().setAssaultTimer(a + 2);
          else {
  ...
  public void updateSpecialBar(int weaponId) {
    ...
    if (def != null) {
      ...
      if (weaponId == 15486) stoner.getClient().queueOutgoingPacket(new SendConfig(78, 1));
      else {
