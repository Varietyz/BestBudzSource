...
public class ItemInteraction {
	...
	public static boolean clickPouch(Stoner p, int id, int option) {
	if (id == 5509 || id == 5510 || id == 5512 || id == 5514) {
		if (option == 1) {
			...
			if ((index > 0) && (p.getMaxGrades()[20] < index * 25)) {
				p.getClient().queueOutgoingPacket(new SendMessage("You need a Consumer grade of " + index * 25 + " to use this pouch."));
				return true;
			}
			int amount = ess - p.getPouches()[index];
			int invAmount = p.getBox().getItemAmount(1436);
			if (invAmount == 0) {
			...
			p.getBox().remove(1436, amount, true);
			...
			int add = p.getBox().add(1436, p.getPouches()[index]);
