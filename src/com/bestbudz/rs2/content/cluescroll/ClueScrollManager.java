...
public enum ClueScrollManager {
	...
	/**
	 * Declares all the functioning clue scrolls to make them accessable in game.
	 */
	public static void declare() {
	CROSS_TRAILS.add(100, new Item(995, 500)); // BestBucks
	CROSS_TRAILS.add(100, new Item(995, 750)); // BestBucks
	CROSS_TRAILS.add(100, new Item(995, 1000)); // BestBucks
	CROSS_TRAILS.add(100, new Item(995, 5000)); // BestBucks
	CROSS_TRAILS.add(100, new Item(995, 5500)); // BestBucks
	...
	CROSS_TRAILS.add(75, new Item(10284, 1)); // Mage comp bow
	...
	EASY.add(17, new Item(10366, 1)); // Amulet of mage (t)
	...
	MEDIUM.add(10, new Item(2577, 1)); // Sagittarius boots
	...
	MEDIUM.add(35, new Item(10364, 1)); // Vigour amulet (t)
	...
	HARD.add(200, new Item(859, 1)); // Mage longbow
	HARD.add(200, new Item(861, 1)); // Mage shortbow
	...
	CLUE_SCROLLS.put(2678, new EmoteScroll(2678, ClueDifficulty.EASY, new Item[] { null, null, new Item(1654), new Item(1237), null, null, null, null, null, null, null, null, new Item(1635), null }, new Location(2977, 3240, 0), 11, 2113, "", "", "Shrug in the quarry near Rimmington,", "quip a gold necklace, a gold ring", "and a bronze spear.", "", "", ""));
	...
	CLUE_SCROLLS.put(2681, new EmoteScroll(2681, ClueDifficulty.EASY, new Item[] { null, null, new Item(1694), null, new Item(1103), null, null, null, null, null, null, null, new Item(1639), null }, new Location(2611, 3393, 0), 2, 2106, "", "", "Dance a jig by the entrance to the", "Fisher Guild, equip a sapphire", "amulet, an emerald ring and a", "bronze chainbody", "", "", ""));
	...
	/* Emote Scrolls - Hard */
	CLUE_SCROLLS.put(2722, new EmoteScroll(2722, ClueDifficulty.HARD, new Item[] { null, null, null, new Item(1347), null, new Item(2890), null, new Item(2493), null, null, null, null, null, null }, new Location(2587, 3420, 0), 2, 2110, "", "", "Blow a raspberry in the Fisher Guild", "bank. Beware of double agents!", "Equip an elemental shield, blue", "dragonhide chaps and a rune", "warhammer", "", ""));
	 ...
	 *
	 * @param stoner
	 *                       - The stoner to generate the clue scroll for.
	 * @param difficulty
	...
	List<ClueScroll> scrolls = CLUE_SCROLLS.values().stream().filter(scroll -> checkScroll(stoner, scroll, difficulty)).collect(Collectors.toList());
	...
	private static final boolean checkScroll(Stoner stoner, ClueScroll scroll, ClueDifficulty difficulty) {
	...
	for (ClueScroll s : difficultyScrolls) {
		if (stoner.getBank().hasItemId(s.getScrollId()) || stoner.getBox().hasItemId(s.getScrollId())) {
			return false;
		}
	}

	return scroll.getDifficulty().equals(difficulty) && !stoner.getBank().hasItemId(scrollId) && !stoner.getBox().hasItemId(scrollId);
	}
	 ...
	 * 
	 * @param stoner
	 *                       - The stoner receiving the reward.
	 * @param difficulty
	 *                       - The rewards table.
	 */
	...
	for (int i = 0; i < items.items.length; i++) {
		...
		if (items.items[i].getDefinition().getGeneralPrice() >= 500_000 || items.items[i].getDefinition().getName().contains("ornament") || items.items[i].getDefinition().getName().contains("Gilded") || items.items[i].getDefinition().getName().contains("Sagittarius") || items.items[i].getDefinition().getName().contains("Robin") || items.items[i].getDefinition().getName().contains("3rd")) {
			World.sendGlobalMessage("<img=8> <col=C42BAD>" + stoner.deterquarryIcon(stoner) + Utility.formatStonerName(stoner.getUsername()) + " has recieved " + Utility.deterquarryIndefiniteArticle(items.items[i].getDefinition().getName()) + " " + items.items[i].getDefinition().getName() + " from a " + Utility.capitalize(difficulty.name().toLowerCase()) + " clue scroll.");
	...
	for (int i = 0; i < stoner.getBox().getItems().length; i++) {
		if (stoner.getBox().getItems()[i] != null && getClue(stoner.getBox().getItems()[i].getId()) != null) {
			return true;
		}
	}
	return false;
