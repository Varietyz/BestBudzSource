package com.bestbudz.rs2.content.cannacredits;
...
public class MysteryBoxMinigame {
  ...
  private static final int EXPENSIVE_AMOUNT = 1_500_000;

  private static final int CREDITS_REQUIRED = 200;

  private static final int INTERFACE_ID = 17000;

  private static final int CONTAINER_ID = 17002;

  private static final String MESSAGE_COLOR = "<col=255>";

  public static Chance<Item> available =
      new Chance<Item>(
          Arrays.asList(
  ...
  public static boolean can(Stoner stoner) {
    ...
    if (stoner.getCredits() < CREDITS_REQUIRED) {
      stoner.send(
          new SendMessage(MESSAGE_COLOR + "You do not have enough cannacredits to do this!"));
      return false;
    }
    ...
    stoner.send(
        new SendMessage(
            MESSAGE_COLOR
                ...
                + Utility.getAOrAn(itemDef.getName())
                + " "
                + itemDef.getName()
                + "!"));
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        formatter.format(common * 100 / trials) + "% - common: " + Utility.format(common));
    System.out.println(
        formatter.format(uncommon * 100 / trials) + "% - uncommon: " + Utility.format(uncommon));
    System.out.println(
        formatter.format(rare * 100 / trials) + "% - rares: " + Utility.format(rare));
    System.out.println(
        formatter.format(very_rare * 100 / trials)
            + "% - very rares: "
            + Utility.format(very_rare));
  }
}
