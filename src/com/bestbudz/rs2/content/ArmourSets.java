...
public enum ArmourSets {
	...
	HALLOWEEN_SET(13175, new Item(1053), new Item(1055), new Item(1057)),
	PARTY_HAT_SET(13173, new Item(1038), new Item(1040), new Item(1042), new Item(1044), new Item(1046), new Item(1048));

	private static final HashMap<Integer, ArmourSets> armour = new HashMap<Integer, ArmourSets>();
	...
	ArmourSets(int armourID, Item... items) {
	...
	stoner.getBox().remove(data.armourID);
	stoner.getBox().addItems(data.items);
	stoner.send(new SendMessage("You successfully open the " + GameDefinitionLoader.getItemDef(data.armourID).getName() + "."));
	return true;
	}
}