...
public class CrystalChest {
  ...
  public static final Item[] UNCOMMON_CHEST_REWARDS = {
  ...
  public static final Item[] RARE_CHEST_REWARDS = {
    new Item(ItemNames.UNCUT_DRAGONSTONE),
    new Item(ItemNames.UNCUT_ONYX),
    new Item(ItemNames.DRAGON_SPEAR),
  ...
  public static void createKey(final Stoner stoner) {
    if (stoner.getBox().contains(KEY_HALVES)) {
      ...
      DialogueManager.sendItem1(
          stoner, "You have combined the two parts to form a key.", KEY.getId());
    }
  }
  ...
  public static void searchChest(final Stoner stoner, final int x, final int y) {
    if (stoner.getBox().contains(KEY)) {
      ...
      stoner.send(
          new SendMessage(
              "You find "
                  + Utility.deterquarryIndefiniteArticle(itemReceived.getDefinition().getName())
                  + " "
                  + itemReceived.getDefinition().getName()
                  + " in the chest."));
