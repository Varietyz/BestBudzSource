...
public class DropTable {
	...
	if (npcDef == null) {
		stoner.send(new SendMessage("It appears this NPC is nulled! Please report it to a staff member."));
		return;
	}
	...
	if (itemDef == null) {
		stoner.send(new SendMessage("It appears this item is nulled! Please report it to a staff member."));
		return;
	}

	stoner.send(new SendUpdateItemsAlt(59757, itemDef.getId(), 1, 0));
	stoner.send(new SendString("</col>Item: @gre@" + itemDef.getName(), 59753));
	stoner.send(new SendString("</col>Price: @gre@" + Utility.formatPrice(itemDef.getGeneralPrice()), 59754));
	stoner.send(new SendString("</col>Tradeable: @gre@" + Utility.formatBoolean(itemDef.isTradable()), 59755));
	stoner.send(new SendString("</col>Noted: @gre@" + Utility.formatBoolean(itemDef.isNote()), 59756));
