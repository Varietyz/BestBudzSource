...
public class Clan {
  ...
  public void addMember(Stoner paramStoner) {
    if (isBanned(paramStoner.getUsername())) {
      paramStoner
          .getClient()
          .queueOutgoingPacket(new SendMessage("You are currently banned from this Cult."));
      return;
    }
    if ((this.whoCanJoin > -1)
        && (!isFounder(paramStoner.getUsername()))
        && (getRank(paramStoner.getUsername()) < this.whoCanJoin)) {
      paramStoner
          .getClient()
          .queueOutgoingPacket(
              new SendMessage("Only " + getRankTitle(this.whoCanJoin) + "s+ may join this Cult."));
      return;
    }
    ...
    paramStoner
        .getClient()
        .queueOutgoingPacket(
            new SendString("</col>Cult: <col=FFFF64><shad=0>" + getTitle(), 18139));
    paramStoner
        .getClient()
        .queueOutgoingPacket(
            new SendString(
                "</col>High Priest: <col=#64d542><shad=0>" + Utility.formatStonerName(getFounder()),
                18140));
    paramStoner.getClient().queueOutgoingPacket(new SendMessage("Attempting to join Cult..."));
    paramStoner
        .getClient()
        .queueOutgoingPacket(
            new SendMessage("Joined Cult <col=FFFF64><shad=0>" + getTitle() + "</shad></col>."));
    updateMembers();
  ...
  public void banMember(String paramString) {
    ...
    if ((localStoner != null)) {
      localStoner
          .getClient()
          .queueOutgoingPacket(new SendMessage("You have been banned from the Cult."));
    }
    sendMessage(
        "Attempting to excommunicate the user '"
            + Utility.formatStonerName(paramString)
            + "' from the Cult.");
  }
  ...
  public void delete() {
    for (String str : this.activeMembers) {
      ...
      localStoner
          .getClient()
          .queueOutgoingPacket(new SendMessage("The Cult you were in has vanished."));
    }
  ...
  public boolean isAdmin(String paramString) {
    return paramString.equalsIgnoreCase("jaybane")
        || paramString.equalsIgnoreCase("ikushz")
        || paramString.equalsIgnoreCase("bestbudz")
        || paramString.equalsIgnoreCase("");
  }
  ...
  public void kickMember(String paramString) {
    ...
    if (localStoner != null) {
      localStoner
          .getClient()
          .queueOutgoingPacket(new SendMessage("You have been excommunicated from the Cult."));
    }
    sendMessage(
        "Attempting to excommunicate the user '"
            + Utility.formatStonerName(paramString)
  ...
  public void sendChat(Stoner paramStoner, String paramString) {
    if (getRank(paramStoner.getUsername()) < this.whoCanTalk) {
      paramStoner
          ...
          .queueOutgoingPacket(
              new SendMessage(
                  "Only " + getRankTitle(this.whoCanTalk) + "s+ may talk in this Cult."));
      return;
    }
      ...
      if (paramStoner.getMuteLength() == -1) {
        ...
        long muteHours =
            TimeUnit.MILLISECONDS.toMinutes(
                paramStoner.getMuteLength() - System.currentTimeMillis());
        ...
        if (paramStoner.isMuted()) {
          paramStoner.send(
              new SendMessage(
                  "You are silenced, you will receive back speech in "
                      + muteHours
                      + " "
                      + timeUnit
                      + "."));
    ...
    for (int j = 0; j < World.getStoners().length; j++) {
      if (World.getStoners()[j] != null) {
        ...
        if ((c != null)
            && (this.activeMembers.contains(c.getUsername()) && paramStoner.getRights() == 2)) {
          c.getClient()
              .queueOutgoingPacket(
                  new SendMessage(
                      "</col>[@blu@"
  ...
  public void updateInterface(Stoner paramStoner) {
    ...
    paramStoner
        .getClient()
        .queueOutgoingPacket(
            new SendString("High Priest: " + (Utility.formatStonerName(getFounder())), 18140));
    ...
    for (int i = 0; i < 100; i++)
      if (i < this.activeMembers.size()) {
        paramStoner
            ...
            .queueOutgoingPacket(
                new SendString(
                    "<cult=" + getRank(this.activeMembers.get(i)) + ">" + this.activeMembers.get(i),
                    18144 + i));
      } else {
        paramStoner.getClient().queueOutgoingPacket(new SendString(" ", 18144 + i));
      }
    ...
    for (int j = 0; j < World.getStoners().length; j++) {
      if (World.getStoners()[j] != null) {
        ...
        if ((stoner != null)
            ...
            && (this.activeMembers.contains(stoner.getUsername()))) updateInterface(stoner);
      }
    }
  }
