...
public class Yelling {
  ...
  public static void yell(Stoner stoner, String message) {
    ...
    if (rights == 0) {
      send =
          "[@whi@"
              + Utility.capitalize(stoner.getYellTitle())
              + "</col>] <img=11>@whi@"
              + stoner.getUsername()
              + "</col>: "
              + message;
      ...
      send =
          "[<col=D17417>A.BestBud</col>]  <img=1><col=D17417>"
              + stoner.getUsername()
              + "</col>: "
              + message;
    } else if (rights == 3) {
    ...
    if (stoner.isYellMuted()) {
      stoner
          .getClient()
          .queueOutgoingPacket(new SendMessage("You are muted are not allowed to yell."));
      return;
    }
    ...
    if (message.contains("<")) {
      stoner
          .getClient()
          .queueOutgoingPacket(new SendMessage("You cannot use text arguments when yelling."));
      return;
    }

    for (Stoner i : World.getStoners())
      if (i != null && send != null) i.getClient().queueOutgoingPacket(new SendMessage(send));
  }
}
