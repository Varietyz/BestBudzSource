...
public final class StonerSave {
	...
	public static final class StonerCultivation {
		...
		try {
			...
			if (details.lastCultivationTimer > 0) {
				stoner.getCultivation().setCultivationTimer(details.cultivationTimer + TimeUnit.MILLISECONDS.toMinutes(millis - details.lastCultivationTimer));
			} else {
			...
			stoner.getCultivation().getSpecialPlantOne().specialPlantStages = details.specialPlantOneStages;
			stoner.getCultivation().getSpecialPlantOne().specialPlantSaplings = details.specialPlantOneSeeds;
			stoner.getCultivation().getSpecialPlantOne().specialPlantState = details.specialPlantOneState;
			stoner.getCultivation().getSpecialPlantOne().specialPlantTimer = details.specialPlantOneTimer;
			stoner.getCultivation().getSpecialPlantOne().diseaseChance = details.specialPlantOneDiseaseChance;
			stoner.getCultivation().getSpecialPlantOne().hasFullyGrown = details.specialPlantOneHasFullyGrown;
			stoner.getCultivation().getSpecialPlantTwo().specialPlantStages = details.specialPlantTwoStages;
			stoner.getCultivation().getSpecialPlantTwo().specialPlantSeeds = details.specialPlantTwoSeeds;
			stoner.getCultivation().getSpecialPlantTwo().specialPlantState = details.specialPlantTwoState;
			stoner.getCultivation().getSpecialPlantTwo().specialPlantTimer = details.specialPlantTwoTimer;
			stoner.getCultivation().getSpecialPlantTwo().diseaseChance = details.specialPlantTwoDiseaseChance;
			stoner.getCultivation().getSpecialPlantTwo().hasFullyGrown = details.specialPlantTwoHasFullyGrown;
		...
		this.specialPlantOneDiseaseChance = stoner.getCultivation().getSpecialPlantOne().diseaseChance;
		this.specialPlantOneHasFullyGrown = stoner.getCultivation().getSpecialPlantOne().hasFullyGrown;
		...
		this.specialPlantTwoDiseaseChance = stoner.getCultivation().getSpecialPlantTwo().diseaseChance;
		this.specialPlantTwoHasFullyGrown = stoner.getCultivation().getSpecialPlantTwo().hasFullyGrown;
		...
		BufferedWriter writer = new BufferedWriter(new FileWriter("./data/characters/cultivation/" + stoner.getUsername() + ".json", false));
		try {
	...
	public static final class StonerContainer {
		...
		try {
			...
			if (details.bank != null) {
				...
				if (total != tabs) {
					...
					DialogueManager.sendStatement(stoner, "@dre@There was an issue loading your bank tabs.", "@dre@Your bank tabs have been collapsed as a safety measure.");
				}
			}
		...
		bobBox = (stoner.getSummoning().isFamilarBOB() ? stoner.getSummoning().getContainer().getItems() : null);
		...
		public void parseDetails(Stoner stoner) throws IOException {
		BufferedWriter writer = new BufferedWriter(new FileWriter("./data/characters/containers/" + stoner.getUsername() + ".json", false));
		try {
			writer.write(StonerSave.GSON.toJson(this));
			writer.flush();
		} finally {
	...
	public static final class StonerDetails {
		...
		try {
			File file = new File("./data/characters/details/" + NameUtil.uppercaseFirstLetter(stoner.getUsername()) + ".json");
			...
			if (details.summoningFamiliar != -1) {
				stoner.getAttributes().set("summoningfamsave", Integer.valueOf(details.summoningFamiliar));
			}
			...
			stoner.setMoneySpent(details.moneySpent);
			...
			if (details.professionsGrade != null) {
				for (int i = 0; i < details.professionsGrade.length; i++) {
					stoner.getGrades()[i] = details.professionsGrade[i];
				}
			}
			if (details.experience != null) {
				for (int i = 0; i < details.experience.length; i++) {
					stoner.getMaxGrades()[i] = stoner.getProfession().getGradeForExperience(i, details.experience[i]);
				}
			}
			...
			if (details.stonerProperties != null) {
				for (Object attribute : details.stonerProperties.keySet()) {
					stoner.getAttributes().getAttributes().put(attribute, details.stonerProperties.get(attribute));
				}
			}
		...
		private final short[] professionsGrade;
		...
		moneySpent = stoner.getMoneySpent();
		...
		summoningFamiliar = (stoner.getSummoning().getFamiliarData() != null ? stoner.getSummoning().getFamiliarData().mob : -1);
	...
	if (!StonerCultivation.loadDetails(p)) {
		return false;
	}
	...
	public static synchronized final void save(Stoner p) {
	try {
