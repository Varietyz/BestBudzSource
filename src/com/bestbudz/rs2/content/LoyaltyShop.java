...
public class LoyaltyShop {
    ...
    if (!stoner.unlockedTitles.contains(button.getTitle())) {
      ...
      if (button.getPrice() instanceof AchievementList) {
        ...
        if (stoner.getStonerAchievements().get(achievement) != achievement.getCompleteAmount()) {
          stoner.send(
              new SendMessage(
                  "<col=128>Completion of the achievement '"
                      + achievement.getName()
                      + "' is required."));
          return true;
        }
      ...
      if (button.ordinal() >= TitleButton.RED.ordinal()
          && button.ordinal() <= TitleButton.WHITE.ordinal()) {
        ...
        if (!stoner
            .getBox()
            .hasItemAmount(995, Integer.parseInt(String.valueOf(button.getPrice())))) {
          stoner.send(new SendMessage("<col=128>You need more BestBucks to buy this color."));
          return true;
        }
        stoner.setStonerTitle(
            StonerTitle.create(
                stoner.getStonerTitle().getTitle(),
                button.getTitle().getColor(),
                stoner.getStonerTitle().isSuffix()));
        stoner.setAppearanceUpdateRequired(true);
        stoner.getBox().remove(995, Integer.parseInt(String.valueOf(button.getPrice())));
        stoner.send(
            new SendMessage(
                "<col=128>You have changed your title color to '<col="
                    + Integer.toHexString(button.getTitle().getColor())
                    + ">"
                    + button.name().toLowerCase().replaceAll("_", " ")
                    + "</col>'!"));
      } else {
        ...
        if (button.getPrice() instanceof Integer) {
          stoner.setCredits(
              stoner.getCredits() - Integer.parseInt(String.valueOf(button.getPrice())));
        }
        stoner.unlockedTitles.add(button.getTitle());
        stoner.send(new SendConfig(1040 + button.ordinal(), 1));
        stoner.send(
            new SendMessage(
                "<col=128>You have unlocked the title '<col="
                    + Integer.toHexString(button.getTitle().getColor())
                    + ">"
                    + button.getTitle().getTitle()
                    + "</col>'!"));
      }
      return true;
    } else {
      ...
      stoner.send(
          new SendMessage(
              "<col=128>You have changed your title to '<col="
                  + Integer.toHexString(button.getTitle().getColor())
                  + ">"
                  + button.getTitle().getTitle()
                  + "</col>'."));
  ...
  public enum TitleButton {
    ...
    SKELETAL(
        215164,
        StonerTitle.create("Skeletal", 0xB0720E, false),
        AchievementList.KILL_250_SKELETAL_WYVERNS),
    MULTI_TASK(
        215180,
        StonerTitle.create("Multi-Task", 0xB0720E, false),
        AchievementList.COMPLETE_100_MERCENARY_TASKS),
    PET(215188, StonerTitle.create("Pet", 0xB0720E, false), AchievementList.OBTAIN_10_BOSS_PET),
    TZTOK(
        215196, StonerTitle.create("TzTok", 0xB0720E, false), AchievementList.OBTAIN_50_FIRECAPES),
    BIG_BEAR(
        215212, StonerTitle.create("Big Bear", 0xB0720E, false), AchievementList.KILL_100_CALLISTO),
    ...
    private final StonerTitle title;
    private final Object price;

    TitleButton(int button, StonerTitle title, Object price) {
      this.button = button;
      this.title = title;
      this.price = price;
    }
