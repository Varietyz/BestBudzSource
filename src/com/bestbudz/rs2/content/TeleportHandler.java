...
public class TeleportHandler {
	...
	public enum TeleportationData {
		...
		// VENENATIS(250064, 6250, "High combat", "40+ Wild", new Location(3283, 3853,
		// 0), false, false),
		...
		private TeleportationData(int button, int cost, String requirement, String other, int grade, Location position, boolean instanced, boolean specialCase) {
		this.button = button;
		this.cost = cost;
	...
	if (!stoner.isCreditUnlocked(CreditPurchase.FREE_TELEPORTS)) {
		...
		stoner.send(new SendMessage(teleportation.getCost() == 0 ? "You paid @red@nothing</col> and were teleported!" : "You have paid @red@" + Utility.format(teleportation.getCost()) + "</col> bestbucks and were teleported!"));
	} else {
		stoner.send(new SendMessage("@red@You have been teleported for free."));
	}

	int height = teleportation.isInstanced() ? stoner.getIndex() << 2 : teleportation.getPosition().getZ();

	stoner.getMage().teleport(teleportation.getPosition().getX(), teleportation.getPosition().getY(), height, TeleportTypes.SPELL_BOOK);
	...
	case 250053:// Barrelchest
		TaskQueue.queue(new Task(stoner, 4, false, Task.StackType.STACK, Task.BreakType.NEVER, TaskIdentifier.CURRENT_ACTION) {
			@Override
			public void execute() {
			new Mob(stoner, 6342, true, false, false, new Location(3815, 2844, stoner.getIndex() << 2));
			stop();
			}
	...
	case 242107:// Cultivation
		stoner.start(new OptionDialogue("Catweedy", p -> {
			p.teleport(new Location(2805, 3464, 0));
			p.send(new SendMessage("<col=482CB8>You have teleported to the Catweedy cultivation area."));
		}, "Ardougne", p -> {
			p.teleport(new Location(2662, 3375, 0));
			p.send(new SendMessage("<col=482CB8>You have teleported to the Ardougne cultivation area."));
		}, "Falador", p -> {
			p.teleport(new Location(3056, 3310, 0));
			p.send(new SendMessage("<col=482CB8>You have teleported to the Falador cultivation area."));
		}, "Phasmatys", p -> {
			p.teleport(new Location(3600, 3524, 0));
			p.send(new SendMessage("<col=482CB8>You have teleported to the Phasmatys cultivation area."));
		}));
		return false;

	case 242102:// exercisement
		stoner.start(new OptionDialogue("Gnome exercisement course", p -> {
			p.teleport(new Location(2480, 3437, 0));
			p.send(new SendMessage("<col=482CB8>You have teleported to the Gnome exercisement course."));
		}, "Barbarian exercisement course", p -> {
			p.teleport(new Location(2546, 3551, 0));
			p.send(new SendMessage("<col=482CB8>You have teleported to the Barbarian exercisement course."));
		}, "Wilderness exercisement course (50+ wild)", p -> {
			p.teleport(new Location(2998, 3915, 0));
			p.send(new SendMessage("<col=482CB8>You have teleported to the Wilderness exercisement course."));
		}, "Rooftop courses", p -> {
			stoner.start(new OptionDialogue("Seer's course", p2 -> {
				p.teleport(new Location(2729, 3488, 0));
				p.send(new SendMessage("<col=482CB8>You have teleported to the Seer's Village rooftop exercisement course."));
			}, "Ardougne course", p2 -> {
				p.teleport(new Location(2674, 3297, 0));
				p.send(new SendMessage("<col=482CB8>You have teleported to the Ardougne rooftop exercisement course."));
			}));
		}));
		return false;
	...
	case 250058:// Kraken
		TaskQueue.queue(new Task(5) {
			...
			for (int i = 0; i < DATA.length; i++) {
				Mob mob = new Mob(stoner, DATA[i][0], false, false, false, new Location(DATA[i][1], DATA[i][2], stoner.getZ()));
	...
	case 61000:// Training
		stoner.send(new SendString("Selected: @red@" + Utility.formatStonerName(teleportation.name().replaceAll("_", " ").toLowerCase()), 61031));
		...
		break;
	case 62000:// Professioning
		stoner.send(new SendString("Selected: @red@" + Utility.formatStonerName(teleportation.name().replaceAll("_", " ").toLowerCase()), 62031));
		...
		break;
	case 63000:// Stoner Vs Stoner
		stoner.send(new SendString("Selected: @red@" + Utility.formatStonerName(teleportation.name().replaceAll("_", " ").toLowerCase()), 63031));
		...
		break;
	case 64000:// Stoner Vs Monster
		stoner.send(new SendString("Selected: @red@" + Utility.formatStonerName(teleportation.name().replaceAll("_", " ").toLowerCase()), 64031));
		...
		break;
	case 65000:// Minigame
		stoner.send(new SendString("Selected: @red@" + Utility.formatStonerName(teleportation.name().replaceAll("_", " ").toLowerCase()), 65031));
		...
		break;
	case 61500:// Other
		stoner.send(new SendString("Selected: @red@" + Utility.formatStonerName(teleportation.name().replaceAll("_", " ").toLowerCase()), 61531));
