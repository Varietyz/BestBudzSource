...
public class PilesDialogue extends Dialogue {
		...
		for (int i = 0; i < ITEMS.length; i++) {
			if (stoner.getBox().hasItemId(new Item(ITEMS[i][0]))) {
				...
				if (!stoner.getBox().hasItemId(new Item(995, payment))) {
					DialogueManager.sendStatement(stoner, Utility.format(payment) + " bestbucks is required to do this; which you do not have!");
					break;
				}
				stoner.getBox().remove(new Item(ITEMS[i][0], amount));
				stoner.getBox().add(new Item(ITEMS[i][1], amount));
				DialogueManager.sendInformationBox(stoner, "Piles", "You have noted:", "@blu@" + amount + " </col>items", "You have paid:", "@blu@" + Utility.format(payment) + " </col>bestbucks");
				break;
			} else {
				DialogueManager.sendStatement(stoner, "You do not contain any items that are allowed to be noted!");
			}
		}
	...
	case 0:
		DialogueManager.sendNpcChat(stoner, 13, Emotion.HAPPY, "I can note any items obtained from the resource", "arena for 50 bestbucks an item.");
		next++;
		break;
	case 1:
