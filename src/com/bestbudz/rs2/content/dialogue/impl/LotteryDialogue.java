    ...
    switch (id) {
      ...
      case DialogueConstants.OPTIONS_4_2:
        DialogueManager.sendInformationBox(
            stoner,
            "Lottery Information",
            "</col>Current Pot: @blu@" + Utility.format(Lottery.getPot()),
            "</col>Pot Limit: @blu@" + Utility.format(Lottery.getLimit()),
            "</col>Stoners: @blu@" + Lottery.getStoners(),
            "</col>You are "
                + (Lottery.hasEntered(stoner) ? "@gre@entered</col>" : "@red@not entered</col>")
                + " in the lottery");
        break;
    ...
    switch (next) {
      case 0:
        DialogueManager.sendOption(
            stoner, "Lottery guide", "Current pot", "Enter lottery", "Nevermind");
        break;
    }
  }
}
