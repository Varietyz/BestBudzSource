    ...
    switch (button) {
      ...
      case DialogueConstants.OPTIONS_5_2:
        DialogueManager.sendOption(
            stoner,
            "Edit shop",
            "Edit shop motto (10 cannacredits)",
            "Edit shop color (10 cannacredits)");
        return true;
      case DialogueConstants.OPTIONS_5_1:
        stoner.getClient().queueOutgoingPacket(new SendInterface(53500));
        List<Stoner> available =
            Arrays.stream(World.getStoners())
                .filter(p -> p != null && p.isActive() && p.getStonerShop().hasAnyItems())
        ...
        for (int i = 53516; i < 53716; i++) {
          ...
          if (i - 53516 < available.size()) {
            ...
            stoner
                .getClient()
                .queueOutgoingPacket(new SendString(p.deterquarryIcon(p) + p.getUsername(), i));

            if (stoner.getShopMotto() != null) {
              stoner
                  .getClient()
                  .queueOutgoingPacket(new SendString(color + p.getShopMotto(), i + 200));
            } else {
              stoner
                  .getClient()
