  ...
  public boolean clickButton(int id) {
    switch (id) {
      case DialogueConstants.OPTIONS_3_1:
        if (stoner.getCredits() < 2) {
          DialogueManager.sendNpcChat(
              stoner, 490, Emotion.DEFAULT, "You do not have enough cannacredits to do this!");
          end();
          return false;
        }
        if (stoner.getMercenary().hasTask()) {
          DialogueManager.sendNpcChat(
              ...
              "Complete your current task or reset it to get a new.");
          return false;
        }
      ...
      case DialogueConstants.OPTIONS_3_3:
        if (!stoner.getMercenary().hasTask()) {
          DialogueManager.sendNpcChat(
              stoner, 490, Emotion.DEFAULT, "A mercenary task is required to reset it!");
          end();
          return false;
        }
        if (!stoner.getBox().hasItemAmount(995, 250_000)) {
          DialogueManager.sendNpcChat(
              stoner, 490, Emotion.DEFAULT, "You do not have enough BestBucks to do this!");
          end();
          return false;
        }
        ...
        DialogueManager.sendNpcChat(
            stoner, 490, Emotion.DEFAULT, "Your current task has been reset!");
        end();
        break;
    }
    return false;
    ...
    switch (next) {
      case 0:
        DialogueManager.sendNpcChat(
            ...
            490,
            Emotion.DEFAULT,
            "Hello my elite friend.",
            "I can give you a boss task.",
            "It will cost you 2 cannacredits.",
            "Are you interested?");
        next++;
        break;
