...
public class EmblemDialogue extends Dialogue {
    ...
    switch (id) {
        ...
        for (int i = 0; i < TIER_DATA.length; i++) {
          if (stoner.getBox().hasItemId(new Item(TIER_DATA[i][0]))) {
            ...
            DialogueManager.sendStatement(
                stoner, "You traded your emblems for " + Utility.format(bountyTotal) + " points.");
            bountyTotal = 0;
            stoner.getBox().remove(new Item(TIER_DATA[i][0]));
            break;
          } else {
            DialogueManager.sendNpcChat(
                stoner, 315, Emotion.ANGRY_1, "You do not have any tiers on you!");
          }
        }
        break;
      case DialogueConstants.OPTIONS_5_2:
        ...
        DialogueManager.sendInformationBox(
            stoner,
            "PvP Statistics:",
            "Points: @red@" + stoner.getBountyPoints(),
            "Kills: @red@" + stoner.getKills(),
            "Deaths: @red@" + stoner.getDeaths(),
            "KDR: @red@" + kdr);
        break;
  ...
  public void execute() {
    switch (next) {
      case 0:
        DialogueManager.sendNpcChat(
            stoner, 315, Emotion.CALM, "Hello stoner.", "How may I help you today?");
        next++;
        break;
      case 1:
        DialogueManager.sendOption(
            stoner,
            "Sell tiers",
            "Show me my PvP statistics",
            "I would like to trade",
            "Give me a wilderness skull",
            "Reset KDR");
        break;
    }
  }
