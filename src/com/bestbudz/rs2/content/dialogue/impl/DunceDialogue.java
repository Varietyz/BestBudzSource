...
public class DunceDialogue extends Dialogue {
	...
	case DialogueConstants.OPTIONS_4_1:
		if (StonerConstants.isStaff(stoner) || stoner.getRights() == 5 || stoner.getRights() == 6 || stoner.getRights() == 7 || stoner.getRights() == 8) {
			title("Member", 0xCC0000);
		} else {
			DialogueManager.sendNpcChat(stoner, 6749, Emotion.SAD, "You need to at least be a <img=5> <col=0x015E7D>Member</col>!");
		}
		break;

	case DialogueConstants.OPTIONS_4_2:
		if (StonerConstants.isStaff(stoner) || stoner.getRights() == 6 || stoner.getRights() == 7 || stoner.getRights() == 8) {
			title("Super", 0x3366CC);
		} else {
			DialogueManager.sendNpcChat(stoner, 6749, Emotion.SAD, "You need to at least be a <img=6> <col=0x015E7D>Super Member</col>!");
		}
		break;
	...
	case DialogueConstants.OPTIONS_4_3:
		if (StonerConstants.isStaff(stoner) || stoner.getRights() == 7 || stoner.getRights() == 8) {
			...
			DialogueManager.sendNpcChat(stoner, 6749, Emotion.SAD, "You need to at least be a <img=7> <col=0x158A08>Extreme Member</col>!");
		}
		break;
	...
	case DialogueConstants.OPTIONS_4_4:
		if (StonerConstants.isStaff(stoner) || stoner.getRights() == 8) {
			...
			DialogueManager.sendNpcChat(stoner, 6749, Emotion.SAD, "You need to at least be a <img=8> <col=0x7D088A>Elite Member</col>!");
		}
		break;
