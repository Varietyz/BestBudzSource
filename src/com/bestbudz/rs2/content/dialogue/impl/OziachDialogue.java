...
public class OziachDialogue extends Dialogue {
	...
	if (!stoner.getBox().hasItemAmount(995, 15_000_000)) {
		DialogueManager.sendNpcChat(stoner, 822, Emotion.DEFAULT, "You need 15,000,000 bestbucks to do this!", "You have " + stoner.getBox().getItemAmount(995) + ".");
		return;
	}
	if (!stoner.getBox().hasItemAmount(11286, 1)) {
		DialogueManager.sendNpcChat(stoner, 822, Emotion.DEFAULT, "You need a " + GameDefinitionLoader.getItemDef(11286).getName() + ".");
		return;
	}
	...
	case 0:
		DialogueManager.sendNpcChat(stoner, 822, Emotion.DEFAULT, "Hello there!", "How may I help you?");
		next++;
		break;
	case 1:
		...
		break;
	case 2:
		DialogueManager.sendNpcChat(stoner, 822, Emotion.DEFAULT, "Yes of course!", "It will cost 15m bestbucks.", "You also need a Dragonic visage.");
		next++;
		break;
	case 3:
		makeShield();
