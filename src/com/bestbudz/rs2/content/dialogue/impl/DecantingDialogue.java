...
public class DecantingDialogue extends Dialogue {
	...
	switch (next) {
	case 0:
		DialogueManager.sendNpcChat(stoner, 6524, Emotion.HAPPY, "Hello stoner.", "I can decant your potions for 250gp a potion.", "What would you like to do?");
		next++;
		break;
	case 1:
		...
		break;
	case 5:
		DialogueManager.sendNpcChat(stoner, 6524, Emotion.HAPPY, "Decanting is the method in which a stoner combines", "partially full potions of the same kind to produce", "one full potion and one partially empty potion.");
		next++;
		break;
	case 6:
		DialogueManager.sendNpcChat(stoner, 6524, Emotion.HAPPY, "For example, decanting a potion containing 3 doses and", "another containing 2 doses yields one full potion (4 doses)", "and one partially full potion (1 dose).");
		setNext(1);
		break;
	}
	}
