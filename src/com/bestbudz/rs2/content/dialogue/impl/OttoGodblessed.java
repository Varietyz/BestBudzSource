...
public class OttoGodblessed extends Dialogue {
	...
	case DialogueConstants.OPTIONS_3_1:
		if (stoner.getBox().hasItemId(ZAMORAKIAN_SPEAR)) {
			if (stoner.getBox().hasItemAmount(995, CREATION_COST)) {
				...
				DialogueManager.sendItem1(stoner, "Otto has given you a @dre@Zamorakian hasta</col>!", ZAMORAKIAN_HASTA);
				setNext(-1);
			} else {
				DialogueManager.sendNpcChat(stoner, 2914, Emotion.ANNOYED, "You need " + Utility.format(CREATION_COST) + " bestbucks to do this!");
				setNext(-1);
			}
		} else {
			DialogueManager.sendNpcChat(stoner, 2914, Emotion.ANNOYED, "You need a Zamorakian spear to do this!");
			setNext(-1);
		}
		break;
	...
	case DialogueConstants.OPTIONS_3_2:
		if (stoner.getBox().hasItemId(ZAMORAKIAN_HASTA)) {
			...
			DialogueManager.sendItem1(stoner, "Otto has given you a @dre@Zamorakian spear</col>!", ZAMORAKIAN_SPEAR);
			setNext(-1);

		} else {
			DialogueManager.sendNpcChat(stoner, 2914, Emotion.ANNOYED, "You need a Zamorakian hasta to do this!");
			setNext(-1);
		}
		break;
	...
	case 0:
		DialogueManager.sendNpcChat(stoner, 2914, Emotion.HAPPY_TALK, "Hello young warrior!", "You have quite the body on you I may say.", "I can offer you some services.");
		next++;
		break;

	case 1:
		DialogueManager.sendOption(stoner, "Make Zamorakian hasta", "Revert Zamorakian spear", "Nothing");
		break;
