...
public class StonerSounds {
	...
	if ((shield != null) && (sounds[shield.getId()] != null)) {
		stoner.getClient().queueOutgoingPacket(new SendSound(sounds[shield.getId()][com.bestbudz.core.util.Utility.randomNumber(sounds[shield.getId()].length)].id, 0, 0));
	}
	...
	if (sounds[id] != null)
		if (spec)
			stoner.getClient().queueOutgoingPacket(new SendSound(sounds[id][4]));
		else
			stoner.getClient().queueOutgoingPacket(new SendSound(sounds[id][stoner.getEquipment().getAssaultStyle().ordinal()]));
	}
}
