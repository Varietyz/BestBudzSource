...
import com.bestbudz.rs2.entity.Sound;
...
public class StonerSounds {
  ...
  public static final int[] RANDOM_METAL_ARMOR_SOUNDS =
      new int[] {405, 406, 785, 786, 790, 1052, 1066};
    ...
    for (int i = 0; i < 20145; i++) {
      if (GameDefinitionLoader.getItemDef(i) != null) {
        ...
        if (check.contains("abyssal whip"))
          ...
          if ((check.contains("mage shortbow")) || (check.contains("mage longbow"))) {
            sounds[i] =
                new Sound[] {
                  new Sound(370, 38),
                  new Sound(370, 38),
        ...
        else if ((check.contains("2h"))
            || (check.contains("godsword"))
            || (check.contains("saradomin sword"))
        ...
        else if ((check.contains("dart"))
            || (check.contains("knife"))
            || (check.contains("javelin"))
            || (check.contains("thrown axe"))
            || (check.contains("throwing axe")))
          sounds[i] =
              new Sound[] {
                new Sound(364, 38),
                new Sound(364, 38),
  ...
  public static void sendBlockOrHitSound(Stoner stoner, boolean hit) {
    Item shield = stoner.getEquipment().getItems()[5];
    ...
    if ((shield != null) && (sounds[shield.getId()] != null)) {
      stoner
          ...
          .queueOutgoingPacket(
              new SendSound(
                  sounds[shield.getId()][
                      com.bestbudz.core.util.Utility.randomNumber(sounds[shield.getId()].length)]
                      .id,
                  0,
                  0));
    }
    ...
    if (hit) {
      int id = stoner.getGender() == 0 ? 816 : 818;
      stoner.getClient().queueOutgoingPacket(new SendSound(id, 10, 0));
    } else {
      stoner.getClient().queueOutgoingPacket(new SendSound(75, 10, 0));
    }
  }

  public static void sendSoundForId(Stoner stoner, boolean spec, int id) {
    if (sounds[id] != null)
      if (spec) stoner.getClient().queueOutgoingPacket(new SendSound(sounds[id][4]));
      else
        stoner
            .getClient()
            .queueOutgoingPacket(
                new SendSound(sounds[id][stoner.getEquipment().getAssaultStyle().ordinal()]));
  }
}
