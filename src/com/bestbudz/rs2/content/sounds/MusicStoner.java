...
import com.bestbudz.rs2.entity.stoner.net.out.impl.SendSong;
import com.bestbudz.rs2.entity.stoner.net.out.impl.SendSound;

@SuppressWarnings("all")
public class MusicStoner {
	...
	if ((c.equals(ControllerManager.DEFAULT_CONTROLLER)) && (stoner.getCurrentSongId() == -3) && (Utility.randomNumber(250) == 0)) {
		stoner.getClient().queueOutgoingPacket(new SendSound(RANDOM_BIRD_SOUNDS[Utility.randomNumber(RANDOM_BIRD_SOUNDS.length)], 0, 0));
	}

	if (stoner.getAttributes().get("manual") != null) {
		return;
	}

	if (stoner.getCombat().inCombat()) {
		if (!stoner.isDead()) {
			stoner.getAttributes().set("combatsongdelay", Long.valueOf(System.currentTimeMillis() + 40000L));
			startSong(stoner, COMBAT_SONGS[Utility.randomNumber(COMBAT_SONGS.length)], -4);
			return;
		}

	} else if (stoner.getAttributes().get("combatsongdelay") != null) {
		if (System.currentTimeMillis() > ((Long) stoner.getAttributes().get("combatsongdelay")).longValue())
	...
	if (c.equals(ControllerManager.WILDERNESS_CONTROLLER)) {
		startSong(stoner, WILDERNESS[Utility.randomNumber(WILDERNESS.length)].music, -2);
		return;
	}

	Music song = getMusicId(stoner);

	if (song == null) {
		startSong(stoner, DEFAULT_SONGS[Utility.randomNumber(DEFAULT_SONGS.length)], -3);
		return;
	}

	startSong(stoner, song.music, song.music);
	}

	public static final boolean restart(Stoner p) {
	return (p.getAttributes().get("musicstarttime") != null) && (System.currentTimeMillis() - ((Long) p.getAttributes().get("musicstarttime")).longValue() > 240000L);
	}

	public static final void select(Stoner stoner, int button) {
	int id = RSInterface.getSongId(button);

	if (stoner.getCurrentSongId() != id) {
		stoner.getClient().queueOutgoingPacket(new SendSong(RSInterface.getSongId(button)));
		stoner.setCurrentSongId(id);
		stoner.getAttributes().set("musicStartTime", Long.valueOf(System.currentTimeMillis()));
	...
	public static void startSong(Stoner p, int songId, int setId) {
