...
public class Bank extends ItemContainer {
	...
	case 195085:
		...
		if (k.length > 1) {
			stoner.getClient().queueOutgoingPacket(new SendMessage("You have deposited some of the items in your box."));
		}
		return true;
	case 195086:
			...
			if (e[i] != null) {
				...
				if (deposited != 0) {
					if (e[i].getAmount() == deposited)
						stoner.getEquipment().getItems()[i] = null;
	...
	if (withdrawType == WithdrawTypes.NOTE) {
		if (!Item.getDefinition(item).canNote())
			stoner.getClient().queueOutgoingPacket(new SendMessage("This item cannot be withdrawn as a note."));
		else {
	...
	stoner.send(new OutgoingPacket() {
		@Override
