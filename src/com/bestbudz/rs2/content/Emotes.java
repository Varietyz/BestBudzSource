...
public class Emotes {
	...
	for (Emote i : Emote.values()) {
		if (i.buttonID == id) {
			...
			if (i.gfxID != 1)
				stoner.getUpdateFlags().sendGraphic(Graphic.lowGraphic(i.gfxID, 0));
			return true;
		}
	}
	...
	for (int i = 0; i < 20145; i++) {
		...
		if ((def != null) && (def.getName() != null)) {
			...
			else if ((name.contains("Lumber. cape")) || (name.contains("Lumbering cape")) || (name.contains("Lumbering Master Cape")))
			...
			else if ((name.contains("THC-hempistry cape")) || (name.contains("THC-hempistry Master Cape")))
			...
			else if ((name.contains("Accomplisher cape")) || (name.contains("Accomplisher Master Cape")))
			...
			else if ((name.contains("Cultivation cape")) || (name.contains("Cultivation Master Cape")))
	...
	if (cape == null) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("You need to be wearing a professioncape to do this."));
		return;
	}
