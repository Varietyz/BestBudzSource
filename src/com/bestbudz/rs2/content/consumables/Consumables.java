    ...
    switch (type) {
      case FOOD:
        ...
        if (heal > stoner.getMaxGrades()[3]) {
          if (id != 15272) heal = stoner.getMaxGrades()[3];
          else {
      ...
      case POTION:
        ...
        if ((professionData != null) && (professionData.length > 0)) {
          for (int i = 0; i < professionData.length; i++) {
            ...
            long grade = stoner.getProfession().getGrades()[professionId];
            long gradeForExp = stoner.getMaxGrades()[professionId];

            if (modifier < 0.0D) {
              long affectedGrade = grade + (int) (gradeForExp * modifier) + add;
              ...
              long maxLvl =
                  potions.getPotionType() == PotionDefinition.PotionTypes.RESTORE
                      ? gradeForExp
                      : gradeForExp + (int) (gradeForExp * modifier) + add;
              long affectedGrade = grade + (int) (gradeForExp * modifier) + add;
