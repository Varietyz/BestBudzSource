...
public class DwarfCannon extends RSObject {
  ...
  public Mob[] getMobsInPath() {
    ...
    for (Iterator<Mob> mobs = cannonOwner.getClient().getNpcs().iterator(); mobs.hasNext(); ) {
      Mob mob = mobs.next();
      int dir =
          GameConstants.getDirection(
              Integer.signum(cannonLocation.getX() - mob.getX()),
              Integer.signum(cannonLocation.getY() - mob.getY()));
      if (DIRECIONS[dir] == this.dir) {
        boolean canAssault =
            !cannonOwner.getCombat().inCombat()
                || cannonOwner.inMultiArea()
                || (cannonOwner.getCombat().inCombat()
                    && cannonOwner.getCombat().getLastAssaultedBy().equals(mob));
        boolean clearPath =
            StraightPathFinder.isProjectilePathClear(
                getX(), getY(), cannonLocation.getZ(), mob.getX(), mob.getY());
