...
public class Trade {
	...
	if (stage == TradeStages.STAGE_1) {
		if (!stoner.getBox().hasSpaceFor(tradingWith.getTradedItems())) {
			stoner.getClient().queueOutgoingPacket(new SendMessage("You do not have enough box space to make this " + getAction() + "."));
			return;
		}
		stoner.getClient().queueOutgoingPacket(new SendString("Waiting for stoner...", 3431));
		tradingWith.getStoner().getClient().queueOutgoingPacket(new SendString("Fellow stoner has accepted.", 3431));
		...
		tradingWith.getStoner().getClient().queueOutgoingPacket(new SendString("Fellow stoner has accepted.", 3535));
	}
	...
	stoner.getClient().queueOutgoingPacket(new SendString(getStatus() + " with: " + NameUtil.uppercaseFirstLetter(tradingWith.getStoner().getUsername()), 3417));
	tradingWith.getStoner().getClient().queueOutgoingPacket(new SendString(getStatus() + " with: " + NameUtil.uppercaseFirstLetter(stoner.getUsername()), 3417));
	...
	stoner.getClient().queueOutgoingPacket(new SendString("Are you sure you want to make this " + getAction() + "?", 3535));
	tradingWith.getStoner().getClient().queueOutgoingPacket(new SendString("Are you sure you want to make this " + getAction() + "?", 3535));
	...
	for (int item : trade.keySet()) {
		...
		strings[0][index] = new String[] { Utility.formatStonerName(stoner.getUsername()), "" + tradedItem.getAmount(), Utility.formatStonerName(tradedItem.getDefinition().getName()), Utility.formatStonerName(tradingWith.stoner.getUsername()) };
		strings[1][index] = new String[] { Utility.formatStonerName(tradingWith.stoner.getUsername()), "" + tradedItem.getAmount(), Utility.formatStonerName(tradedItem.getDefinition().getName()), Utility.formatStonerName(stoner.getUsername()) };
		index++;
	}

	StonerLogger.TRADE_LOGGER.multiLog(stoner.getUsername(), "%s has given %s %s to %s", strings[0]);
	StonerLogger.TRADE_LOGGER.multiLog(tradingWith.stoner.getUsername(), "%s has received %s %s from %s", strings[1]);
	strings = new String[2][recieved.size()][4];
	index = 0;

	for (int item : recieved.keySet()) {
		Item tradedItem = new Item(item, recieved.get(item));
		strings[0][index] = new String[] { Utility.formatStonerName(tradingWith.stoner.getUsername()), "" + tradedItem.getAmount(), Utility.formatStonerName(tradedItem.getDefinition().getName()), Utility.formatStonerName(stoner.getUsername()) };
		strings[1][index] = new String[] { Utility.formatStonerName(stoner.getUsername()), "" + tradedItem.getAmount(), Utility.formatStonerName(tradedItem.getDefinition().getName()), Utility.formatStonerName(tradingWith.stoner.getUsername()) };
		index++;
	}

	StonerLogger.TRADE_LOGGER.multiLog(tradingWith.stoner.getUsername(), "%s has given %s %s to %s", strings[0]);
	StonerLogger.TRADE_LOGGER.multiLog(stoner.getUsername(), "%s has received %s %s from %s", strings[1]);

	if (success) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("The Deal has been accepted."));
		tradingWith.getStoner().getClient().queueOutgoingPacket(new SendMessage("The trade has been accepted."));
	} else {
		stoner.getClient().queueOutgoingPacket(new SendMessage("You decline the " + getAction() + "."));
		tradingWith.getStoner().getClient().queueOutgoingPacket(new SendMessage("Fellow stoner declined the " + getAction() + "."));
	}
	...
	stoner.getClient().queueOutgoingPacket(new SendMessage("Sending " + getRequestString() + " offer.."));
	lastRequest = requested.getUsername();

	if (requested.getTrade().requested(stoner))
		begin(requested.getTrade());
	else if (!requested.getPrivateMessaging().ignored(stoner.getUsername()))
		requested.getClient().queueOutgoingPacket(new SendMessage(NameUtil.uppercaseFirstLetter(stoner.getUsername()) + ":" + getRequestString() + "req:"));
	}
