...
public class Shop extends ItemContainer {
	...
	if (gold.getAmount() > 0) {
		if (stoner.isPouchPayment()) {
			if (stoner.getMoneyPouch() < gold.getAmount()) {
				stoner.send(new SendMessage("[" + ShopConstants.COLOUR + "*</col>] Insufficient funds on your Debit card!"));
				return;
			}
		} else {
	...
	for (int i = 0; i < ShopConstants.NO_SELL.length; i++) {
		if (id == ShopConstants.NO_SELL[i]) {
			stoner.getClient().queueOutgoingPacket(new SendMessage("[" + ShopConstants.COLOUR + "*</col>] You may not sell this item! Try selling it to a stoner."));
			return false;
		}
	}
	...
	if (amount > 50000) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("[" + ShopConstants.COLOUR + "*</col>] You can only sell 50,000 at a time to these shops."));
		amount = 50000;
	}
	...
	if (!hasSpaceFor(item)) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("[" + ShopConstants.COLOUR + "*</col>] The shop does not have enough space to buy this item."));
		return false;
	}
	...
	if (!stoner.getBox().hasSpaceOnRemove(item, gold)) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("[" + ShopConstants.COLOUR + "*</col>] You do not have enough box space to sell this item."));
		return false;
	}
