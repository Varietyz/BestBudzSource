	...
	for (int index = 0; index < Professions.PROFESSION_COUNT; index++) {
		...
		if (grade >= 420) {
			if (index == Professions.HUNTER) {
				...
				add(new Item(trimmed ? Professioncape.CONSTRUCTION.getTrimmedCape() : Professioncape.CONSTRUCTION.getCape()), false);
			} else {
				add(new Item(trimmed ? Professioncape.values()[index].getTrimmedCape() : Professioncape.values()[index].getCape()), false);
			}
		}
	}
	}
