...
public class ExerciseShop extends Shop {
  ...
  public ExerciseShop() {
    super(
        91,
        new Item[] {new Item(2997, 1), new Item(9472, 1), new Item(3257, 1)},
        false,
        "Exercisement Shop");

    for (Item i : getItems()) if (i != null) i.getDefinition().setUntradable();
  }
  ...
  public void buy(Stoner stoner, int slot, int id, int amount) {
    ...
    if (stoner.getBox().getItemAmount(2996) < amount * getPrice(id)) {
      stoner
          .getClient()
          .queueOutgoingPacket(
              new SendMessage("You do not have enough exercisement tokens to buy that."));
      return;
    }
