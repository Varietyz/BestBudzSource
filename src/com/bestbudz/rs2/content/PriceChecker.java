...
public class PriceChecker extends ItemContainer {
	...
	private int[][] ITEM_STRINGS = { { 0, 48550 }, { 1, 48551 }, { 2, 48552 }, { 3, 48553 }, { 4, 48554 }, { 5, 48555 }, { 6, 48556 }, { 7, 48557 }, { 8, 48558 }, { 9, 48559 }, { 10, 48560 }, { 11, 48561 }, { 12, 48562 }, { 13, 48563 }, { 14, 48564 }, { 15, 48565 }, { 16, 48566 }, { 17, 48567 }, { 18, 48568 }, { 19, 48569 }, { 20, 48570 }, { 21, 48571 }, { 22, 48572 }, { 23, 48573 }, { 24, 48574 }, { 25, 48575 }, { 26, 48576 }, { 27, 48577 }, };

	public void open() {
	if (WeaponGame.gameStoners.contains(stoner) || stoner.getCombat().inCombat() || stoner.isDead() || stoner.getMage().isTeleporting()) {
		return;
	}
	update();
	for (int i = 0; i < 27; i++) {
	...
	// Add the item prices
	stoner.send(new SendString("" + NumberFormat.getInstance().format(GameDefinitionLoader.getStoreSellToValue(id)) + "x" + NumberFormat.getInstance().format(amount), ITEM_STRINGS[slot][1]));

	// Remove the item from box
	...
	for (int i = 0; i < getSize(); i++) {
		if (getItems()[i] != null) {
			stoner.send(new SendString("" + NumberFormat.getInstance().format(GameDefinitionLoader.getStoreSellToValue(getItems()[i].getId())), ITEM_STRINGS[i][1]));
		} else {
			stoner.send(new SendString("", ITEM_STRINGS[i][1]));
		}
	}
