...
public class SagittariusProfession {
  ...
  public static final boolean requiresArrow(Stoner stoner, int id) {
    ...
    return (weapon != null)
        ...
        && (weapon.getSagittariusDefinition().getType()
  ...
  public boolean canUseSagittarius() {
    ...
    if (weapon.getId() == 12926) {
      if (stoner.getToxicBlowpipe().getBlowpipeAmmo() == null
          || stoner.getToxicBlowpipe().getBlowpipeCharge() == 0) {
        stoner.send(
            new SendMessage(
    ...
    if ((def.getType() == SagittariusWeaponDefinition.SagittariusTypes.SHOT)
        && (requiresArrow(stoner, weapon.getId()))
        && (arrows != null)
        && (arrows.length != 0)) {
      if (ammo == null) {
        stoner
  ...
  public void doOnyxEffect(long damage) {
    if (damage > 0) {
      long max = stoner.getMaxGrades()[3];
      long newLvl = stoner.getProfession().getGrades()[3] + (int) (damage * 0.25D);
      long set = newLvl > max ? max : newLvl;
      stoner.getProfession().getGrades()[3] = (set);
    ...
    switch (def.getType()) {
      ...
      case THROWN:
        ...
        if (weapon.getAmount() == 1) {
          stoner
              .getClient()
              .queueOutgoingPacket(new SendMessage("You threw the last of your ammo!"));
        }
        weapon.remove(1);
        if (weapon.getAmount() == 0) stoner.getEquipment().unequip(3);
        else {
