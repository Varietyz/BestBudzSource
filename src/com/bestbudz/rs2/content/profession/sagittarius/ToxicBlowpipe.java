...
public class ToxicBlowpipe {
	...
	if (stoner.getToxicBlowpipe().blowpipeAmmo != null && stoner.getToxicBlowpipe().blowpipeAmmo.getAmount() == FULL) {
		return true;
	}
	...
	if (usedWith.getId() == 12924 || usedWith.getId() == 12926 || itemUsed.getId() == 12924 || itemUsed.getId() == 12926) {
		if (stoner.getToxicBlowpipe().blowpipeAmmo != null) {
			if (dart.getAmount() + stoner.getToxicBlowpipe().blowpipeAmmo.getAmount() > FULL) {
				dart.setAmount((dart.getAmount() + stoner.getToxicBlowpipe().blowpipeAmmo.getAmount()) - FULL);
			}
	...
	String ammo = "None";
	if (stoner.getToxicBlowpipe().blowpipeAmmo != null) {
		ammo = stoner.getToxicBlowpipe().blowpipeAmmo.getDefinition().getName() + " x " + stoner.getToxicBlowpipe().blowpipeAmmo.getAmount();
	}
	String scales = FORMATTER.format((stoner.getToxicBlowpipe().blowpipeCharge / 3.0) * 100.0 / (double) FULL) + "%";
	stoner.send(new SendMessage("Darts: <col=007F00>" + ammo + "</col>. Scales: <col=007F00>" + scales));
	}
	...
	if (stoner.getToxicBlowpipe().blowpipeCharge > 0) {
		stoner.getBox().addOrCreateGroundItem(12934, stoner.getToxicBlowpipe().blowpipeCharge / 3, true);
	}
	if (stoner.getToxicBlowpipe().blowpipeAmmo != null) {
		stoner.getBox().addOrCreateGroundItem(stoner.getToxicBlowpipe().blowpipeAmmo.getId(), stoner.getToxicBlowpipe().blowpipeAmmo.getAmount(), true);
	}
	...
	if (cape != null && (cape.getId() == 10499 || cape.getId() == 10498)) {
		if (Math.random() > 1 - 1 / 4.0) {
			if (stoner.getCombat().getAssaulting().getLocation() != null && Math.random() > 1 - 1 / 3.0) {
				stoner.getGroundItems().drop(blowpipe.blowpipeAmmo.getSingle(), stoner.getCombat().getAssaulting().getLocation());
			}
