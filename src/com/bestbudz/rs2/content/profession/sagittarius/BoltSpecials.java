...
public class BoltSpecials {
	...
	if ((arrow == null) || (weapon == null) || (stoner.getCombat().getCombatType() != CombatTypes.SAGITTARIUS) || (!SagittariusProfession.requiresArrow(stoner, weapon.getId()))) {
		return;
	}

	int decrease = (assaulting instanceof Stoner && assaulting.getStoner() != null && assaulting.getStoner().getEquipment().contains(9944) ? 5 : 0);
	...
	case 9242:
		...
		int self_inflict_damage = (int) (stoner.getProfession().getGrades()[Professions.LIFE] * 0.1);

		int max_hit = (int) (assaulting.isNpc() ? assaulting.getGrades()[Professions.LIFE] * 0.2 : assaulting.getStoner().getProfession().getGrades()[Professions.LIFE] * 0.2);
		if (max_hit > 200) {
			max_hit = 200;
		}
		...
		stoner.getClient().queueOutgoingPacket(new SendMessage("You drain 10% of your life and 20% of your opponent's life."));
