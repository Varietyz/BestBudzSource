...
public class MercenaryMonsters {
	...
	if ((req != 0) && (stoner.getMaxGrades()[18] < req)) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("You need a Mercenary grade of " + req + " to assault this mob."));
		return false;
	}
	...
	for (int i = 0; i < 18000; i++) {
		...
		if (lvl > 0)
			mercenaryRequired.put(Integer.valueOf(i), Byte.valueOf((byte) lvl));
	}
	}
	...
	return mercenaryRequired.containsKey(Integer.valueOf(id)) ? mercenaryRequired.get(Integer.valueOf(id)).byteValue() : 1;
	}
}
