...
public class BarrowsSpecials {
	...
	else if ((stoner.getCombat().getCombatType() == CombatTypes.MAGE) && (ItemCheck.wearingFullBarrows(stoner, "Ahrim")))
	...
	public static void doAhrimEffect(Stoner stoner, Entity assault, int damage) {
	...
	if ((damage > 0) && (Utility.randomNumber(4) == 0)) {
		int newLvl = assault.getGrades()[2] - 5;

		assault.getGrades()[2] = ((byte) (newLvl > 0 ? newLvl : 0));
		...
		stoner.getClient().queueOutgoingPacket(new SendMessage("You drain 5 grades from your opponent's Vigour grade."));
	}
	}

	public static void doGuthanEffect(Stoner stoner, Entity assault, Hit hit) {
	int newLvl = stoner.getGrades()[3] + hit.getDamage();
	int maxLvl = stoner.getMaxGrades()[3];

	stoner.getGrades()[3] = ((short) (newLvl > maxLvl ? maxLvl : newLvl));
	...
	int newLvl = assault.getGrades()[6] - 5;

	assault.getGrades()[6] = ((byte) (newLvl > 0 ? newLvl : 0));
	...
	stoner.getClient().queueOutgoingPacket(new SendMessage("You drain 5 grades from your opponent's Mage grade."));
	}
	...
	if (!assault.isNpc()) {
		...
		stoner.getClient().queueOutgoingPacket(new SendMessage("You have drained 20% of your opponent's energy."));
	}
	}
}
