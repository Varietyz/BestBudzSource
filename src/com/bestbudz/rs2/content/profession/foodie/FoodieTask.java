...
public class FoodieTask extends Task {
  ...
  public static boolean handleFoodieByAmount(Stoner stoner, int buttonId) {
    ...
    if (amount != 100)
      attemptFoodie(
          stoner,
          ((Integer) stoner.getAttributes().get("foodieitem")).intValue(),
          ((Integer) stoner.getAttributes().get("foodieobject")).intValue(),
          amount);
    else {
      stoner
          .getClient()
          .queueOutgoingPacket(
              new SendEnterXInterface(
                  1743, ((Integer) stoner.getAttributes().get("foodieitem")).intValue()));
    }
    return true;
  }
  ...
  private void burnFood() {
    ...
    stoner
        ...
        .queueOutgoingPacket(
            new SendMessage(
                "The " + Item.getDefinition(used).getName() + " has something shiny in it."));
    stoner
        .getClient()
        .queueOutgoingPacket(new SendMessage("You have messed the fish up and got a BestBucks."));
  }
  ...
  public void execute() {
    ...
    if (successfulAttempt()) cookFood();
    else {
      burnFood();
    }
    amountToCook -= 1;
    if (amountToCook == 0) stop();
  }
  ...
  private boolean successfulAttempt() {
    ...
    return Professions.isSuccess(
        stoner.getMaxGrades()[7] + getFoodieGradeBoost(),
        foodieData.getGradeRequired() / 2 == 0 ? 1 : foodieData.getGradeRequired() / 2);
  }
}
