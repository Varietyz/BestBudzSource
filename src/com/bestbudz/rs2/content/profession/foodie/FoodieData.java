...
public enum FoodieData {
	...
	private static Map<Integer, FoodieData> food = new HashMap<Integer, FoodieData>();

	public static FoodieData forId(int id) {
	return food.get(Integer.valueOf(id));
	}

	private FoodieData(int food, int grade, int noBurn, int replacement, int burnt, double exp) {
	foodId = food;
	gradeRequired = grade;
	noBurnGrade = noBurn;
	experience = exp;
