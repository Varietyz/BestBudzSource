...
public class FamiliarMob extends Mob {
  ...
  public FamiliarMob(Familiar familiar, Stoner owner, Location l) {
    ...
    getCombat()
        .getMage()
        .setAssault(new Assault(3, 5), new Animation(familiar.assaultAnimation), null, null, null);
  ...
  public void afterCombatProcess(Entity assault) {
    if (assault.isDead()) getCombat().reset();
    else {
  ...
  public void doAliveMobProcessing() {
    if ((inWilderness()) && (getId() != familiar.wildMob)) transform(familiar.wildMob);
    else if ((!inWilderness()) && (getId() != familiar.mob)) {
      transform(familiar.mob);
    }

    if (getOwner().getSummoning().isAssault()) {
      if ((getOwner().getCombat().getAssaulting() != null)
          && (inMultiArea())
          && (getOwner().getCombat().getAssaulting().inMultiArea())) {
