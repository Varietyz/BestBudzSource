		...
		if ((index >= 0) && (index < Pouch.values().length)) {
			...
			if (pouch != null) {
				if (!p.getBox().hasSpaceFor(new Item(pouch.pouchId, 1))) {
					p.getClient().queueOutgoingPacket(new SendMessage("You must free some box space to do this."));
					return true;
				}
				...
				if (!BestbudzConstants.DEV_MODE) {
					if (p.getMaxGrades()[21] < pouch.gradeRequired) {
						p.getClient().queueOutgoingPacket(new SendMessage("You need a Summoning grade of " + pouch.gradeRequired + " to make this."));
						return true;
					}
					...
					if (!p.getBox().hasItemAmount(new Item(18016, shards))) {
						p.getClient().queueOutgoingPacket(new SendMessage("You do not have enough shards to create this."));
						return true;
					}

					if (!p.getBox().hasItemAmount(new Item(ingredient, 1))) {
						String name = Item.getDefinition(ingredient).getName();
						p.getClient().queueOutgoingPacket(new SendMessage("You need " + Utility.getAOrAn(name) + " " + name + " to create this."));
						return true;
					}
		...
		if ((index >= 0) && (index < Pouch.values().length)) {
			...
			if (scroll != null) {
				...
				if (p.getMaxGrades()[21] < scroll.gradeRequired) {
					p.getClient().queueOutgoingPacket(new SendMessage("You need a Summoning grade of " + scroll.gradeRequired + " to make this."));
					return true;
				}

				if (!p.getBox().hasItemAmount(new Item(pouch))) {
					String name = pouch.getDefinition().getName();
					p.getClient().queueOutgoingPacket(new SendMessage("You need " + Utility.getAOrAn(name) + " " + name + " to make this."));
					return true;
				}
