...
public class SpiritWolf implements FamiliarSpecial {
	...
	if (a != null) {
		...
		if (!a.isNpc()) {
			stoner.getClient().queueOutgoingPacket(new SendMessage("This special does not effect stoners!"));
		} else {
			Mob m = World.getNpcs()[a.getIndex()];

			if (m != null) {
				if (m.getDefinition().getGrade() > 100) {
					stoner.getClient().queueOutgoingPacket(new SendMessage("The mob was too strong and resisted the special move!"));
					return true;
				}
