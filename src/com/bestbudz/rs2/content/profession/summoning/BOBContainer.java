...
public class BOBContainer extends ItemContainer {
	...
	if (GameDefinitionLoader.getHighAlchemyValue(id) > 50000) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("You cannot store an item this valuable in your familiar's box."));
		return;
	}
	int value = 0;
	...
	if (value + GameDefinitionLoader.getHighAlchemyValue(id) > 175000) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("Your familiars maximum carried value limit has been reached!"));
		return;
	}

	if ((!Item.getDefinition(id).isTradable()) || (id == 7936)) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("You cannot store this item in your familiar's box."));
		return;
	}
