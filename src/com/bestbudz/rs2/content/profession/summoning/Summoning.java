...
public class Summoning {
      ...
      case 70112:
        if (familiar != null) {
          if (p.getBox().hasItemId(familiar.getData().pouch)) {
            ...
            p.getClient()
                .queueOutgoingPacket(
                    new SendMessage("You need another pouch to renew this familiar."));
          }
        } else p.getClient().queueOutgoingPacket(new SendMessage("You do not have a familiar."));
      ...
      case 70118:
        ...
        p.start(
            new ConfirmDialogue(
                p,
                new String[] {
                  "Are you sure you want to dismiss this familiar?",
                  "Any BoB items will be dropped."
                }) {
      ...
      case 70115:
        if (familiar != null) {
          if (Utility.getManhattanDistance(p.getLocation(), familiar.getLocation()) > 3) {
            ...
            Location l =
                GameConstants.getClearAdjacentLocation(p.getLocation(), familiar.getSize());
      ...
      case 70109:
        if (familiar != null) {
          if (container != null) container.open();
            ...
            p.getClient()
                .queueOutgoingPacket(new SendMessage("Your current familiar cannot store items."));
        } else {
          p.getClient().queueOutgoingPacket(new SendMessage("You do not have a familiar."));
        }
        return true;
      ...
      if (spec != null) {
        ...
        if (scroll == -1) {
          p.getClient()
              .queueOutgoingPacket(
                  new SendMessage(
                      "This familiar's scroll could not be found, please report this error."));
          return;
        }

        if ((spec.getSpecialType() == FamiliarSpecial.SpecialType.COMBAT)
            && (familiar.getCombat().getAssaulting() == null)) {
          p.getClient()
              .queueOutgoingPacket(
                  new SendMessage("You must be assaulting something to use this special move!"));
          return;
        }
        ...
        if (special < spec.getAmount()) {
          p.getClient()
              .queueOutgoingPacket(
                  new SendMessage(
                      "You do not have enough special points to perform that assault."));
          return;
        }
  ...
  public boolean interact(Mob mob, int option) {
    ...
    if ((option == 2)
        && familiar.getData() != null
        && (familiar.getData().bobSlots > 0)
        && (container != null)) {
      container.open();
      return true;
    }
  ...
  public void renew() {
    if (familiar != null) {
      ...
      removalTask =
          new Task(familiar, familiar.getData().time / 20) {
            @Override
            public void execute() {
              ...
              if (time == 1) {
                p.getClient()
                    .queueOutgoingPacket(
                        new SendMessage(
                            "Your familiar will dissapear in "
                                + (int) (familiar.getData().time / 20 * 0.6D)
                                + " seconds."));
              }
    ...
    if (f != null) {
      ...
      if (p.getMaxGrades()[21] < f.gradeRequired) {
        p.getClient()
            .queueOutgoingPacket(
                new SendMessage(
                    "You need a Summoning grade of " + f.gradeRequired + " to summon this mob."));
        return true;
      }
      ...
      Location l =
          GameConstants.getClearAdjacentLocation(
              p.getLocation(), GameDefinitionLoader.getNpcDefinition(f.mob).getSize());
      ...
      if (f.bobSlots > 0) container = new BOBContainer(p, f.bobSlots);
      else {
        container = null;
      }
