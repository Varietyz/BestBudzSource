...
public class Handiness {
  ...
  private static final int[][] LEATHER_ARMOR_IDS = {
    {33187, 1, 1129},
    {33186, 5, 1129},
    {33185, 10, 1129},
    {33190, 1, 1059},
          ...
          switch (((HandinessType) stoner.getAttributes().get("handinessType"))) {
            case WHEEL_SPINNING:
              if (stoner.getAttributes().get("spinnable") != null) {
                TaskQueue.queue(
                    new WheelSpinning(
                        stoner, (short) 28, (Spinnable) stoner.getAttributes().get("spinnable")));
        ...
        if (stoner.getAttributes().get("handinessType") != null) {
          switch (((HandinessType) stoner.getAttributes().get("handinessType"))) {
            case WHEEL_SPINNING:
              TaskQueue.queue(
                  new WheelSpinning(
                      stoner, (short) 5, (Spinnable) stoner.getAttributes().get("spinnable")));
              stoner.getClient().queueOutgoingPacket(new SendRemoveInterfaces());
              stoner.getAttributes().remove("spinnable");
              break;
            default:
        ...
        if (stoner.getAttributes().get("handinessType") != null) {
          switch (((HandinessType) stoner.getAttributes().get("handinessType"))) {
            case WHEEL_SPINNING:
              TaskQueue.queue(
                  new WheelSpinning(
                      stoner, (short) 1, (Spinnable) stoner.getAttributes().get("spinnable")));
              stoner.getClient().queueOutgoingPacket(new SendRemoveInterfaces());
              stoner.getAttributes().remove("spinnable");
              stoner.getAttributes().remove("handinessType");
              return true;
