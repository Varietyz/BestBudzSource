...
public class ArmourCreation extends ProductionTask {
	...
	public ArmourCreation(Stoner entity, short creationAmount, Craftable craft) {
	...
	if (craftable.getItemId() == 1741) {
		if (stoner.getBox().hasItemId(1741)) {
	...
	return "You need a " + com.bestbudz.rs2.content.profession.Professions.PROFESSION_NAMES[getProfession()] + " grade of " + getRequiredGrade() + " to create " + GameDefinitionLoader.getItemDef(getRewards()[0].getId()).getName() + ".";
	}
	...
	return craftable.getRequiredGrade();
	}
