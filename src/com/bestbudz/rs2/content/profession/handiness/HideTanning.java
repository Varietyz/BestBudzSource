...
public class HideTanning {
	...
	for (int i = 0; i < TANNING_HIDE.length; i++) {
		stoner.getClient().queueOutgoingPacket(new SendItemOnInterface(14769 + i, 250, TANNING_HIDE[i][1]));
		stoner.getClient().queueOutgoingPacket(new SendString(Item.getDefinition(TANNING_HIDE[i][1]).getName(), 14777 + i));
		stoner.getClient().queueOutgoingPacket(new SendString(TANNING_HIDE[i][2] + " BestBucks", 14785 + i));
	}
	...
	stoner.getClient().queueOutgoingPacket(new SendMessage("You tan " + amount + " hide(s) for " + total + " BestBucks."));
	}
}
