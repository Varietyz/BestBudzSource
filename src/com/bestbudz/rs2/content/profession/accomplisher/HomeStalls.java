...
public class HomeStalls extends Task {
  ...
  public static boolean meetsRequirements(Stoner stoner, stallData stall) {
    ...
    if (stoner.getBox().getFreeSlots() == 0) {
      stoner
          .getClient()
          .queueOutgoingPacket(
              new SendMessage("You don't have enough box spaces left to hold this."));
      return false;
    }
    return true;
  }
  ...
  public static void successfull(Stoner stoner, stallData stall) {
    ...
    stoner
        .getClient()
        .queueOutgoingPacket(
            new SendMessage(stall.name + " stall keeps generating new free junk."));
  }
  ...
  private enum stallData {
    ...
    stallData(String name, int objectId, int itemId, int itemAmount) {
      this.name = name;
    ...
    public static stallData getObjectById(int id) {
      for (stallData data : stallData.values()) if (data.objectId == id) return data;
      return null;
    }
  }
}
