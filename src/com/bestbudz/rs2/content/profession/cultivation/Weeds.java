...
public class Weeds {
	...
	for (int i = 0; i < weedSeeds.length; i++) {
		if (weedStages[i] > 0 && weedStages[i] <= 3 && (Cultivation.getMinutesCounter(stoner) - weedTimer[i]) >= 5) {
			weedStages[i] -= 2;
	...
	int finalAnimation;
	int finalDelay;
	if (weedFieldsData == null || (itemId != CultivationConstants.RAKE && itemId != CultivationConstants.SPADE)) {
		return false;
	}
	...
	if (weedData.getGradeRequired() > stoner.getGrades()[Professions.CULTIVATION]) {
		DialogueManager.sendStatement(stoner, "You need a cultivation grade of " + weedData.getGradeRequired() + " to plant this seed.");
		return true;
	}
	...
	stoner.getUpdateFlags().sendAnimation(new Animation((CultivationConstants.PICKING_VEGETABLE_ANIM)));
	Controller controller = stoner.getController();
	stoner.setController(ControllerManager.FORCE_MOVEMENT_CONTROLLER);
	TaskQueue.queue(new Task(stoner, 3, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.CULTIVATION) {
		@Override
		...
		stoner.getUpdateFlags().sendAnimation(new Animation((CultivationConstants.PICKING_WEED_ANIM)));
	...
	if (weedStages[weedFieldsData.getWeedIndex()] != 3 || weedState[weedFieldsData.getWeedIndex()] == 4) {
	...
	TaskQueue.queue(new Task(stoner, 7, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.CULTIVATION) {
		@Override
		public void execute() {
		diseaseChance[weedFieldsData.getWeedIndex()] *= itemId == 6032 ? COMPOST_CHANCE : SUPERCOMPOST_CHANCE;
	...
	final WeedData weedData = WeedData.forId(weedSeeds[weedFieldsData.getWeedIndex()]);
	System.out.println(weedStages[weedFieldsData.getWeedIndex()] + " 0x" + Integer.toHexString(weedStages[weedFieldsData.getWeedIndex()]));
	if (weedState[weedFieldsData.getWeedIndex()] == 1) {
		DialogueManager.sendStatement(stoner, "This plant is ready to flower. Use PK 13-14 to put it in flowering mode. ", "or clear the patch.");
		return true;
	} else if (weedState[weedFieldsData.getWeedIndex()] == 2) {
		DialogueManager.sendStatement(stoner, "This plant is out of flowering time. You did not add PK 13-14 in time.", "Clear the patch.");
		return true;
	}
	if (weedStages[weedFieldsData.getWeedIndex()] == 0) {
		DialogueManager.sendStatement(stoner, "This is an cannabis patch.", "The patch needs weeding.");
	} else if (weedStages[weedFieldsData.getWeedIndex()] == 3) {
		DialogueManager.sendStatement(stoner, "This is an cannabis patch.", "The patch is empty and weeded.");
		...
		TaskQueue.queue(new Task(stoner, 5, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.CULTIVATION) {
			@Override
			public void execute() {
			if (weedStages[weedFieldsData.getWeedIndex()] - 4 < inspectData.getMessages().length - 2) {
				DialogueManager.sendStatement(stoner, inspectData.getMessages()[weedStages[weedFieldsData.getWeedIndex()] - 4]);
			} else if (weedStages[weedFieldsData.getWeedIndex()] < weedData.getEndingState() - weedData.getStartingState() + 2) {
	...
	if (weedState[weedFieldsData.getWeedIndex()] != 1) {
		stoner.send(new SendMessage("This plant doesn't need to transition to flowering stage at this point."));
		return true;
	}
	stoner.getBox().remove(itemId, 1);
	...
	TaskQueue.queue(new Task(stoner, 7, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.CULTIVATION) {
		@Override
		public void execute() {
		stoner.send(new SendMessage("Your plant reached flowering stage after giving, PK 13-14."));
		weedState[weedFieldsData.getWeedIndex()] = 0;
		stop();
		}
