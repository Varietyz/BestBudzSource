...
public class Compost {
	...
	// handle the compost bin state when the stoner retrieve the compost
	...
	ObjectManager.send(new GameObject(finalObject, x, y, stoner.getZ(), 10, CompostBinLocations.forId(index).getObjectFace()));
	}

	/* handle what happens when the stoner close the compost bin */
	...
	TaskQueue.queue(new Task(stoner, 2, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.CULTIVATION) {
		@Override
		public void execute() {
		stoner.send(new SendMessage("You close the compost bin, and its content start to rot."));
	...
	/* handle what happens when the stoner opens the compost bin */
	...
	if (Cultivation.getMinutesCounter(stoner) - compostBinsTimer[index] >= timerRequired) {
		...
		stoner.send(new SendMessage("The compost bin is still rotting. I should wait until it is complete."));
	}
	}
	...
	if (incrementFactor == 0) {
		stoner.send(new SendMessage("You need to put organic items into the compost bin in order to make compost."));
		return;
	}
	...
	// handle what happens when the stoner retrieve the compost
	...
	TaskQueue.queue(new Task(stoner, 2, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.CULTIVATION) {
		@Override
		public void execute() {
		if (!stoner.getBox().hasItemId(BUCKET) && compostBins[index] != 350 || organicItemAdded[index] == 0) {
			stop();
			return;
		}
		stoner.getProfession().addExperience(Professions.CULTIVATION, finalItem == COMPOST ? COMPOST_EXP_RETRIEVE : finalItem == SUPER_COMPOST ? SUPER_COMPOST_EXP_RETRIEVE : ROTTEN_TOMATOES_EXP_RETRIEVE);
