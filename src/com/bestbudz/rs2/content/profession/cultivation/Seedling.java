...
public class Seedling {
  ...
  public boolean waterSeedling(int itemUsed, int usedWith, int itemUsedSlot, int usedWithSlot) {
    ...
    if (seedlingData == null) seedlingData = SeedlingData.getUnwatered(usedWith);
    if (seedlingData == null
        || (!GameDefinitionLoader.getItemDef(itemUsed).getName().toLowerCase().contains("watering")
            && !GameDefinitionLoader.getItemDef(itemUsed)
                .getName()
                .toLowerCase()
                .contains("watering"))) return false;

    stoner.send(
        new SendMessage(
            "You water the " + GameDefinitionLoader.getItemDef(seedlingData.getSeedId()) + "."));
  ...
  public boolean placeSeedInPot(int itemUsed, int usedWith, int itemUsedSlot, int usedWithSlot) {
    ...
    if (seedlingData == null) seedlingData = SeedlingData.getUnwatered(usedWith);
    if (seedlingData == null || (itemUsed != 5354 && usedWith != 5354)) return false;
  ...
  public enum SeedlingData {
    ...
    private static final Map<Integer, SeedlingData> unwatered =
        new HashMap<Integer, SeedlingData>();
