...
public class SpecialPlantTwo {
	...
	public enum SpecialPlantData {
		...
		SpecialPlantData(int seedId, int harvestId, int seedAmount, int gradeRequired, int growthTime, double diseaseChance, double plantingXp, double harvestXp, int startingState, int endingState, int checkHealthState, double checkHealthExperience, int diseaseDiffValue, int deathDiffValue) {
		this.seedId = seedId;
	...
	if (specialPlantData.getGradeRequired() > stoner.getGrades()[Professions.CULTIVATION]) {
		DialogueManager.sendStatement(stoner, "You need a cultivation grade of " + specialPlantData.getGradeRequired() + " to plant this seed.");
		return true;
	}
	...
	if (stoner.getBox().getItemAmount(specialPlantData.getSeedId()) < specialPlantData.getSeedAmount()) {
		DialogueManager.sendStatement(stoner, "You need atleast " + specialPlantData.getSeedAmount() + " seeds to plant here.");
		return true;
	}
	...
	TaskQueue.queue(new Task(stoner, 3, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.CULTIVATION) {
		...
		specialPlantTimer[specialPlantFieldsData.getSpecialPlantsIndex()] = Cultivation.getMinutesCounter(stoner);
	...
	TaskQueue.queue(new Task(stoner, 2, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.CULTIVATION) {
		...
		stoner.send(new SendMessage("You harvest the crop, and pick some " + specialPlantData.getHarvestId() + "."));
