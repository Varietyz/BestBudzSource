...
public class Hops {
	...
	public enum HopsData {
		...
		HopsData(int seedId, int harvestId, int seedAmount, int gradeRequired, int[] paymentToWatch, int growthTime, double diseaseChance, double plantingXp, double harvestXp, int startingState, int endingState) {
		this.seedId = seedId;
	...
	public enum HopsFieldsData {
		...
		for (HopsFieldsData hopsFieldsData : HopsFieldsData.values()) {
			if (CultivationConstants.inRangeArea(hopsFieldsData.getHopsPosition()[0], hopsFieldsData.getHopsPosition()[1], x, y)) {
	...
	public enum InspectData {
		...
		JUTE(5306, new String[][] { { "The Jute seeds have only just been planted." }, { "The jute plants grow taller." }, { "The jute plants grow taller." }, { "The jute plants grow taller." }, { "The jute plant grows taller. They are as high", "as the stoner." }, { "The jute plants are ready to harvest." } }),
	...
	for (int i = 0; i < hopsSeeds.length; i++) {
		if (hopsStages[i] > 0 && hopsStages[i] <= 3 && Cultivation.getMinutesCounter(stoner) - hopsTimer[i] >= 5) {
			hopsStages[i]--;
	...
	if (hopsData.getGradeRequired() > stoner.getGrades()[Professions.CULTIVATION]) {
		DialogueManager.sendStatement(stoner, "You need a cultivation grade of " + hopsData.getGradeRequired() + " to plant this seed.");
		return true;
	}
	...
	if (stoner.getBox().getItemAmount(hopsData.getSeedId()) < hopsData.getSeedAmount()) {
		DialogueManager.sendStatement(stoner, "You need at least " + hopsData.getSeedAmount() + " seeds to plant here.");
		return true;
	}
	...
	if (hopsStages[hopsFieldsData.getHopsIndex()] == 0) {
		DialogueManager.sendStatement(stoner, "This is a hops patch. The soil has not been treated.", "The patch needs weeding.");
	} else if (hopsStages[hopsFieldsData.getHopsIndex()] == 3) {
		DialogueManager.sendStatement(stoner, "This is a hops patch. The soil has not been treated.", "The patch is empty and weeded.");
		...
		TaskQueue.queue(new Task(stoner, 5, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.CULTIVATION) {
			...
			if (hopsStages[hopsFieldsData.getHopsIndex()] - 4 < inspectData.getMessages().length - 2) {
				DialogueManager.sendStatement(stoner, inspectData.getMessages()[hopsStages[hopsFieldsData.getHopsIndex()] - 4]);
