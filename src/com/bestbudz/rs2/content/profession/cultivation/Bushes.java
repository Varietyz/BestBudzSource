...
public class Bushes {
  ...
  public void doCalculations() {
    for (int i = 0; i < bushesSeeds.length; i++) {
      if (bushesStages[i] > 0
          && bushesStages[i] <= 3
          && Cultivation.getMinutesCounter(stoner) - bushesTimer[i] >= 5) {
        bushesStages[i]--;
      ...
      if (4 + state != bushesStages[i]) {
        ...
        if (bushesStages[i] <= 4 + state)
          for (int j = bushesStages[i]; j <= 4 + state; j++) doStateCalculation(i);
        updateBushesStates();
      }
    }
  }
  ...
  public boolean plantSeed(int objectX, int objectY, final int seedId) {
    ...
    if (stoner.getBox().getItemAmount(bushesData.getSeedId()) < bushesData.getSeedAmount()) {
      DialogueManager.sendStatement(
          stoner, "You need atleast " + bushesData.getSeedAmount() + " seeds to plant here.");
      return true;
    }
    ...
    TaskQueue.queue(
        new Task(
          ...
          public void execute() {
            ...
            stoner
                .getProfession()
                .addExperience(Professions.CULTIVATION, bushesData.getPlantingXp());
            stop();
          }
        ...
        new Task(
          ...
          public void execute() {
            ...
            if (bushesState[bushesFieldsData.getBushesIndex()] == 3) {
              stoner.send(
                  new SendMessage(
                      "You inspect the bush for signs of disease and find that it's in perfect health."));
              stoner
                  .getProfession()
                  .addExperience(Professions.CULTIVATION, bushesData.getCheckHealthXp());
              bushesState[bushesFieldsData.getBushesIndex()] = 0;
              hasFullyGrown[bushesFieldsData.getBushesIndex()] = false;
              bushesTimer[bushesFieldsData.getBushesIndex()] =
                  Cultivation.getMinutesCounter(stoner) - bushesData.getGrowthTime();
            ...
            stoner
                .getProfession()
                .addExperience(Professions.CULTIVATION, bushesData.getHarvestXp());
            bushesTimer[bushesFieldsData.getBushesIndex()] = Cultivation.getMinutesCounter(stoner);
            int difference = bushesData.getEndingState() - bushesData.getStartingState();
            int growth = bushesData.getGrowthTime();
            lowerStage(
                ...
                growth
                    - (growth / difference)
                        * (difference + 5 - bushesStages[bushesFieldsData.getBushesIndex()]));
            modifyStage(bushesFieldsData.getBushesIndex());
            stop();
          }
  ...
  public boolean putCompost(int objectX, int objectY, final int itemId) {
    ...
    TaskQueue.queue(
        new Task(
          ...
          @Override
          public void execute() {
            diseaseChance[bushesFieldsData.getBushesIndex()] *=
  ...
  public boolean inspect(int objectX, int objectY) {
    ...
    final InspectData inspectData =
        InspectData.forId(bushesSeeds[bushesFieldsData.getBushesIndex()]);
    final BushesData bushesData = BushesData.forId(bushesSeeds[bushesFieldsData.getBushesIndex()]);
    ...
    if (bushesStages[bushesFieldsData.getBushesIndex()] == 0) {
      ...
      TaskQueue.queue(
          new Task(
            ...
            @Override
            public void execute() {
              if (bushesStages[bushesFieldsData.getBushesIndex()] - 4
                  < inspectData.getMessages().length - 2) {
                DialogueManager.sendStatement(
                    stoner,
                    inspectData.getMessages()[bushesStages[bushesFieldsData.getBushesIndex()] - 4]);
              } else if (bushesStages[bushesFieldsData.getBushesIndex()]
                  < bushesData.getEndingState() - bushesData.getStartingState() + 2) {
                DialogueManager.sendStatement(
  ...
  public boolean checkIfRaked(int objectX, int objectY) {
    ...
    if (bushesFieldData == null) return false;
    return bushesStages[bushesFieldData.getBushesIndex()] == 3;
  }

  public enum BushesData {
    REDBERRY(
        5101, 1951, 1, 10, new int[] {5478, 4}, 100, 0.20, 11.5, 4.5, 0x05, 0x0e, 0x09, 0x3a, 64),
    CADAVABERRY(
        5102, 753, 1, 22, new int[] {5968, 3}, 140, 0.20, 18, 7, 0x0f, 0x19, 0x14, 0x3b, 102.5),
    DWELLBERRY(
        5103, 2126, 1, 36, new int[] {5406, 3}, 140, 0.20, 31.5, 12, 0x1a, 0x25, 0x20, 0x3c, 177.5),
    JANGERBERRY(
        5104, 247, 1, 48, new int[] {5982, 6}, 160, 0.20, 50.5, 19, 0x26, 0x32, 0x2d, 0x3d, 284.5),
    WHITEBERRY(
        5105, 239, 1, 59, new int[] {6004, 8}, 160, 0.20, 78, 29, 0x33, 0x3f, 0x3a, 0x3e, 437.5),
  ...
  public enum BushesFieldsData {
    ...
    private static final Map<Integer, BushesFieldsData> npcsProtecting =
        new HashMap<Integer, BushesFieldsData>();
    ...
    public static BushesFieldsData forIdPosition(int x, int y) {
      for (BushesFieldsData bushesFieldsData : BushesFieldsData.values()) {
        if (CultivationConstants.inRangeArea(
            ...
            bushesFieldsData.getBushesPosition()[1],
            x,
            y)) {
  ...
  public enum InspectData {
    REDBERRY(
        ...
        new String[][] {
          ...
          {"The Redberry bush is ready to harvest.", "The berries on the bush are red."},
        }),
    CADAVABERRY(
        5102,
        new String[][] {
          ...
          {"The berries grow larger, and pink."},
          {"The Cadavaberry bush is ready to harvest.", "The berries on the bush are purple."}
        }),
    DWELLBERRY(
        5103,
        new String[][] {
          ...
          {"The Dwellbery bush is ready to harvest.", "The berries on the bush are blue."},
        }),
    JANGERBERRY(
        5104,
        new String[][] {
    ...
    WHITEBERRY(
        ...
        new String[][] {
          ...
          {"The berries grow larger."},
          {"The Whiteberry bush is ready to harvest.", "The berries on the bush are white."},
        }),
    POISONIVYBERRY(
        5106,
        new String[][] {
