...
public class WoodTrees {
  ...
  public int getConfigValue(int treeStage, int saplingId, int plantState, int index) {
    ...
    if (plantState == 6) return treeData.getChopDownState();
    if (plantState == 7) return treeData.getStumpState();
  ...
  public void doCalculations() {
    for (int i = 0; i < treeSaplings.length; i++) {
      if (treeStages[i] > 0
          && treeStages[i] <= 3
          && Cultivation.getMinutesCounter(stoner) - treeTimer[i] >= 5) {
        treeStages[i]--;
      ...
      if (4 + state != treeStages[i]) {
        ...
        if (treeStages[i] <= 4 + state)
          for (int j = treeStages[i]; j <= 4 + state; j++) doStateCalculation(i);
        updateTreeStates();
      }
    }
  }
  ...
  public boolean checkHealth(int objectX, int objectY) {
    ...
    if (treeState[treeFieldsData.getTreeIndex()] != 0) return false;
    ...
    TaskQueue.queue(
        new Task(
          ...
          public void execute() {
            stoner.send(
                new SendMessage(
                    "You inspect the tree for signs of disease and find that it is in perfect health"));
            stoner
                .getProfession()
                .addExperience(Professions.CULTIVATION, treeData.getCheckHealthXp());
  ...
  public void respawnStumpTimer(final int index) {
    TaskQueue.queue(
        new Task(
          ...
          @Override
          public void execute() {
            if (treeState[index] == 7) treeState[index] = 6;
            stop();
          }
  ...
  public boolean putCompost(int objectX, int objectY, final int itemId) {
    ...
    TaskQueue.queue(
        new Task(
          ...
          @Override
          public void execute() {
            diseaseChance[treeFieldsData.getTreeIndex()] *=
    ...
    if (treeStages[treeFieldsData.getTreeIndex()] == 0) {
      ...
      TaskQueue.queue(
          new Task(
            ...
            @Override
            public void execute() {
              if (treeStages[treeFieldsData.getTreeIndex()] - 4
                  < inspectData.getMessages().length - 2) {
                DialogueManager.sendStatement(
                    stoner,
                    inspectData.getMessages()[treeStages[treeFieldsData.getTreeIndex()] - 4]);
              } else if (treeStages[treeFieldsData.getTreeIndex()]
                  < treeData.getEndingState() - treeData.getStartingState() + 2) {
                DialogueManager.sendStatement(
  ...
  public boolean cut(final int x, final int y) {
    ...
    if (treeFieldsData == null) return false;
    final TreeData treeData = TreeData.forId(treeSaplings[treeFieldsData.getTreeIndex()]);
    ...
    if (ChopTree.getAxe(stoner) == null) {
      stoner.send(
          new SendMessage("You do not have an axe which you have the lumbering grade to use."));
      return true;
    }
        ...
        new Task(
          ...
          public void execute() {
            ...
            if (Professions.isSuccess(
                stoner, ChopTree.getTree(object).getGrade(), ChopTree.getAxe(stoner).getGrade())) {
              stoner.getBox().add(ChopTree.getTree(object).getLog(), 1);
              stoner
                  .getProfession()
                  .addExperience(Professions.LUMBERING, ChopTree.getTree(object).getXP());
    ...
    if (stoner.getGrades()[Professions.LUMBERING] < ChopTree.getTree(object).getGrade()) {
      DialogueManager.sendStatement(
          ...
          "You need a lumbering grade of "
              + ChopTree.getTree(object).getGrade()
              + " to cho this tree.");
      return false;
    }
    return true;
  ...
  public enum TreeData {
    ...
    WILLOW(
        5371, 6045, 30, new int[] {5386, 1}, 240, 0.20, 25, 1456.3, 0x0f, 0x15, 0x16, 0x17, 1308),
    ...
    private static final Map<Integer, TreeData> saplings = new HashMap<Integer, TreeData>();
    ...
    private final int chopDownState;
    private final int stumpState;
    private final int treeObjectAssociated;
  ...
  public enum TreeFieldsData {
    ...
    private static final Map<Integer, TreeFieldsData> npcsProtecting =
        new HashMap<Integer, TreeFieldsData>();
    ...
    private final Point[] treePosition;
    ...
    public static TreeFieldsData forIdPosition(int x, int y) {
      for (TreeFieldsData treeFieldsData : TreeFieldsData.values()) {
        if (CultivationConstants.inRangeArea(
            treeFieldsData.getTreePosition()[0], treeFieldsData.getTreePosition()[1], x, y)) {
  ...
  public enum InspectData {
    OAK(
        ...
        new String[][] {
          ...
          {"The oak tree is ready to harvest."}
        }),
    WILLOW(
        5371,
        new String[][] {
          ...
          {"The willow tree is fully grown."}
        }),
    MAPLE(
        5372,
        new String[][] {
          ...
          {"The maple tree is ready to be harvested."}
        }),
    YEW(
        5373,
        new String[][] {
          ...
          {
            "The yew tree canopies become more angular and", "cone shaped due to texture placement."
          },
          ...
          {"The yew tree is ready to be harvested."}
        }),
    MAGE(
        5374,
        new String[][] {
          {"The mage sapling has only just been planted."},
          {"The mage sapling grows a little bit."},
          {"The mage sapling grows a little bit more."},
          {"The mage sapling grows a few small branches."},
          {"The mage tree grows a small canopy."},
          {"The mage tree canopy becomes larger, and", "starts producing sparkles."},
          {"The mage tree grows and the base becomes lighter."},
          {"The mage tree grows and the base becomes darker."},
          {"The mage tree's bark is more prominent and the", "canopy gains more sparkles."},
          {"The mage tree grows taller."},
          {"The mage tree grows taller."},
          {"The mage tree is ready to be harvested."}
        });
