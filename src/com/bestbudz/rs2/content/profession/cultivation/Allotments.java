...
public class Allotments {
  ...
  public void updateAllotmentsStates() {
    ...
    configValue =
        configValues[4] << 16 | configValues[5] << 8 << 16 | configValues[6] | configValues[7] << 8;
  ...
  public void doCalculations() {
    ...
    for (int i = 0; i < allotmentSeeds.length; i++) {
      if (allotmentStages[i] > 0
          && allotmentStages[i] <= 3
          && Cultivation.getMinutesCounter(stoner) - allotmentTimer[i] >= 5) {
        allotmentStages[i]--;
      ...
      if (4 + state != allotmentStages[i]) {
        ...
        if (allotmentStages[i] <= 4 + state)
          for (int j = allotmentStages[i]; j <= 4 + state; j++) doStateCalculation(i);
        updateAllotmentsStates();
      }
    }
  }
  ...
  public boolean waterPatch(int objectX, int objectY, int itemId) {
    ...
    final AllotmentFieldsData allotmentFieldsData =
        AllotmentFieldsData.forIdPosition(objectX, objectY);
    if (allotmentFieldsData == null) {
      return false;
    }
    AllotmentData allotmentData =
        AllotmentData.forId(allotmentSeeds[allotmentFieldsData.getAllotmentIndex()]);
    if (allotmentData == null) {
      return false;
    }
    if (allotmentState[allotmentFieldsData.getAllotmentIndex()] == 1
        ...
        || allotmentStages[allotmentFieldsData.getAllotmentIndex()]
            == allotmentData.getEndingState() - allotmentData.getStartingState() + 4) {
    ...
    TaskQueue.queue(
        new Task(
          ...
          public void execute() {
            ...
            allotmentTimer[allotmentFieldsData.getAllotmentIndex()] =
  ...
  public boolean plantSeed(int objectX, int objectY, final int seedId) {
    ...
    if (allotmentData.getGradeRequired() > stoner.getGrades()[Professions.CULTIVATION]) {
      DialogueManager.sendStatement(
          ...
          "You need a cultivation grade of "
              + allotmentData.getGradeRequired()
              + " to plant this seed.");
      return true;
    }
    ...
    if (!stoner.getBox().hasItemAmount(allotmentData.getSeedId(), allotmentData.getSeedAmount())) {
      DialogueManager.sendStatement(
          stoner, "You need atleast " + allotmentData.getSeedAmount() + " seeds to plant here.");
      return true;
    }
    ...
    stoner
        .getBox()
        .removeFromSlot(stoner.getBox().getItemSlot(seedId), seedId, allotmentData.getSeedAmount());
    ...
    TaskQueue.queue(
        new Task(
          ...
          public void execute() {
            ...
            stoner
                .getProfession()
                .addExperience(Professions.CULTIVATION, allotmentData.getPlantingXp());
            stop();
          }
  ...
  public boolean harvest(int objectX, int objectY) {
    ...
    final AllotmentData allotmentData =
        AllotmentData.forId(allotmentSeeds[allotmentFieldsData.getAllotmentIndex()]);
    if (allotmentData == null) {
      return false;
    }
    ...
    TaskQueue.queue(
        new Task(
          ...
          public void execute() {
              ...
              allotmentHarvest[allotmentFieldsData.getAllotmentIndex()] =
                  1
                      + (START_HARVEST_AMOUNT
                          + Utility.random(
                              (END_HARVEST_AMOUNT
                                      + (stoner.getEquipment().isWearingItem(7409) ? 15 : 0))
                                  - START_HARVEST_AMOUNT));
            }
            ...
            stoner
                .getProfession()
                .addExperience(Professions.CULTIVATION, allotmentData.getHarvestXp());
  ...
  public boolean putCompost(int objectX, int objectY, final int itemId) {
    ...
    TaskQueue.queue(
        new Task(
          ...
          @Override
          public void execute() {
            diseaseChance[allotmentFieldsData.getAllotmentIndex()] *=
                0.001 * (itemId == 6032 ? COMPOST_CHANCE : SUPERCOMPOST_CHANCE);
            allotmentState[allotmentFieldsData.getAllotmentIndex()] = 5;
            stop();
          }
  ...
  public boolean inspect(int objectX, int objectY) {
    ...
    final InspectData inspectData =
        InspectData.forId(allotmentSeeds[allotmentFieldsData.getAllotmentIndex()]);
    final AllotmentData allotmentData =
        AllotmentData.forId(allotmentSeeds[allotmentFieldsData.getAllotmentIndex()]);
    ...
    if (allotmentStages[allotmentFieldsData.getAllotmentIndex()] == 0) {
      ...
      TaskQueue.queue(
          new Task(
            ...
            @Override
            public void execute() {
              if (allotmentStages[allotmentFieldsData.getAllotmentIndex()] - 4
                ...
                DialogueManager.sendStatement(
                    ...
                    inspectData
                        .getMessages()[
                        allotmentStages[allotmentFieldsData.getAllotmentIndex()] - 4]);
              } else if (allotmentStages[allotmentFieldsData.getAllotmentIndex()]
                  < allotmentData.getEndingState() - allotmentData.getStartingState() + 2) {
                DialogueManager.sendStatement(
  ...
  public void handleFlowerProtection(int index) {
    ...
    if (!allotmentWatched[index]) {
      ...
      if (stoner.getCultivation().getFlowers().flowerSeeds[indexGiven] >= 0x21
          && stoner.getCultivation().getFlowers().flowerSeeds[indexGiven] <= 0x24) {
        if (allotmentData.getFlowerProtect() == Flowers.SCARECROW) {
          return;
        }
      }
      if (stoner.getCultivation().getFlowers().flowerState[indexGiven] != 3
          && stoner.getCultivation().getFlowers().hasFullyGrown[indexGiven]
          && stoner.getCultivation().getFlowers().flowerSeeds[indexGiven]
              == allotmentData.getFlowerProtect()) {
        stoner.getCultivation().getFlowers().flowerState[indexGiven] = 3;
        stoner.getCultivation().getFlowers().updateFlowerStates();
      } else if (Utility.random(100) <= maxChance
  ...
  public boolean checkIfRaked(int objectX, int objectY) {
    final AllotmentFieldsData allotmentFieldsData =
        AllotmentFieldsData.forIdPosition(objectX, objectY);
    if (allotmentFieldsData == null) return false;
  ...
  public enum AllotmentData {
    ...
    private static final Map<Integer, AllotmentData> seeds = new HashMap<Integer, AllotmentData>();
  ...
  public enum AllotmentFieldsData {
    CATWEEDY_NORTH(
        0,
        new Point[] {
          new Point(2805, 3466), new Point(2806, 3468), new Point(2805, 3467), new Point(2814, 3468)
        },
        2324),
    ...
    CATWEEDY_SOUTH(
        1,
        new Point[] {
          new Point(2805, 3459), new Point(2806, 3461), new Point(2802, 3459), new Point(2814, 3460)
        },
        2324),
    ...
    FALADOR_NORTH_WEST(
        2,
        new Point[] {
          new Point(3050, 3307), new Point(3051, 3312), new Point(3050, 3311), new Point(3054, 3312)
        },
        2323),
    ...
    FALADOR_SOUTH_EAST(
        3,
        new Point[] {
          new Point(3055, 3303), new Point(3059, 3304), new Point(3058, 3303), new Point(3059, 3308)
        },
        2323),

    PHASMATYS_NORTH_WEST(
        4,
        new Point[] {
          new Point(3597, 3525), new Point(3598, 3530), new Point(3597, 3529), new Point(3601, 3530)
        },
        2326),

    PHASMATYS_SOUTH_EAST(
        5,
        new Point[] {
          new Point(3602, 3521), new Point(3606, 3522), new Point(3605, 3521), new Point(3606, 3526)
        },
        2326),
    ...
    ARDOUGNE_NORTH(
        6,
        new Point[] {
          new Point(2662, 3377), new Point(2663, 3379), new Point(2662, 3378), new Point(2671, 3379)
        },
        2325),
    ...
    ARDOUGNE_SOUTH(
        7,
        new Point[] {
          new Point(2662, 3370), new Point(2663, 3372), new Point(2662, 3370), new Point(2671, 3371)
        },
        2325);

    private final int allotmentIndex;
    private final Point[] allotmentPosition;
    ...
    public static AllotmentFieldsData forIdPosition(int x, int y) {
      for (AllotmentFieldsData allotmentFieldsData : AllotmentFieldsData.values()) {
        if (CultivationConstants.inRangeArea(
                ...
                x,
                y)
            || CultivationConstants.inRangeArea(
