...
public class FruitTree {
	...
	public enum FruitTreeFieldsData {
		...
		for (FruitTreeFieldsData fruitTreeFieldsData : FruitTreeFieldsData.values()) {
			if (CultivationConstants.inRangeArea(fruitTreeFieldsData.getFruitTreePosition()[0], fruitTreeFieldsData.getFruitTreePosition()[1], x, y)) {
	...
	// brimhaven - catweedy - tree stronghold - tree village
	...
	for (int i = 0; i < fruitTreeSaplings.length; i++) {
		if (fruitTreeStages[i] > 0 && fruitTreeStages[i] <= 3 && Cultivation.getMinutesCounter(stoner) - fruitTreeTimer[i] >= 5) {
			fruitTreeStages[i]--;
	...
	int finalAnimation;
	int finalDelay;
	if (fruitTreeFieldsData == null || (itemId != CultivationConstants.RAKE && itemId != CultivationConstants.SPADE)) {
		return false;
	}
	...
	TaskQueue.queue(new Task(stoner, finalDelay, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.CULTIVATION) {
		...
		fruitTreeTimer[fruitTreeFieldsData.getFruitTreeIndex()] = Cultivation.getMinutesCounter(stoner);
		updateFruitTreeStates();
		if (fruitTreeStages[fruitTreeFieldsData.getFruitTreeIndex()] == 3) {
			stop();
			return;
	...
	if (fruitTreeData.getGradeRequired() > stoner.getGrades()[Professions.CULTIVATION]) {
		DialogueManager.sendStatement(stoner, "You need a cultivation grade of " + fruitTreeData.getGradeRequired() + " to plant this sapling.");
		return true;
	}
	...
	TaskQueue.queue(new Task(stoner, 3, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.CULTIVATION) {
		...
		fruitTreeTimer[fruitTreeFieldsData.getFruitTreeIndex()] = Cultivation.getMinutesCounter(stoner);
		stoner.getProfession().addExperience(Professions.CULTIVATION, fruitTreeData.getPlantingXp());
		stop();
		}
	...
	TaskQueue.queue(new Task(stoner, 2, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.CULTIVATION) {
		...
		if (fruitTreeState[fruitTreeFieldsData.getFruitTreeIndex()] == 3) {
			stoner.send(new SendMessage("You inspect the tree for signs of disease and find that it's in perfect health."));
			stoner.getProfession().addExperience(Professions.CULTIVATION, fruitTreeData.getCheckHealthXp());
			fruitTreeState[fruitTreeFieldsData.getFruitTreeIndex()] = 0;
			hasFullyGrown[fruitTreeFieldsData.getFruitTreeIndex()] = false;
			fruitTreeTimer[fruitTreeFieldsData.getFruitTreeIndex()] = Cultivation.getMinutesCounter(stoner) - fruitTreeData.getGrowthTime();
	...
	if (fruitTreeState[fruitTreeFieldsData.getFruitTreeIndex()] == 1) {
		...
		DialogueManager.sendStatement(stoner, "This is a fruit tree stump, to remove it, use a ", "spade on it to clear the patch");
		return true;
	}
	if (fruitTreeStages[fruitTreeFieldsData.getFruitTreeIndex()] == 0) {
		DialogueManager.sendStatement(stoner, "This is a fruit tree patch. The soil has not been treated.", "The patch needs weeding.");
	} else if (fruitTreeStages[fruitTreeFieldsData.getFruitTreeIndex()] == 3) {
		DialogueManager.sendStatement(stoner, "This is a fruit tree patch. The soil has not been treated.", "The patch is empty and weeded.");
		...
		TaskQueue.queue(new Task(stoner, 5, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.CULTIVATION) {
			...
			if (fruitTreeStages[fruitTreeFieldsData.getFruitTreeIndex()] - 4 < inspectData.getMessages().length - 2) {
				DialogueManager.sendStatement(stoner, inspectData.getMessages()[fruitTreeStages[fruitTreeFieldsData.getFruitTreeIndex()] - 4]);
	...
	final FruitTreeFieldsData fruitTreeFieldsData = FruitTreeFieldsData.forIdPosition(objectX, objectY);
	if (fruitTreeFieldsData == null || (itemId != CultivationConstants.SECATEURS && itemId != CultivationConstants.MAGE_SECATEURS)) {
		return false;
	}
	final FruitTreeData fruitTreeData = FruitTreeData.forId(fruitTreeSaplings[fruitTreeFieldsData.getFruitTreeIndex()]);
	if (fruitTreeData == null) {
