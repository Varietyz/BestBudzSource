...
public class Flowers {
  ...
  public void doCalculations() {
    for (int i = 0; i < flowerSeeds.length; i++) {
      if (flowerStages[i] > 0
          && flowerStages[i] <= 3
          && Cultivation.getMinutesCounter(stoner) - flowerTimer[i] >= 5) {
        flowerStages[i]--;
        flowerTimer[i] = Cultivation.getMinutesCounter(stoner);
        updateFlowerStates();
      }
      if (Cultivation.getMinutesCounter(stoner) - flowerTimer[i] >= 5
          && flowerSeeds[i] > 0x21
          && flowerSeeds[i] <= 0x24) {
        flowerSeeds[i]--;
        updateFlowerStates();
  ...
  public boolean waterPatch(int objectX, int objectY, int itemId) {
    ...
    if (flowerState[flowerFieldsData.getFlowerIndex()] == 1
        ...
        || flowerStages[flowerFieldsData.getFlowerIndex()]
            == flowerData.getEndingState() - flowerData.getStartingState() + 4) {
    ...
    TaskQueue.queue(
        new Task(
          ...
          public void execute() {
            ...
            stoner
                .getProfession()
                .addExperience(Professions.CULTIVATION, flowerData.getPlantingXp());
            stop();
          }
        ...
        new Task(
          ...
          public void execute() {
            ...
            stoner
                .getBox()
                .add(
                    flowerData.getHarvestId(),
                    flowerData.getHarvestId() == 5099 || flowerData.getHarvestId() == 5100 ? 3 : 1);
            stoner
                .getProfession()
                .addExperience(Professions.CULTIVATION, flowerData.getHarvestXp());
            stop();
          }
  ...
  public boolean putCompost(int objectX, int objectY, final int itemId) {
    ...
    TaskQueue.queue(
        new Task(
          ...
          @Override
          public void execute() {
            diseaseChance[flowerFieldsData.getFlowerIndex()] *=
  ...
  public boolean inspect(int objectX, int objectY) {
    ...
    final InspectData inspectData =
        InspectData.forId(flowerSeeds[flowerFieldsData.getFlowerIndex()]);
    final FlowerData flowerData = FlowerData.forId(flowerSeeds[flowerFieldsData.getFlowerIndex()]);
  ...
  public enum FlowerData {
    ...
    private static final Map<Integer, FlowerData> seeds = new HashMap<Integer, FlowerData>();
  ...
  public enum FlowerFieldsData {
    ...
    private final int flowerIndex;
    private final Point[] flowerPosition;
    ...
    public static FlowerFieldsData forIdPosition(Point point) {
      for (FlowerFieldsData flowerFieldsData : FlowerFieldsData.values()) {
        if (CultivationConstants.inRangeArea(
            ...
            flowerFieldsData.getFlowerPosition()[1],
            point)) {
