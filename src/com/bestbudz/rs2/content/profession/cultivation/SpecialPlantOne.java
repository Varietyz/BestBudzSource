...
public class SpecialPlantOne {
  ...
  public void updateSpecialPlants() {
    ...
    for (int i = 0; i < specialPlantStages.length; i++) {
      configValues[i] =
          getConfigValue(specialPlantStages[i], specialPlantSaplings[i], specialPlantState[i], i);
    }
      ...
      if (4 + state != specialPlantStages[i]
        ...
        if (specialPlantStages[i]
            == specialPlantData.getEndingState()
                ...
                + (specialPlantData == SpecialPlantData.SPIRIT_TREE ? 3 : -2)) {
          specialPlantStages[i] =
              specialPlantData.getEndingState() - specialPlantData.getStartingState() + 7;
          specialPlantState[i] = 3;
          updateSpecialPlants();
          return;
        }
    ...
    TaskQueue.queue(
        new Task(
          ...
          public void execute() {
            ...
            specialPlantTimer[specialPlantFieldData.getSpecialPlantsIndex()] =
                Cultivation.getMinutesCounter(stoner);
            stoner
                .getProfession()
            ...
            stoner.send(
                new SendMessage(
                    "You harvest the crop, and pick some "
                        + GameDefinitionLoader.getItemDef(specialPlantData.getHarvestId())
                            .getName()
                            .toLowerCase()
                        + "."));
  ...
  public boolean inspect(int objectX, int objectY) {
    ...
    final InspectData inspectData =
        InspectData.forId(specialPlantSaplings[specialPlantFieldsData.getSpecialPlantsIndex()]);
    final SpecialPlantData specialPlantData =
        SpecialPlantData.forId(
            specialPlantSaplings[specialPlantFieldsData.getSpecialPlantsIndex()]);
  ...
  public enum SpecialPlantData {
    ...
    private static final Map<Integer, SpecialPlantData> saplings =
    ...
    SPIRIT_TREE(
        ...
        new String[][] {
          ...
          },
          {
            ...
            "The Spirit tree is ready to be checked."
          }
        }),
    CALQUAT_TREE(
        5503,
