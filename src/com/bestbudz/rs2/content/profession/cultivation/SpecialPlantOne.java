...
public class SpecialPlantOne {
	...
	public SpecialPlantOne(Stoner stoner) {
	this.stoner = stoner;
	}
	...
	public enum SpecialPlantData {
		...
		SpecialPlantData(int saplingId, int harvestId, int saplingAmount, int gradeRequired, int growthTime, double diseaseChance, double plantingXp, double harvestXp, int startingState, int endingState, int checkHealthState, double checkHealthExperience, int diseaseDiffValue, int deathDiffValue) {
	...
	public enum SpecialPlantFieldsData {
		...
		for (SpecialPlantFieldsData specialPlantFieldsData : SpecialPlantFieldsData.values()) {
			if (CultivationConstants.inRangeArea(specialPlantFieldsData.getSpecialPlantPosition()[0], specialPlantFieldsData.getSpecialPlantPosition()[1], x, y)) {
	...
	for (int i = 0; i < specialPlantSaplings.length; i++) {
		if (specialPlantStages[i] > 0 && specialPlantStages[i] <= 3 && Cultivation.getMinutesCounter(stoner) - specialPlantTimer[i] >= 5) {
	...
	TaskQueue.queue(new Task(stoner, finalDelay, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.CULTIVATION) {
		...
		specialPlantTimer[hopsFieldsData.getSpecialPlantsIndex()] = Cultivation.getMinutesCounter(stoner);
		updateSpecialPlants();
		if (specialPlantStages[hopsFieldsData.getSpecialPlantsIndex()] == 3) {
			stop();
			return;
	...
	if ((specialPlantStages[0] > 3 || specialPlantStages[2] > 3 || specialPlantStages[3] > 3) && (specialPlantFieldData.getSpecialPlantsIndex() != 1)) {
		stoner.send(new SendMessage("You already have a spirit tree planted somewhere else."));
		return true;
	...
	if (specialPlantData.getGradeRequired() > stoner.getGrades()[Professions.CULTIVATION]) {
		DialogueManager.sendStatement(stoner, "You need a cultivation grade of " + specialPlantData.getGradeRequired() + " to plant this sapling.");
		return true;
	}
	...
	TaskQueue.queue(new Task(stoner, 3, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.CULTIVATION) {
		...
		specialPlantTimer[specialPlantFieldData.getSpecialPlantsIndex()] = Cultivation.getMinutesCounter(stoner);
		stoner.getProfession().addExperience(Professions.CULTIVATION, specialPlantData.getPlantingXp());
		stop();
		}
	...
	TaskQueue.queue(new Task(stoner, 2, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.CULTIVATION) {
		...
		if (specialPlantState[specialPlantFieldsData.getSpecialPlantsIndex()] == 3) {
			stoner.send(new SendMessage("You inspect the plant for signs of disease and find that it's in perfect health."));
			stoner.getProfession().addExperience(Professions.CULTIVATION, specialPlantData.getCheckHealthXp());
			...
			specialPlantTimer[specialPlantFieldsData.getSpecialPlantsIndex()] = Cultivation.getMinutesCounter(stoner) - specialPlantData.getGrowthTime();
			modifyStage(specialPlantFieldsData.getSpecialPlantsIndex());
			stop();
			return;
		}

		stoner.send(new SendMessage("You harvest the crop, and pick some " + GameDefinitionLoader.getItemDef(specialPlantData.getHarvestId()).getName().toLowerCase() + "."));
		stoner.getBox().add(specialPlantData.getHarvestId(), 1);
		stoner.getProfession().addExperience(Professions.CULTIVATION, specialPlantData.getHarvestXp());
	...
	// SpiritTree.sendDialogue(stoner, 3636);
	}
	...
	if (specialPlantStages[specialPlantFieldsData.getSpecialPlantsIndex()] == 0) {
		DialogueManager.sendStatement(stoner, "This is one of the special patches. The soil has not been treated.", "The patch needs weeding.");
	} else if (specialPlantStages[specialPlantFieldsData.getSpecialPlantsIndex()] == 3) {
		DialogueManager.sendStatement(stoner, "This is one of the special patches. The soil has not been treated.", "The patch is empty and weeded.");
		...
		TaskQueue.queue(new Task(stoner, 5, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.CULTIVATION) {
			...
			if (specialPlantStages[specialPlantFieldsData.getSpecialPlantsIndex()] - 4 < inspectData.getMessages().length - 2) {
				DialogueManager.sendStatement(stoner, inspectData.getMessages()[specialPlantStages[specialPlantFieldsData.getSpecialPlantsIndex()] - 4]);
