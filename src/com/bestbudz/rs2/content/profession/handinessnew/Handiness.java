...
public enum Handiness {
	...
	case 1:
		...
		stoner.send(new SendItemOnInterface(1746, 170, craftable.getCraftableItems()[0].getProduct().getId()));
		stoner.send(new SendChatBoxInterface(4429));
		return true;

	case 2:
		stoner.getAttributes().set(CRAFTABLE_KEY, craftable);
		stoner.send(new SendItemOnInterface(8869, 170, craftable.getCraftableItems()[0].getProduct().getId()));
		stoner.send(new SendItemOnInterface(8870, 170, craftable.getCraftableItems()[1].getProduct().getId()));
	...
	case 3:
		stoner.getAttributes().set(CRAFTABLE_KEY, craftable);
		stoner.send(new SendItemOnInterface(8883, 170, craftable.getCraftableItems()[0].getProduct().getId()));
		stoner.send(new SendItemOnInterface(8884, 170, craftable.getCraftableItems()[1].getProduct().getId()));
		stoner.send(new SendItemOnInterface(8885, 170, craftable.getCraftableItems()[2].getProduct().getId()));
	...
	case 4:
		stoner.getAttributes().set(CRAFTABLE_KEY, craftable);
		stoner.send(new SendItemOnInterface(8902, 170, craftable.getCraftableItems()[0].getProduct().getId()));
		stoner.send(new SendItemOnInterface(8903, 170, craftable.getCraftableItems()[1].getProduct().getId()));
		stoner.send(new SendItemOnInterface(8904, 170, craftable.getCraftableItems()[2].getProduct().getId()));
		stoner.send(new SendItemOnInterface(8905, 170, craftable.getCraftableItems()[3].getProduct().getId()));
	...
	case 5:
		stoner.getAttributes().set(CRAFTABLE_KEY, craftable);
		stoner.send(new SendItemOnInterface(8941, 170, craftable.getCraftableItems()[0].getProduct().getId()));
		stoner.send(new SendItemOnInterface(8942, 170, craftable.getCraftableItems()[1].getProduct().getId()));
		stoner.send(new SendItemOnInterface(8943, 170, craftable.getCraftableItems()[2].getProduct().getId()));
		stoner.send(new SendItemOnInterface(8944, 170, craftable.getCraftableItems()[3].getProduct().getId()));
		stoner.send(new SendItemOnInterface(8945, 170, craftable.getCraftableItems()[4].getProduct().getId()));
		stoner.send(new SendString("\\n \\n \\n \\n \\n".concat("Body"), 8949));
		stoner.send(new SendString("\\n \\n \\n \\n \\n".concat("Chaps"), 8953));
	...
	if (start(stoner, craftable, index, amount)) {
		return true;
	}
	...
	if (CRAFTABLES.put(craftable.getWith().getId(), craftable) != null) {
		System.out.println("[Handiness] Conflicting item values: " + craftable.getWith().getId() + " Type: " + craftable.getName());
	}
	}
	...
	if (stoner.getGrades()[Professions.HANDINESS] < item.getGrade()) {
		DialogueManager.sendStatement(stoner, "<col=369>You need a Handiness grade of " + item.getGrade() + " to do that.");
		return true;
	}
	...
	if (!stoner.getBox().hasAllItems(craftable.getIngediants(index))) {
		...
		stoner.send(new SendMessage("You need " + requiredItem.getAmount() + " piece" + (requiredItem.getAmount() > 1 ? "s" : "") + " of " + requiredItem.getDefinition().getName().toLowerCase() + " to make" + productAmount + " " + product.getDefinition().getName().toLowerCase() + "."));
		return true;
	}
