...
public final class SpellBookTeleporting {
  ...
  public static void teleport(final Stoner stoner, final int button) {
    ...
    if (stoner.getBox().hasItemId(new Item(data.getRunes()[0], data.getRunes()[1]))
        && stoner.getBox().hasItemId(new Item(data.getRunes()[2], data.getRunes()[3]))
        && stoner.getBox().hasItemId(new Item(data.getRunes()[4], data.getRunes()[5]))) {
      if (stoner.getProfession().getGrades()[Professions.MAGE] >= data.getReq()) {
        ...
        stoner
            .getClient()
            .queueOutgoingPacket(
                new SendMessage("You don't have a high enough mage grade to cast this spell."));
      }
    } else {
      stoner
          .getClient()
          .queueOutgoingPacket(
              new SendMessage("You don't have the required runes to cast this spell."));
    }
  }
  ...
  enum TeleportationData {
    ...
    TeleportationData(
        final int x, final int y, final int button, final int req, final int[] runes) {
      this.x = x;
      this.y = y;
      this.button = button;
      this.req = req;
