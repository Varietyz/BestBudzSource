...
public class Charge extends Spell {
  ...
  public static boolean isChargeActive(Stoner p) {
    return (p.getAttributes().get("charge") != null)
        && (System.currentTimeMillis() - ((Long) p.getAttributes().get("charge")).longValue()
            < 420000L);
  }
  ...
  public boolean execute(Stoner stoner) {
    if (stoner.getAttributes().get("charge") != null) {
      if (System.currentTimeMillis() - ((Long) stoner.getAttributes().get("charge")).longValue()
          < 420000L) {
        stoner
            .getClient()
            .queueOutgoingPacket(new SendMessage("You are still affected by a mage charge."));
        return false;
      }
    ...
    if (stoner.getAttributes().get("chargeCoolDown") != null) {
      if (System.currentTimeMillis()
              - ((Long) stoner.getAttributes().get("chargeCoolDown")).longValue()
          < 60000L) {
        stoner
            .getClient()
            .queueOutgoingPacket(
    ...
    } else stoner.getAttributes().remove("chargeCoolDown");
