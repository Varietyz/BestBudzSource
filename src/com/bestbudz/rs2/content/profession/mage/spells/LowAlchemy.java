...
public class LowAlchemy extends Spell {
  ...
  public boolean execute(Stoner stoner) {
    if (stoner.getProfession().locked()) return false;
    ...
    if (item == 995) {
      stoner
          .getClient()
          .queueOutgoingPacket(new SendMessage("You cannot cast alchemy on this item."));
      return false;
    }
    ...
    if (!stoner.getBox().hasSpaceFor(bestbucks)) {
      stoner
          .getClient()
          .queueOutgoingPacket(
              new SendMessage("You do not have enough box space to cast High alchemy."));
      return false;
    }
