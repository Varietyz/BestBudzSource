  ...
  public boolean execute(Stoner stoner) {
    ...
    if (!required.isEmpty()) {
      stoner.send(
          new SendMessage(
              "Super heating "
                  + GameDefinitionLoader.getItemDef(data.getProduct()).getName()
                  + " requires: "
                  + required
                  + "."));
      return false;
    ...
    if (stoner.getProfession().getGrades()[Professions.FORGING] < data.getGrade()) {
      DialogueManager.sendStatement(
          stoner, "You need a forging grade of " + data.getGrade() + " to do this!");
      return false;
    }
    ...
    if (Utility.random(100) <= 10) {
      ...
      stoner.send(
          new SendMessage(
              "Something went wrong whilst super heating your item; hurting you badly!"));
    ...
    stoner.send(
        new SendMessage(
            "You have successfully super heated "
                + GameDefinitionLoader.getItemDef(data.getProduct()).getName()
                + "."));
  ...
  public enum HeatData {
    ...
    public static HeatData getItem(Stoner stoner, int item) {
      for (HeatData data : HeatData.values()) {
        for (int i = 0; i < data.getRequired().length; i++) {
          if (item == data.getRequired()[i][0]) return data;
        }
      }
