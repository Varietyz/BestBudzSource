...
public class MageProfession {
	...
	if (stoner.inWilderness() && stoner.getWildernessGrade() >= 20) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("You can't teleport above grade 20 Wilderness."));
		return false;
	}
	if (stoner.getController().equals(ControllerManager.FIGHT_PITS_CONTROLLER) || stoner.getController().equals(ControllerManager.FIGHT_PITS_WAITING_CONTROLLER)) {
	...
	if (stoner.getController().equals(ControllerManager.PEST_WAITING_ROOM_CONTROLLER)) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("Please Exit the boat via the ladder."));
		return false;
	} else if (stoner.getController().equals(ControllerManager.PEST_CONTROLLER)) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("You can't teleport whilst in pest control."));
		stoner.getClient().queueOutgoingPacket(new SendMessage("If you wish to leave speak with the squire back at the boat."));
		return false;
	}
	...
	if (stoner.isTeleblocked()) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("A mage force blocks you from teleporting."));
		return false;
	} else if (teleporting) {
		return false;
	...
	switch (buttonId) {
	case 75008:
		stoner.getClient().queueOutgoingPacket(new SendMessage("You don't have a house to teleport to!"));
		return true;
	case 4169:
	...
	case 8015:
		...
		stoner.send(new SendMessage("You have converted " + amount + " bones to " + (id == 8014 ? "bananas" : "peaches") + " ."));
	...
	if (dragonFireShieldCharges == 0)
		stoner.getClient().queueOutgoingPacket(new SendMessage("Your Dragonfire shield is now empty."));
	}
	...
	case OBELISK:
		...
		TaskQueue.queue(new Task(stoner, 1, false, Task.StackType.STACK, Task.BreakType.NEVER, TaskIdentifier.CURRENT_ACTION) {
			@Override
			public void execute() {
			stop();
			}
			...
			@Override
			public void onStop() {
			stoner.getClient().queueOutgoingPacket(new SendMessage("Ancient mage teleports you somewhere in the wilderness"));
			}
		});
		break;
	case TABLET:
	...
	TaskQueue.queue(new Task(stoner, delay, false, Task.StackType.STACK, Task.BreakType.NEVER, TaskIdentifier.CURRENT_ACTION) {
		@Override
		public void execute() {
	...
	TaskQueue.queue(new Task(stoner, delay, false, Task.StackType.STACK, Task.BreakType.NEVER, TaskIdentifier.CURRENT_ACTION) {
		...
		if (stoner.getBossPet() != null) {
			...
			final Mob mob = new Mob(stoner, stoner.getBossID(), false, false, true, stoner.getLocation());
	...
	if (dragonFireShieldCharges == 0) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("You do not have any charges on your shield."));
		return;
	}
	if ((!StonerConstants.isOwner(stoner)) && (System.currentTimeMillis() - dFireShieldTime < 300000L)) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("You must let your shield cool down before using it again."));
		return;
	}
	dFireShieldEffect = (!dFireShieldEffect);

	if (dFireShieldEffect) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("Your Dragonfire shield assault is now active."));
		...
		stoner.getCombat().getMage().setAssault(new Assault(5, 5), new Animation(6696), new Graphic(1167, 45, true), new Graphic(1167, 0, true), p);
	}
	...
	public enum Enchant {
		...
		private Enchant(int unenchanted, int enchanted, int gradeReq, int xpGiven, int anim, int gfx, int reqEnchantmentGrade) {
	...
	if (ens.getReq3() == 0) {
		return stoner.getBox().hasItemAmount(ens.getReq1(), ens.getReqAmt1()) && stoner.getBox().hasItemAmount(ens.getReq2(), ens.getReqAmt2()) && stoner.getBox().hasItemAmount(ens.getReq3(), ens.getReqAmt3());
	} else {
		return stoner.getBox().hasItemAmount(ens.getReq1(), ens.getReqAmt1()) && stoner.getBox().hasItemAmount(ens.getReq2(), ens.getReqAmt2());
	}
	}
	...
	if (stoner.getProfession().getGrades()[Professions.MAGE] >= enc.getGradeReq()) {
		if (stoner.getBox().hasItemAmount(enc.getUnenchanted(), 1)) {
			if (hasRunes(spellID)) {
				if (getEnchantmentGrade(spellID) == enc.getEGrade()) {
					...
					stoner.send(new SendMessage("You can only enchant this jewelry using a grade-" + enc.getEGrade() + " enchantment spell!"));
				}
			} else {
		...
		stoner.send(new SendMessage("You need a mage grade of at least " + enc.getGradeReq() + " to cast this spell."));
	}
	}
