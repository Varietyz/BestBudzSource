...
public class Autocast {
	...
	case ANCIENT:
		...
		if (!hasStaff) {
			...
			String def = GameDefinitionLoader.getItemDef(stoner.getEquipment().getItems()[3].getId()).getName().toLowerCase();

			stoner.send(new SendMessage("You can't autocast ancient spells with " + Utility.getAOrAn(def) + " " + def + "."));
			return;
		}
