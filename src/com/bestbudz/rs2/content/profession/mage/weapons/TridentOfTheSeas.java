...
public class TridentOfTheSeas {
  ...
  public static boolean itemOnItem(Stoner stoner, Item itemUsed, Item usedWith) {
    ...
    switch (with.getId()) {
      ...
      case 995:
        if (stoner
            .getBox()
            .hasAllItems(new Item(554, 5), new Item(560), new Item(562), new Item(995, 500))) {
          ...
          for (Item item : stoner.getBox().getItems()) {
            if (item != null) {
              switch (item.getId()) {
                ...
                case 562:
                case 995:
                  if (item.getAmount() / (item.getId() == 995 ? 500 : item.getId() == 554 ? 5 : 1)
                      < min) {
                    min =
                        item.getAmount()
                            / (item.getId() == 995 ? 500 : item.getId() == 554 ? 5 : 1);
                  }
                  break;
              }
            }
  ...
  public static void ask(Stoner stoner, int itemId) {
    ...
    String[][] info = {
      ...
      {"", "14177"},
      {"", "14182"},
      {"If you uncharge the trident, all runes will fall out.", "14183"},
  ...
  public static void degrade(Stoner stoner) {
    ...
    if (stoner.getSeasTrident().tridentCharge == 0) {
      stoner.send(
          new SendMessage(
              "The trident needs to be charged with 500 bestbucks, 1 death, 1 chaos, and 5 fire runes."));
