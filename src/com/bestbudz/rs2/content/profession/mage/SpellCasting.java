...
public class SpellCasting {
	...
	if (getCurrentSpellId() != -1) {
		stoner.getProfession().addExperience(6, GameDefinitionLoader.getCombatSpellDefinition(getCurrentSpellId()).getBaseExperience());
	}
	}

	public void appendMultiSpell(Stoner p) {
	if ((stoner.getCombat().getMage().isMulti()) && (p.inMultiArea()) && (p.getController().allowMultiSpells())) {
		byte affected = 0;
		...
		if (p.getController().allowPvPCombat()) {
			for (Stoner i : p.getStoners()) {
				if ((p.getController().canAssaultStoner(p, i)) && (i.inMultiArea()) && (!i.equals(a))) {
		...
		if (p.getController().canAssaultNPC())
	...
	if (!TridentOfTheSeas.hasTrident(stoner) || !TridentOfTheSwamp.hasTrident(stoner)) {
		if ((id <= 12445) && (stoner.getMage().getSpellBookType() != MageProfession.SpellBookTypes.MODERN))
			return false;
		if ((id > 12445) && (stoner.getMage().getSpellBookType() != MageProfession.SpellBookTypes.ANCIENT)) {
			return false;
		}
	} else if (definition.getId() == 9999) {
		if (stoner.getCombat().getAssaulting() != null && !stoner.getCombat().getAssaulting().isNpc()) {
	...
	// if (stoner.getMaxGrades()[6] < definition.getGrade()) {
	// stoner.getClient().queueOutgoingPacket(new SendMessage("You need a Mage
	// grade of " + definition.getGrade() + " to cast " + definition.getName() +
	// "."));
	// return false;
	// } else if (stoner.getProfession().getGrades()[6] < definition.getGrade()) {
	// stoner.getClient().queueOutgoingPacket(new SendMessage("You need a Mage
	// grade of " + definition.getGrade() + " to cast " + definition.getName() +
	// "."));
	// return false;
	...
	if ((wep != null) && (wep.length > 0)) {
		...
		if (weapon == null) {
			stoner.getClient().queueOutgoingPacket(new SendMessage("You need a " + Item.getDefinition(wep[0]).getName() + " to cast " + definition.getName() + "."));
			return false;
		}
	...
	// int runeId = stoner.getMage().hasRunes(spell.getRunes());
	...
	// if (stoner.getMaxGrades()[6] < spell.getGrade()) {
	// stoner.getClient().queueOutgoingPacket(new SendMessage("You need a Mage
	// grade of " + spell.getGrade() + " to cast " + spell.getName() + "."));
	// return;
	// }
	// if (runeId != -1) {
	// String name = Item.getDefinition(runeId).getName();
	// stoner.getClient().queueOutgoingPacket(new SendMessage("You do not have
	// enough " + name + "s to cast " + spell.getName() + "."));
	// return;
	// }

	if (spell.execute(stoner)) {
		// stoner.getMage().removeRunes(spell.getRunes());
	...
	if (castId == 12445 && other.isTeleblocked()) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("@dre@This stoner is already affected by this spell."));
		return;
	}
	...
	stoner.getCombat().getMage().setAssault(MAGE_ASSAULT, def.getAnimation(), def.getStart(), def.getEnd(), def.getProjectile());
