...
public class TabCreation {
	...
	public enum ItemData {
		...
		private ItemData(int tabID, int gradeRequired, Item... itemsRequired) {
		this.tabID = tabID;
	...
	if (stoner.getGrades()[Professions.MAGE] < data.getGrade()) {
		stoner.send(new SendMessage("<col=2555>You need a Mage grade of " + data.getGrade() + " to do this!"));
		return;
	}
	...
	for (int index = 0; index < data.getItems().length; index++) {
		if (!stoner.getBox().hasAllItems(data.getItems()[index]) || !stoner.getBox().hasItemAmount(1761, 1)) {
	...
	stoner.send(new SendMessage("<col=2555>You have successfully created " + Utility.getAOrAn(name) + " " + name + "."));
