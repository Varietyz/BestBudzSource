...
/**
 * Manages the functionality of parts of the hunter profession
 * 
...
public class Impling {
	...
	public enum ImplingRewards {
		 ...
		 * 
		 * @param stoner
		 *                   the stoner opening the jar
		 * @param itemId
		 *                   the itemId of loot recieved
		 */
		public static void lootImpling(Stoner stoner, int itemId) {
		if (!stoner.getBox().hasSpaceFor(new Item(itemId))) {
			stoner.getClient().queueOutgoingPacket(new SendMessage("You don't have enough room to loot the impling."));
			return;
		}
		ImplingRewards t = ImplingRewards.impReward.get(itemId);
		stoner.getBox().remove(new Item(itemId));
		stoner.getBox().getItemSlot(itemId);
		...
		if (t.getLoot()[r].length == 3) {
			...
			stoner.getBox().add(new Item(t.getLoot()[r][0], amount));
		} else {
			stoner.getBox().add(new Item(t.getLoot()[r][0], t.getLoot()[r][1]));
		}
		if (Utility.randomNumber(15) == 0) {
			stoner.getClient().queueOutgoingPacket(new SendMessage("The jar breaks as you open it."));
		} else {
			stoner.getBox().add(new Item(11260));
			stoner.getClient().queueOutgoingPacket(new SendMessage("You loot the impling jar."));
		}
		}
		...
		public enum Implings {
			...
			BABY_IMPLING(1028, 11238),
			YOUNG_IMPLING(1029, 11240),
			GOURMET_IMPLING(1030, 11242),
			EARTH_IMPLING(1031, 11244),
			ESSENCE_IMPLING(1032, 11246),
			ECLECTIC_IMPLING(1033, 11248),
			NATURE_IMPLING(1034, 11250),
			MAGPIE_IMPLING(1035, 11252),
			NINJA_IMPLING(6063, 11254),
			DRAGON_IMPLING(6064, 11256);
			...
			Implings(final int npc, final int itemId) {
			this.npc = npc;
			this.itemId = itemId;
			}
			 ...
			 * 
			 * @param stoner
			 *                    the stoner catching the impling
			 * @param impling
			 *                    the impling mob being caught
			 */
			public static void catchImp(Stoner stoner, Mob impling) {
			if (stoner.getEquipment().getItems()[3] == null) {
				return;
			}
			Implings t = Implings.implings.get(impling.getId());

			if (!stoner.getBox().hasSpaceFor(new Item(t.getJar()))) {
				stoner.getClient().queueOutgoingPacket(new SendMessage("You don't have enough room to do this."));
				return;
			}

			if (stoner.getEquipment().getItems()[3].getId() != 10010) {
				stoner.getClient().queueOutgoingPacket(new SendMessage("You need a butterfly net to catch this impling."));
				return;
			}

			if (!stoner.getBox().hasItemId(new Item(11260))) {
				stoner.getClient().queueOutgoingPacket(new SendMessage("You need an impling jar to catch and store the impling."));
			}
			if (Utility.randomNumber(25) == 0) {
				stoner.getUpdateFlags().sendAnimation(new Animation(6999));
				stoner.getClient().queueOutgoingPacket(new SendMessage("You fail to catch the impling."));
			} else {
				stoner.getUpdateFlags().sendAnimation(new Animation(6999));
				stoner.getClient().queueOutgoingPacket(new SendMessage("You catch the impling and place it in the jar."));
				stoner.getBox().add(t.getJar(), 1);
				stoner.getBox().remove(11260, 1);
