...
public class ForgingTask extends Task {
	...
	TaskQueue.queue(new ForgingTask(p, new Item(item, make), new Item(bar, ForgingConstants.getBarAmount(interfaceId, slot)), amount));
	}
	...
	super(stoner, 2, true, Task.StackType.NEVER_STACK, Task.BreakType.ON_MOVE, TaskIdentifier.CURRENT_ACTION);
	this.stoner = stoner;
	this.forger = forger;
	this.bar = bar;
	this.amount = amount;
	...
	if (stoner.getMaxGrades()[13] < lvl) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("You need a Forging grade of " + lvl + " to make that."));
		stop();
	} else if (!stoner.getBox().hasItemAmount(new Item(bar))) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("You do not have enough bars to make that."));
		stop();
	...
	if (forger.getAmount() == 1)
		stoner.getClient().queueOutgoingPacket(new SendMessage("You make " + Utility.getAOrAn(forger.getDefinition().getName()) + " " + forger.getDefinition().getName() + "."));
	else {
		stoner.getClient().queueOutgoingPacket(new SendMessage("You make " + forger.getAmount() + " " + forger.getDefinition().getName() + (!forger.getDefinition().getName().endsWith("s") ? "s" : "") + "."));
	}
	...
	if (!stoner.getBox().hasItemAmount(new Item(bar))) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("You have run out of " + bar.getDefinition().getName() + "s."));
		return false;
	}
