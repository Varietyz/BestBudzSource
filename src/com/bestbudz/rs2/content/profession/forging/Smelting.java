...
public class Smelting extends Task {
	...
	if (stoner.getMaxGrades()[13] < data.getGradeRequired()) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("You need a Forging grade of " + data.getGradeRequired() + " to smelt this bar."));
		return false;
	}

	for (Item i : data.getRequiredOres()) {
		if (!stoner.getBox().hasItemAmount(i.getId(), i.getAmount())) {
			stoner.getClient().queueOutgoingPacket(new SendMessage(taskRunning ? "You have run out of " + i.getDefinition().getName() + "." : "You don't not have any " + i.getDefinition().getName().toLowerCase() + " to smelt."));
			return false;
		}
	}
	...
	if (data == SmeltingData.IRON_BAR) {
		if (Professions.isSuccess(stoner, 13, data.getGradeRequired())) {
			...
			stoner.getClient().queueOutgoingPacket(new SendMessage("You smelt " + Utility.getAOrAn(name) + " " + name + "."));
		} else {
