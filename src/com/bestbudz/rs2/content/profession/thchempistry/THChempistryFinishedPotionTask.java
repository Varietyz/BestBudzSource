...
public class THChempistryFinishedPotionTask extends Task {
	...
	if (data == null) {
		data = FinishedPotionData.forId(((Item) stoner.getAttributes().get("thchempistryitem2")).getId());
	}
	if (data == null) {
	...
	stoner.getClient().queueOutgoingPacket(new SendItemOnInterface(1746, 170, finishedPotion.getId()));
	...
	if (amount != 100)
		attemptPotionMaking(stoner, amount);
	else {
		stoner.getClient().queueOutgoingPacket(new SendEnterXInterface(4430, ((Item) stoner.getAttributes().get("thchempistryitem1")).getId()));
	}
	return true;
	}

	private static boolean meetsRequirements(Stoner stoner, FinishedPotionData data, boolean running) {
	if (stoner.getProfession().getGrades()[15] < data.getGradeReq()) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("You need an thc-hempistry grade of " + data.getGradeReq() + " to make this potion."));
		return false;
	}
	...
	if ((!stoner.getBox().hasItemId(data.getUnfinishedPotion())) || (!stoner.getBox().hasItemId(data.getItemNeeded()))) {
		stoner.getClient().queueOutgoingPacket(new SendMessage(!running ? "You don't have the ingredients required to make this potion." : "You have run out of ingredients to make this potion."));
		return false;
	}
	return true;
	}
