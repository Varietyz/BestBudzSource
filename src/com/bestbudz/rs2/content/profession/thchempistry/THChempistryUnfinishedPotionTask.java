...
public class THChempistryUnfinishedPotionTask extends Task {
	...
	public static boolean attemptToCreateUnfinishedPotion(Stoner stoner, int amount, Item used, Item usedWith) {
	Item[] items = { (Item) stoner.getAttributes().get("thchempistryitem1"), (Item) stoner.getAttributes().get("thchempistryitem2") };
	...
	TaskQueue.queue(new THChempistryUnfinishedPotionTask(stoner, data, new Item(items[0]), new Item(items[1]), amount));
	...
	stoner.getClient().queueOutgoingPacket(new SendItemOnInterface(1746, 170, unfinishedPotion.getId()));
	...
	if (amount != 100)
		attemptToCreateUnfinishedPotion(stoner, amount, (Item) stoner.getAttributes().get("thchempistryitem1"), (Item) stoner.getAttributes().get("thchempistryitem2"));
	else {
		stoner.getClient().queueOutgoingPacket(new SendEnterXInterface(4429, ((Item) stoner.getAttributes().get("thchempistryitem1")).getId()));
	}
	return true;
	}

	private static boolean meetsRequirements(Stoner stoner, UnfinishedPotionData data, Item used, Item used2) {
	if (stoner.getProfession().getGrades()[15] < data.getGradeReq()) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("You need an thc-hempistry grade of " + data.getGradeReq() + " to make this."));
		stoner.getClient().queueOutgoingPacket(new SendRemoveInterfaces());
		return false;
	}

	if ((!stoner.getBox().hasItemId(used.getId())) || (!stoner.getBox().hasItemId(used2.getId()))) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("You don't have the ingredients required to make this potion."));
		return false;
	}
	...
	public THChempistryUnfinishedPotionTask(Stoner stoner, UnfinishedPotionData data, Item used, Item usedWith, int amountToMake) {
	...
	stoner.getClient().queueOutgoingPacket(new SendMessage("You put the " + weed.getDefinition().getName() + " in the vial of water."));
	}
