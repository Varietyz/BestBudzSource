...
public class LootUrnTask extends Task {
	...
	super(stoner, 1, false, StackType.NEVER_STACK, BreakType.ON_MOVE, TaskIdentifier.CURRENT_ACTION);
	this.urn = urn;
	...
	case 2:
		...
		getEntity().getStoner().getAttributes().set(PlunderConstants.URNS_CONFIG_KEY, getEntity().getStoner().getAttributes().getInt(PlunderConstants.URNS_CONFIG_KEY) | urn.getConfig(0));
		break;
	case 3:
		stop();
		break;
	...
	@Override
	public void onStop() {
	getEntity().getStoner().send(new SendConfig(PlunderConstants.URNS_CONFIG, getEntity().getStoner().getAttributes().getInt(PlunderConstants.URNS_CONFIG_KEY)));
	}
}