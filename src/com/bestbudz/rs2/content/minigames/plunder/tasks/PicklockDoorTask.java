...
public class PicklockDoorTask extends Task {
	...
	switch (ticks++) {
	case 1:
		getEntity().getStoner().getAttributes().set(PlunderConstants.DOORS_CHEST_SARCOPHAGUS_CONFIG_KEY, getEntity().getStoner().getAttributes().getInt(PlunderConstants.DOORS_CHEST_SARCOPHAGUS_CONFIG_KEY) | door.getConfig());
		break;

	case 2:
		getEntity().getStoner().send(new SendConfig(PlunderConstants.DOORS_CHEST_SARCOPHAGUS_CONFIG, getEntity().getStoner().getAttributes().getInt(PlunderConstants.DOORS_CHEST_SARCOPHAGUS_CONFIG_KEY)));
		getEntity().getStoner().send(new SendAnimateObject(doorObject, PlunderConstants.DOOR_ANIMATION));
		break;
