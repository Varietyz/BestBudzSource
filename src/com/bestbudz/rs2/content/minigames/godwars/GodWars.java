    ...
    switch (id) {
      ...
      case 26363:
        if (stoner.getCombat().inCombat()) {
          stoner
              .getClient()
              .queueOutgoingPacket(new SendMessage("You cannot use this while in combat!"));
          return true;
        }

        if ((stoner.getAttributes().get(GWD_ALTAR_KEY) == null)
            || (((Long) stoner.getAttributes().get(GWD_ALTAR_KEY)).longValue()
                < System.currentTimeMillis())) {
          stoner
              .getAttributes()
              .set(GWD_ALTAR_KEY, Long.valueOf(System.currentTimeMillis() + 600_000));
