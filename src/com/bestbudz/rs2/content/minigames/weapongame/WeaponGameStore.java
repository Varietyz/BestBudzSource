...
public class WeaponGameStore {
  ...
  private static final int CONTAINER_ID = 56503;

  private static final int INTERFACE_ID = 56500;

  private static final int SELECTED_CONTAINER_ID = 56508;
  ...
  public static void open(Stoner stoner) {
    ...
    stoner.send(
        new SendString(
            "</col>You currently have @gre@"
                + Utility.format(stoner.getWeaponPoints())
                + "</col> points.",
            56505));
  ...
  public static void purchase(Stoner stoner, int item) {
    ...
    if (stoner.getWeaponPoints() < data.getPrice()) {
      stoner.send(
          new SendMessage(
              "@red@You need "
                  + Utility.format(data.getPrice())
                  + " Weapon Game Points to buy this!"));
      return;
    }
    ...
    stoner.send(
        new SendMessage(
            "@red@Congratulations! You have purchased "
  ...
  public enum StoreData {
    ...
    private static final HashMap<Integer, StoreData> items = new HashMap<Integer, StoreData>();
    ...
    StoreData(int item, int amount, int price) {
      this.item = item;
      this.amount = amount;
      this.price = price;
    }
