...
public class WeaponGame {
	...
	if (stoner.getProfession().getCombatGrade() < WeaponGameConstants.COMBAT_REQUIRED) {
		DialogueManager.sendStatement(stoner, "Combat grade of " + WeaponGameConstants.COMBAT_REQUIRED + " is required!");
		return;
	}
	...
	if (!stoner.getController().equals(ControllerManager.WEAPON_LOBBY_CONTROLLER)) {
		...
		stoner.send(new SendMessage("@dre@There are currently " + lobbyStoners.size() + " stoners in the lobby."));
	}
	}

	/**
	 * Handles stoner leaving lobby
	 * 
	 * @param stoner
	 * @param portal
	 */
	...
	for (Stoner stoners : lobbyStoners) {
		...
		DialogueManager.sendInformationBox(stoners, "@dre@Weapon Game", "@dre@Objective: @bla@Be the first to reach 10 kills", "@dre@Note: @bla@Each kill will upgrade your weapon", "@dre@Hint: @bla@Click on scattered crates for supplies ", "Good luck!");
	...
	for (int i = 0; i < Utility.random(10); i++) {
		...
		for (Stoner stoners : gameStoners) {
			TaskQueue.queue(new Task(stoners, 3, false) {
				@Override
				public void execute() {
				stop();
				}
	...
	case 3:
		...
		stoner.send(new SendMessage("@dre@...You have found some " + weapon.getDefinition().getName() + " inside the chest!"));
		break;

	case 4:
		stoner.hit(new Hit(Utility.random(10)));
		stoner.send(new SendMessage("@dre@...While searching you cut your hand on a sharp object!"));
		ObjectManager.remove(object);
		crates.remove(object);
		break;
	...
	case 5:
		...
		stoner.send(new SendMessage("@dre@...While searching you feel a mysterious force move you!"));
		ObjectManager.remove(object);
		crates.remove(object);
		break;
	}
	...
	if (force) {
		...
		World.sendGlobalMessage("[ <col=0079AD>Weapon Game</col> ] Game has been won by <col=0079AD>" + leader.deterquarryIcon(leader) + " " + leader.getUsername() + " </col>!");
		for (Stoner stoners : gameStoners) {
	...
	/**
	 * Gets amount of stoners in lobby
	 * 
	 * @return
	 */
	...
	/**
	 * Gets amount of stoners in game
	 * 
	 * @return
	 */
	...
	/**
	 * Sends messages to stoners in game or lobby
	 * 
	 * @param message
	 * @param lobby
	 * @return
	...
	if (lobbyCount() > 0) {
		...
		if (WeaponGameConstants.LOBBY_TIME == 0 || lobbyCount() == WeaponGameConstants.MAXIMUM_STONERS) {
			startGame();
	...
	if (started) {
		...
		if (WeaponGameConstants.GAME_TIME == 0 || gameStoners.size() == 1 || gameStoners.size() == 0) {
			endGame(true);
			return;
		}
