...
public class WeaponLobbyController extends Controller {
	...
	TaskQueue.queue(new Task(stoner, 1) {
		@Override
		public void execute() {
		DialogueManager.sendStatement(stoner, "Welcome to the @dre@Weapon Game Lobby@bla@!", "Game will start when there are a minimum of @dre@5@bla@ stoners.", "Click on the @dre@portal@bla@ to @dre@exit@bla@.");
		stop();
		}
	...
	stoner.send(new SendString("(Need " + WeaponGameConstants.MINIMUM_STONERS + " to " + WeaponGameConstants.MAXIMUM_STONERS + ")", 41253));
	stoner.send(new SendString("Next Departure: " + Utility.getFormattedTime(WeaponGameConstants.LOBBY_TIME), 41254));
	}
