...
public class WeaponGameController extends GenericMinigameController {
	...
	if (paramStoner.getWeaponKills() == 0) {
		...
		paramStoner.send(new SendUpdateItemsAlt(41283, WeaponGameConstants.TIER_DATA[paramStoner.getWeaponKills()][0].getId(), 1, 0));
	}
	if (paramStoner.getWeaponKills() != 9) {
		paramStoner.send(new SendUpdateItemsAlt(41283, WeaponGameConstants.TIER_DATA[paramStoner.getWeaponKills() + 1][0].getId(), 1, 1));
	} else {
