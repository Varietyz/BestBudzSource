...
public final class TzharrGame {
	...
	if (reward) {
		...
		stoner.getClient().queueOutgoingPacket(new SendMessage("@dre@Congratulations, you have completed The Fight Caves"));
		DialogueManager.sendStatement(stoner, "Congratulations, you have completed The Fight Caves");
		World.sendGlobalMessage("<img=8> <col=C42BAD>" + stoner.getUsername() + " has just completed the Fight Caves minigame!");
		...
		stoner.getClient().queueOutgoingPacket(new SendMessage("@dre@You did not make it far enough to receive a reward."));
	}
	...
	if (stoner.getJadDetails().getStage() != 0)
