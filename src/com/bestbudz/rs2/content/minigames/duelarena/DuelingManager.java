...
public class DuelingManager {
	...
	public static Location getCoordinates(int arenaId, boolean obstacles, boolean secondStoner, boolean noMovement) {
	if (noMovement) {
	...
	Location p = NON_OBSTACLE_ARENA_LOCATIONS[arenaId];
	return new Location(p.getX() + (5 + Utility.randomNumber(5)) * (secondStoner ? 1 : -1), p.getY() + Utility.randomNumber(5) * (Utility.randomNumber(1) == 0 ? 1 : -1));
	}
