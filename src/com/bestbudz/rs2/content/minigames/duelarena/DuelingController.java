...
public class DuelingController extends Controller {
	...
	case MAGE:
		if (p.getDueling().getRuleToggle()[4]) {
			p.getClient().queueOutgoingPacket(new SendMessage("You can't use Mage during this duel!"));
			return false;
		}
		break;
	case MELEE:
	...
	case SAGITTARIUS:
		if (p.getDueling().getRuleToggle()[2]) {
			p.getClient().queueOutgoingPacket(new SendMessage("You can't use Sagittarius during this duel!"));
			return false;
		}
		break;
	case NONE:
	...
	if (p.getDueling().getRuleToggle()[7]) {
		p.getClient().queueOutgoingPacket(new SendMessage("You cannot use necromance during this duel!"));
		return false;
	}
	return true;
	}
	...
	if (p.getDueling().getRuleToggle()[10]) {
		p.getClient().queueOutgoingPacket(new SendMessage("You cannot use special assaults during this duel!"));
		return false;
	}
	return true;
	}
	...
	Location p = DuelingConstants.RESPAWN_LOCATIONS[com.bestbudz.core.util.Utility.randomNumber(DuelingConstants.RESPAWN_LOCATIONS.length)];
	int[] dir = com.bestbudz.rs2.GameConstants.DIR[com.bestbudz.core.util.Utility.randomNumber(com.bestbudz.rs2.GameConstants.DIR.length)];
