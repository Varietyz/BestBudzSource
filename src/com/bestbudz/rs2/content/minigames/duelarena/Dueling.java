...
public class Dueling {
	...
	p1.getClient().queueOutgoingPacket(new SendString("Dueling with: " + p2.getUsername() + "                                  Combat Grade: @yel@" + p2.getProfession().getCombatGrade(), 6671));
	p2.getClient().queueOutgoingPacket(new SendString("Dueling with: " + p1.getUsername() + "                                  Combat Grade: @yel@" + p1.getProfession().getCombatGrade(), 6671));
	...
	case SCREEN_1:
		if ((canAccept()) && (interacting != null)) {
			if (interacting.getDueling().hasAccepted()) {
				...
				interacting.getClient().queueOutgoingPacket(new SendString("Other stoner has accepted.", 6684));
			}
		}
	...
	case SCREEN_2:
		if (interacting != null) {
			if (interacting.getDueling().hasAccepted()) {
				...
				interacting.getClient().queueOutgoingPacket(new SendString("Other stoner has accepted.", 6571));
			}
		}
		break;
	default:
	...
	if (!p.getBox().hasSpaceFor(buildItemArray(p, interacting, toRemove))) {
		p.getClient().queueOutgoingPacket(new SendMessage("You do not have enough box space to accept this duel."));
		return false;
	}
	...
	if (interacting != null) {
		interacting.getClient().queueOutgoingPacket(new SendMessage("The other stoner declined the duel."));
	...
	if (won != null) {
		...
		for (int item : recieved.keySet()) {
			...
			StonerLogger.STAKE_LOGGER.log(p.getUsername(), String.format("%s has won %s %s from %s.", Utility.formatStonerName(p.getUsername()), reward.getAmount(), reward.getDefinition().getName(), Utility.formatStonerName(interacting.getUsername())));
		}
	}
	...
	p.getClient().queueOutgoingPacket(new SendString("" + interacting.getProfession().getCombatGrade(), 6839));
