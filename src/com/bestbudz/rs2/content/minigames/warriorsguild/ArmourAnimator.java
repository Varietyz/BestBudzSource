...
public final class ArmourAnimator {
	 ...
	 * @return
	 */
	public static boolean armorOnAnimator(Stoner stoner, int itemId, GameObject object, int x, int y) {
	if (object.getId() != 23955) {
		return false;
	}

	if (stoner.getAttributes().get("warriorGuildAnimator") != null) {
		return true;
	}
	...
	stoner.getAttributes().set("warriorGuildAnimator", Integer.valueOf(animatorIndex));

	int armorIndex = hasArmor(stoner, itemId);

	if (armorIndex != -1) {
		createAnimatedArmor(stoner, armorIndex);
	}
	 ...
	 * @param index
	 */
	protected static void createAnimatedArmor(final Stoner stoner, final int index) {
	if (!stoner.getAttributes().isSet("warriorGuildAnimator")) {
		return;
	}
	...
	for (int i = 0; i < ARMOR_SETS[index].length; i++) {
		stoner.getBox().remove(ARMOR_SETS[index][i]);
	}
	final int animatorIndex = ((Integer) stoner.getAttributes().get("warriorGuildAnimator")).intValue();
	TaskQueue.queue(new Task(2) {
		...
		if (i == 0) {
			setTaskDelay(1);
			stoner.send(new SendMessage("You place the armour pieces inside the animator..."));
		} else if (i == 1) {
			stoner.send(new SendMessage("The animator begins to hum, something appears to be working..."));

			int x = stoner.getLocation().getX();
			int y = stoner.getLocation().getY();

			stoner.getMovementHandler().addToPath(new Location(x, y + 3));

			stoner.getMovementHandler().finish();
			...
			setTaskDelay(1);
			mob = new Mob(stoner, ANIMATED_ARMOR[index], false, false, false, new Location(ANIMATOR_LOCATIONS[animatorIndex][0], ANIMATOR_LOCATIONS[animatorIndex][1], 0));
		} else if (i == 3) {
			stoner.send(new SendMessage("The animated armour comes to life!"));
			mob.getUpdateFlags().sendForceMessage("I'M ALIVE!");

			mob.getCombat().setAssault(stoner);
	 ...
	 * @param mob
	 */
	public static void dropForAnimatedArmour(Stoner stoner, Mob mob) {
	int index = -1;
	...
	/**
	 * Checks if stoner has armour
	 * 
	 * @param stoner
	 * @param itemId
	 * @return
	 */
	protected static int hasArmor(Stoner stoner, int itemId) {
	int itemIndex = -1;
		...
		for (int j = 0; j < ARMOR_SETS[i].length; j++) {
			if (itemId == ARMOR_SETS[i][j]) {
				...
				for (int k = 0; k < ARMOR_SETS[i].length; k++) {
					if (!stoner.getBox().hasItemId(ARMOR_SETS[i][k])) {
						stoner.send(new SendMessage("You need a complete armour set to do this!"));
						return -1;
					}
				}
				break;
