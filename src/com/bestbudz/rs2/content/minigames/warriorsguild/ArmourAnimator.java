package com.bestbudz.rs2.content.minigames.warriorsguild;

import com.bestbudz.core.task.Task;
import com.bestbudz.core.task.TaskQueue;
import com.bestbudz.rs2.entity.Animation;
import com.bestbudz.rs2.entity.Location;
import com.bestbudz.rs2.entity.mob.Mob;
import com.bestbudz.rs2.entity.object.GameObject;
import com.bestbudz.rs2.entity.stoner.Stoner;
import com.bestbudz.rs2.entity.stoner.net.out.impl.SendMessage;

public final class ArmourAnimator {

  public static final int[][] ANIMATOR_LOCATIONS = {{2851, 3536}, {2857, 3536}};

  public static final int[][] ARMOR_SETS = {
    {1155, 1117, 1075},
    {1153, 1115, 1067},
    {1157, 1119, 1069},
    {1165, 1125, 1077},
    {1159, 1121, 1071},
    {1161, 1123, 1073},
    {1163, 1127, 1079}
  };

  public static final int[] TOKENS = {5, 10, 15, 20, 25, 30, 40};

  public static final int[] ANIMATED_ARMOR = {2450, 2451, 2452, 2453, 2454, 2455, 2456};

  public static final String[] ARMOR_TYPE = {
    "Bronze", "Iron", "Steel", "Black", "Mithril", "Adamant", "Rune"
  };

  public static boolean isAnimatedArmour(int id) {
    for (int i : ANIMATED_ARMOR) {
      if (id == i) return true;
    }
    return false;
  }

  public static boolean armorOnAnimator(
      Stoner stoner, int itemId, GameObject object, int x, int y) {
    if (object.getId() != 23955) {
      return false;
    }

    if (stoner.getAttributes().get("warriorGuildAnimator") != null) {
      return true;
    }

    int objectX = object.getLocation().getX();
    int objectY = object.getLocation().getY();

    int animatorIndex = -1;

    for (int i = 0; i < ANIMATOR_LOCATIONS.length; i++) {
      if ((objectX == ANIMATOR_LOCATIONS[i][0]) && (objectY == ANIMATOR_LOCATIONS[i][1])) {
        animatorIndex = i;
        break;
      }
    }

    if (animatorIndex == -1) {
      return false;
    }

    stoner.getAttributes().set("warriorGuildAnimator", Integer.valueOf(animatorIndex));

    int armorIndex = hasArmor(stoner, itemId);

    if (armorIndex != -1) {
      createAnimatedArmor(stoner, armorIndex);
    }

    return true;
  }

  private static void createAnimatedArmor(final Stoner stoner, final int index) {
    if (!stoner.getAttributes().isSet("warriorGuildAnimator")) {
      return;
    }
    stoner.getUpdateFlags().sendAnimation(new Animation(827));
    stoner.getAttributes().set("stopMovement", Boolean.valueOf(true));
    for (int i = 0; i < ARMOR_SETS[index].length; i++) {
      stoner.getBox().remove(ARMOR_SETS[index][i]);
    }
    final int animatorIndex =
        ((Integer) stoner.getAttributes().get("warriorGuildAnimator")).intValue();
    TaskQueue.queue(
        new Task(2) {
          int i = 0;
          Mob mob = null;

          @Override
          public void execute() {
            if (i == 0) {
              setTaskDelay(1);
              stoner.send(new SendMessage("You place the armour pieces inside the animator..."));
            } else if (i == 1) {
              stoner.send(
                  new SendMessage(
                      "The animator begins to hum, something appears to be working..."));

              int x = stoner.getLocation().getX();
              int y = stoner.getLocation().getY();

              stoner.getMovementHandler().addToPath(new Location(x, y + 3));

              stoner.getMovementHandler().finish();

              stoner.getUpdateFlags().sendFaceToDirection(x, y);

              setTaskDelay(3);

            } else if (i == 2) {
              setTaskDelay(1);
              mob =
                  new Mob(
                      stoner,
                      ANIMATED_ARMOR[index],
                      false,
                      false,
                      false,
                      new Location(
                          ANIMATOR_LOCATIONS[animatorIndex][0],
                          ANIMATOR_LOCATIONS[animatorIndex][1],
                          0));
            } else if (i == 3) {
              stoner.send(new SendMessage("The animated armour comes to life!"));
              mob.getUpdateFlags().sendForceMessage("I'M ALIVE!");

              mob.getCombat().setAssault(stoner);
              mob.getFollowing().setIgnoreDistance(true);

              stop();
            }
            i += 1;
          }

          @Override
          public void onStop() {}
        });
  }

  public static void dropForAnimatedArmour(Stoner stoner, Mob mob) {
    int index = -1;

    for (int i = 0; i < ANIMATED_ARMOR.length; i++) {
      if (mob.getId() == ANIMATED_ARMOR[i]) {
        index = i;
        break;
      }
    }

    if (index == -1) {}
  }

  private static int hasArmor(Stoner stoner, int itemId) {
    int itemIndex = -1;
    for (int i = 0; i < ARMOR_SETS.length; i++) {
      for (int j = 0; j < ARMOR_SETS[i].length; j++) {
        if (itemId == ARMOR_SETS[i][j]) {
          itemIndex = i;
          for (int k = 0; k < ARMOR_SETS[i].length; k++) {
            if (!stoner.getBox().hasItemId(ARMOR_SETS[i][k])) {
              stoner.send(new SendMessage("You need a complete armour set to do this!"));
              return -1;
            }
          }
          break;
        }
      }
    }
    return itemIndex;
  }
}
