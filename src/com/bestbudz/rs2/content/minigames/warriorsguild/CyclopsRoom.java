...
public final class CyclopsRoom {
	...
	if (stoner.getX() == 2847) {
		...
		if (stoner.getBox().getItemAmount(8851) < 100) {
			DialogueManager.sendStatement(stoner, "100 tokens are required to enter! You have " + stoner.getBox().getItemAmount(8851) + ".");
			return true;
		}
		stoner.getBox().remove(8851, 100);
		stoner.getClient().queueOutgoingPacket(new SendMessage("You pay 100 tokens to enter the cyclops arena."));
		executeTimer(stoner);
		int defender = DEFENDERS[getDefenderIndex(stoner)];
		stoner.getAttributes().set("cyclopsdefenderdrop", Integer.valueOf(defender));
		stoner.getClient().queueOutgoingPacket(new SendMessage("@dre@The cyclops are now dropping: " + Item.getDefinition(defender).getName() + "."));
	...
	stoner.getAttributes().set("cyclopsdefenderdrop", Integer.valueOf(DEFENDERS[getDefenderIndex(stoner)]));
	stoner.getClient().queueOutgoingPacket(new SendMessage("@dre@The cyclops are now dropping: " + Item.getDefinition(DEFENDERS[getDefenderIndex(stoner)]).getName() + "."));
	updateInterface(stoner);
	}
	...
	stoner.send(new SendString("</col>Tokens Used: @red@ " + stoner.getAttributes().getInt("warrguildtokensused"), 51205));
	stoner.send(new SendString("</col>Cyclops Killed: @red@ " + (stoner.getAttributes().getInt("CYCLOPS_KILLED") == -1 ? 0 : stoner.getAttributes().getInt("CYCLOPS_KILLED")), 51206));
	...
	for (int i = 0; i < DEFENDERS.length; i++) {
		if (stoner.getBank().hasItemId(DEFENDERS[i]) || stoner.getBox().hasItemId(DEFENDERS[i]) || (shield != null && shield.getId() == DEFENDERS[i])) {
			currentDefender = i;
		}
	}
