...
public class FightPits {
  ...
  private static final List<Stoner> inGame = new LinkedList<Stoner>();
  ...
  public static final boolean clickObject(Stoner p, int id) {
    switch (id) {
      case 9369:
      ...
      case 9368:
        if (!p.getController().equals(ControllerManager.FIGHT_PITS_CONTROLLER)) {
          p.getClient()
              .queueOutgoingPacket(
                  new SendMessage(
                      "The game has not started yet. You will moved inside once it does."));
        } else {
          inGame.remove(p);
          waiting.add(p);
  ...
  public static final void endGame(boolean failure) {
    ...
    if (failure) {
      for (Iterator<Stoner> i = inGame.iterator(); i.hasNext(); ) {
        Stoner p = i.next();
  ...
  public static final void onStonerDeath(Stoner p) {
    inGame.remove(p);
    waiting.add(p);
    p.setController(ControllerManager.FIGHT_PITS_WAITING_CONTROLLER);
  }

  public static void removeFromGame(Stoner p) {
    inGame.remove(p);
    p.teleport(FightPitsConstants.OUT_OF_WAITING_LOCATION);
    p.setController(ControllerManager.DEFAULT_CONTROLLER);
  }

  public static final void removeFromWaitingRoom(Stoner p) {
    waiting.remove(p);
    p.teleport(FightPitsConstants.OUT_OF_WAITING_LOCATION);
    p.setController(ControllerManager.DEFAULT_CONTROLLER);
  ...
  public static final void sendStartFailure() {
    ...
    for (Iterator<Stoner> i = waiting.iterator(); i.hasNext(); ) {
      Stoner p = i.next();
      ...
      if (!p.isActive()) {
        ...
        p.getClient()
            .queueOutgoingPacket(
                new SendMessage("There are not enough stoners to start Fight Pits!"));
      }
    }
  }
  ...
  public static final void tick() {
    ...
    if ((startTime > 0) && (endTime == 0)) startTime -= 1;
    else if ((startTime == 0) && (endTime == 0)) {
      if (waiting.size() + inGame.size() > 1) startGame();
      else {
        sendStartFailure();
      }
    }
  ...
  public static final void updateInterface(Stoner p) {
    ...
    if (endTime == 0) {
      p.getClient()
          .queueOutgoingPacket(
              new SendString(
                  "Time until next game: " + startTime / 2,
                  FightPitsConstants.FIGHT_PITS_INTERFACE_STRINGS[1]));
      p.getClient()
          .queueOutgoingPacket(
              new SendString(
                  "Stoners waiting: " + (waiting.size() + inGame.size()),
                  FightPitsConstants.FIGHT_PITS_INTERFACE_STRINGS[2]));
    } else {
      p.getClient()
          .queueOutgoingPacket(
              new SendString(
                  "Time until game ends: " + endTime / 2,
                  FightPitsConstants.FIGHT_PITS_INTERFACE_STRINGS[1]));
      p.getClient()
          .queueOutgoingPacket(
              new SendString(
                  "Foes Remaining: " + inGame.size(),
                  FightPitsConstants.FIGHT_PITS_INTERFACE_STRINGS[2]));
    }
  }

  public static final void updateStonerForGame(Stoner p) {
    if (p.isPoisoned()) {
      p.curePoison(0);
    }
  }
