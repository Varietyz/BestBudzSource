...
public abstract class Pest extends Mob {
	...
	for (int i : DEFILERS) {
		if (id == i) {
			getCombat().getSagittarius().setAssault(new Assault(3, 5), new Animation(3920), null, null, null);
	...
	if (!(this instanceof Spinner || this instanceof Shifter)) {
		if (!isMovedLastCycle() && getCombat().getAssaultTimer() == 0 && ++offset >= 4) {
			...
			for (Stoner k : game.getStoners()) {
				...
				if (thisDist <= 8) {
					if (p == null && game.getAssaulters(k) < 2 || thisDist < dist && game.getAssaulters(k) < 2) {
