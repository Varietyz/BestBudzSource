...
public class PestWaitingRoomController extends Controller {
  ...
  public void tick(Stoner p) {
    p.getClient()
        .queueOutgoingPacket(
            new SendString(
                "Next Departure: " + Utility.getFormattedTime(PestControl.getMinutesTillDepart()),
                21120));
    p.getClient()
        .queueOutgoingPacket(
            new SendString("Stoners Ready: " + PestControl.getStonersReady(), 21121));
    p.getClient().queueOutgoingPacket(new SendString("(Need 2 to 25 stoners)", 21122));
    p.getClient()
        .queueOutgoingPacket(new SendString("Points: " + Utility.format(p.getPestPoints()), 21123));
  }
