...
public class PestControlGame {
	...
	if (success) {
		World.sendGlobalMessage("<img=8> <col=C42BAD>" + stoners.size() + " stoners have completed a Pest Control minigame.");
	} else {
		World.sendGlobalMessage("<img=8> <col=C42BAD>" + stoners.size() + " stoners have failed a Pest Control minigame.");
	}
	...
	for (Stoner p : stoners) {
		...
		if (success) {
			if (p.getAttributes().get(PEST_DAMAGE_KEY) != null && p.getAttributes().getInt(PEST_DAMAGE_KEY) >= 80) {
				DialogueManager.sendNpcChat(p, 1756, Emotion.HAPPY_TALK, "You have managed to protect our weed!", "You got some points and Khalifa Kush,", "get smoking fam.");
				p.getBox().addOrCreateGroundItem(269, p.getAttributes().getInt(PEST_DAMAGE_KEY) * 6, true);
				p.setPestPoints(p.getPestPoints() + (10));
			} else {
				DialogueManager.sendNpcChat(p, 1756, Emotion.CALM, "Fellow stoners protected the weed,", " but u didnt seem to care much.");
			}
		} else {
			DialogueManager.sendNpcChat(p, 1756, Emotion.ANGRY_3, "SHIT! FUCK! STUPID!", "THEY TOOK OUR WEED!!");
		}
	...
	for (Stoner p : stoners) {
		...
		for (int i = 0; i < 4; i++) {
			...
			p.getClient().queueOutgoingPacket(new SendString((dead ? "@red@Dead" : "" + portals[i].getGrades()[Professions.LIFE]), 21111 + i));
		}
