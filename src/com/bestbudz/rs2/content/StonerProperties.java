...
public class StonerProperties {
	...
	stoner.send(new SendMessage("Your " + mob.getDefinition().getName() + " kill count is: @red@" + Utility.format(stoner.getProperties().getPropertyValue("MOB_" + mob.getDefinition().getName())) + "</col>."));
	}
	...
	for (Object attribute : stoner.getAttributes().getAttributes().keySet()) {
		if (String.valueOf(attribute).startsWith(ATTRIBUTE_KEY + property + "_")) {
			properties.put(String.valueOf(attribute).replace(ATTRIBUTE_KEY + property + "_", ""), stoner.getAttributes().getInt(attribute));
		}
	}
	...
	for (int npc : MobConstants.LOGGED_NPCS) {
		stoner.getProperties().addProperty("MOB", GameDefinitionLoader.getNpcDefinition(npc).getName(), 0);
	}
	}
}