...
public class FlowerGame {
	...
	private static Mob getGambler(Stoner stoner) {
	Mob mob = gamblers.get(0);

	for (Mob gambler : gamblers) {
		if (Math.abs(gambler.getX() - stoner.getX()) <= Math.abs(mob.getX() - stoner.getX()) && Math.abs(gambler.getY() - stoner.getY()) <= Math.abs(mob.getX() - stoner.getX())) {
			mob = gambler;
		}
	}
	...
	// DialogueManager.sendNpcChat(stoner, 1011, Emotion.DEFAULT, "Sorry, but
	// Jaybane has forbidden you from gambling.");
	// return false;
	// } else

	if (busy) {
		DialogueManager.sendNpcChat(stoner, 1011, Emotion.DEFAULT, "I'm busy with another bet at the moment.");
		return false;
	} else if (stoner.isPouchPayment()) {
	...
	} else if (!stoner.getBox().hasItemAmount(995, amount)) {
		DialogueManager.sendNpcChat(stoner, 1011, Emotion.DEFAULT, "You don't have that much money to bet!");
		return false;
	}
	...
	stoner.send(new SendMessage(flower));
	Mob mob = getGambler(stoner);

	stoner.start(new OptionDialogue("Bet on hot", p -> {
		plant(mob, flower);
