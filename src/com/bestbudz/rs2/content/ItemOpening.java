...
public enum ItemOpening {
  ...
  MITRHIL_GOLD_TRIMMED_LG(
      13008, new Item(12277), new Item(12279), new Item(12281), new Item(12283)),
  MITRHIL_GOLD_TRIMMED_SK(
      13010, new Item(12277), new Item(12285), new Item(12281), new Item(12283)),
  ...
  ItemOpening(int itemID, Item... items) {
    this.itemID = itemID;
    this.items = items;
  }
  ...
  public static boolean openSet(Stoner stoner, int item) {
    ...
    if (stoner.getBox().getFreeSlots() <= data.items.length) {
      stoner.send(
          new SendMessage(
              "You need at least " + data.items.length + " available box spaces to do this!"));
      return false;
    }
    ...
    stoner.send(
        new SendMessage(
            "You successfully open the "
                + GameDefinitionLoader.getItemDef(data.itemID).getName()
                + "."));
