...
public class ReportHandler {
  ...
  public static boolean hasSpoke(String s) {
    for (int i = 0; i < 500; i++) {
      if (savedNames[i] != null) {
        if (savedNames[i].equalsIgnoreCase(s)) return true;
      }
    }
    return false;
  }
  ...
  public static void handleReport(Stoner stoner) {
    ...
    if (offending == null) {
      stoner.send(
          new SendMessage(
              stoner.reportName + " couldnt be reported, your too stoned and a shitty source"));
      return;
    }

    if (offending == stoner) {
      stoner.send(
          new SendMessage("Unfortunatly, you need to get someone else to report your ass!"));
      return;
    }

    if (stoner.lastReported.equalsIgnoreCase(stoner.reportName)
        && (System.currentTimeMillis() - stoner.lastReport) < 60000) {
    ...
    if (hasSpoke(offending.getUsername())) {
      ...
      for (int i = 499; i > 0; i--) {
        if (savedNames[i] != null) {
          if (savedNames[i].equalsIgnoreCase(stoner.getUsername())
              || savedNames[i].equalsIgnoreCase(offending.getUsername())) {
            sendText +=
                " -[" + savedTimes[i] + ": " + savedNames[i] + "]: " + savedSpeach[i] + "\r\n";
          }
        }
      }
      ...
      writeReport(
          ...
          offending.getUsername());
      stoner.send(
          new SendMessage("Thank you, we wont review this, your a Karen, now die in hole."));
