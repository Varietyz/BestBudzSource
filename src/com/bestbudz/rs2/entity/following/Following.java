...
public abstract class Following {
	...
	public void follow() {
		...
		if (!pause()) {
			if ((entity.isNpc())
				|| (following.getLocation().getX() != lastX)
	...
	public Location getCombatFollowLocation() {
		if ((entity.getCombat().getCombatType() == CombatTypes.MELEE)
			&& (entity.getSize() == 1)
			&& (following.getSize() == 1)) {
		...
		boolean projectile =
			(entity.getCombat().getCombatType() == CombatTypes.SAGITTARIUS)
				|| (entity.getCombat().getCombatType() == CombatTypes.MAGE);
		...
		if (RS317PathFinder.accessable(x2, y2, z, x3, y3)
			&& (!projectile && StraightPathFinder.isInteractionPathClear(x3, y3, z, x2, y2))
			|| (projectile && StraightPathFinder.isProjectilePathClear(x3, y3, z, x2, y2))) {
	...
	public Location getFollowToLocation() {
		...
		int[][] nodes = {
			{
				x + com.bestbudz.rs2.GameConstants.DIR[NON_DIAGONAL_DIRECTIONS[0]][0],
				y + com.bestbudz.rs2.GameConstants.DIR[NON_DIAGONAL_DIRECTIONS[0]][1]
			},
			{
				x + com.bestbudz.rs2.GameConstants.DIR[NON_DIAGONAL_DIRECTIONS[1]][0],
				y + com.bestbudz.rs2.GameConstants.DIR[NON_DIAGONAL_DIRECTIONS[1]][1]
			},
			{
				x + com.bestbudz.rs2.GameConstants.DIR[NON_DIAGONAL_DIRECTIONS[2]][0],
				y + com.bestbudz.rs2.GameConstants.DIR[NON_DIAGONAL_DIRECTIONS[2]][1]
			},
			{
				x + com.bestbudz.rs2.GameConstants.DIR[NON_DIAGONAL_DIRECTIONS[3]][0],
				y + com.bestbudz.rs2.GameConstants.DIR[NON_DIAGONAL_DIRECTIONS[3]][1]
			}
		};
		...
		for (int i = 0; i < nodes.length; i++) {
			double dist =
				Math.sqrt(
					Math.pow(entity.getLocation().getX() - nodes[i][0], 2.0D)
						+ Math.pow(entity.getLocation().getY() - nodes[i][1], 2.0D));
			if ((dist < bestDist)
				&& (following.getMovementHandler().canMoveTo(NON_DIAGONAL_DIRECTIONS[i]))) {
				bestDist = dist;
				bestX = nodes[i][0];
				bestY = nodes[i][1];
			}
	...
	public boolean outOfRange() {
		return !ignoreDistance && (entity.getZ() != following.getZ() || !isWithinDistance());
	}
	...
	public void updateWaypoint() {
		...
		if ((!following.getFollowing().flag()) && (following.getSize() == entity.getSize())) {
			...
			if ((otherF != null)
				&& (otherF.equals(entity))
				&& (entity.getLocation().equals(following.getLocation()))) {
				flag = true;
				return;
			}
		}
