...
public class StonerFollowing extends Following {
	...
	if (type == Following.FollowType.COMBAT) {
		if (stoner.getCombat().getCombatType() == CombatTypes.MELEE)
			...
			RS317PathFinder.findRoute(stoner, location.getX(), location.getY(), true, 16, 16);
	} else
		RS317PathFinder.findRoute(stoner, location.getX(), location.getY(), true, 16, 16);
	}
	...
	if (type == Following.FollowType.COMBAT) {
		if (GameConstants.withinBlock(following.getLocation().getX(), following.getLocation().getY(), following.getSize(), stoner.getLocation().getX(), stoner.getLocation().getY())) {
			return false;
		}
		...
		if ((!stoner.getLocation().equals(following.getLocation())) && (stoner.getCombat().withinDistanceForAssault(stoner.getCombat().getCombatType(), true))) {
			return true;
		}
