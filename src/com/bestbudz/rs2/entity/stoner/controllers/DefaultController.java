...
public class DefaultController extends Controller {
  ...
  public void onControllerInit(Stoner stoner) {
    if (!stoner.inWilderness()) {
      if (stoner.getAttributes().get("gainEP") != null) {
        ((Task) stoner.getAttributes().get("gainEP")).stop();
      }
      if (stoner.getAttributes().get("gainTarget") != null) {
        ((Task) stoner.getAttributes().get("gainTarget")).stop();
      }
      stoner.getClient().queueOutgoingPacket(new SendStonerOption("null", 3));
      stoner.getClient().queueOutgoingPacket(new SendWalkableInterface(-1));
    }
  }
