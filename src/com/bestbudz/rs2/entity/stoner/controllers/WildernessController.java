...
public class WildernessController extends Controller {
	...
	if (p2.getController().equals(ControllerManager.WILDERNESS_CONTROLLER)) {
		int difference = Math.abs(p.getProfession().getCombatGrade() - p2.getProfession().getCombatGrade());

		if (difference > p.getWildernessGrade()) {
			p.getClient().queueOutgoingPacket(new SendMessage("You must move deeper in the Wilderness to assault this stoner."));
			return false;
		}
		if (difference > p2.getWildernessGrade()) {
			p.getClient().queueOutgoingPacket(new SendMessage("This stoner must move deeper in the Wilderness for you to assault him."));
			return false;
		}
	} else {
		p.getClient().queueOutgoingPacket(new SendMessage("This stoner is busy or they are not in the Wilderness."));
		return false;
	}
	...
	if (stoner.getAttributes().get("gainTarget") == null && !TargetSystem.getInstance().stonerHasTarget(stoner)) {
