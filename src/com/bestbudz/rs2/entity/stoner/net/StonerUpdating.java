...
public final class StonerUpdating {
	...
	public static void addStoner(StreamBuffer.OutBuffer out, StonerUpdateFlags local, StonerUpdateFlags other, int index) {
	...
	public static void appendAppearance(StonerUpdateFlags flags, StreamBuffer.OutBuffer out) {
	...
	block.writeByte(flags.getCombatGrade());
	block.writeShort(0);
	...
	StonerUpdateFlags local = pFlags[stoner.getIndex()];
	...
	out.writeBits(8, stoner.getStoners().size());
	for (Iterator<Stoner> i = stoner.getStoners().iterator(); i.hasNext();) {
		StonerUpdateFlags flags = pFlags[i.next().getIndex()];
		...
		if ((flags != null) && (flags.getLocation().isViewableFrom(local.getLocation())) && (flags.isActive()) && (!flags.isPlacement()) && (flags.isVisible())) {
			...
			if (flags.isUpdateRequired())
				updateState(flags, block, false, stoner.getPrivateMessaging().ignored(flags.getUsername()));
		} else {
			out.writeBit(true);
			out.writeBits(2, 3);
			i.remove();
	...
	for (int i = 0; i < World.getStoners().length; i++) {
		if ((stoner.getStoners().size() >= 255) || (added >= 15)) {
			break;
		}
		...
		if (flags != null && !flags.getUsername().equals(local.getUsername()) && flags.isActive()) {
			...
			if (flags != null) {
				if (!doesLocalListContainStoner(stoner, flags.getUsernameToLong()) && flags.getLocation().isViewableFrom(stoner.getLocation())) {
					stoner.getStoners().add(World.getStoners()[i]);
					addStoner(out, local, flags, i);
					updateState(flags, block, true, stoner.getPrivateMessaging().ignored(flags.getUsername()));
					added++;
				}
			}
		}
	...
	public static void updateLocalStonerMovement(StonerUpdateFlags flags, StreamBuffer.OutBuffer out) {
	...
	public static void updateOtherStonerMovement(StonerUpdateFlags flags, StreamBuffer.OutBuffer out) {
	boolean updateRequired = flags.isUpdateRequired();
	int pDir = flags.getPrimaryDirection();
	int sDir = flags.getSecondaryDirection();
	if (pDir != -1) {
	...
	public static void updateState(StonerUpdateFlags flags, StreamBuffer.OutBuffer block, boolean forceAppearance, boolean noChat) {
	int mask = 0;
