...
public class ClickButtonPacket extends IncomingPacket {
	...
	if (stoner.getAttributes().get("DROPTABLE_SEARCH") != null) {
		...
		HashMap<Integer, Integer> searchButtons = (HashMap<Integer, Integer>) stoner.getAttributes().get("DROPTABLE_SEARCH");
		if (searchButtons.containsKey(buttonId)) {
			DropTable.displayNpc(stoner, searchButtons.get(buttonId));
		}
	}

	if (stoner.getInterfaceManager().getMain() == 53500) {
		if (buttonId <= 209161 && buttonId >= 209012) {
			int c = 0;
			for (Stoner p : World.getStoners()) {
				if (p != null && p.isActive() && p.getStonerShop().hasAnyItems()) {
					if (209012 + c == buttonId) {
						stoner.getShopping().open(p);
						break;
					}
					c++;
				}
	...
	stoner.send(new SendMessage("You have <col=255>" + Utility.format(stoner.getMoneyPouch()) + " </col>BestBucks on your Debit Card."));
		break;
	case 2203:
		stoner.start(new OptionDialogue("Pay by Cash", p -> {
...
///////////////////////////////////////////////////////////////////////
	...
	case 59100:
		...
		if (carrying.equals(BigInteger.ZERO)) {
			...
			stoner.send(new SendString("@red@" + NumberFormat.getNumberInstance(Locale.US).format(carrying) + "</col> bestbucks.", 17122));
		}
		...
		if (risked.equals(BigInteger.ZERO)) {
			...
			stoner.send(new SendString("@red@" + NumberFormat.getNumberInstance(Locale.US).format(risked) + "</col> bestbucks.", 17125));
		}
	...
	default:
		...
		if (GenieReset.handle(stoner, buttonId))
			return;
		if (AchievementButtons.handleButtons(stoner, buttonId))
			return;
		if (ProfessionsChat.handle(stoner, buttonId))
			break;
		if (stoner.getSummoning().click(buttonId))
			break;
		if (SummoningCreation.create(stoner, buttonId))
			break;
		if (Woodcarving.SINGLETON.clickButton(stoner, buttonId))
			break;
		if (com.bestbudz.rs2.content.profession.handinessnew.Handiness.SINGLETON.clickButton(stoner, buttonId))
			break;
		if (Handiness.handleHandinessByButtons(stoner, buttonId))
			break;
		if (HideTanning.clickButton(stoner, buttonId))
			break;
		if (FoodieTask.handleFoodieByAmount(stoner, buttonId))
			break;
		if ((stoner.getDialogue() != null) && (stoner.getDialogue().clickButton(buttonId)))
			break;
		if (Autocast.clickButton(stoner, buttonId))
			break;
		if (Emotes.clickButton(stoner, buttonId))
			break;
		if (DuelingConstants.clickDuelButton(stoner, buttonId))
			break;
		if (stoner.getTrade().clickTradeButton(buttonId))
			break;
		if (stoner.getBank().clickButton(buttonId))
			break;
		if (stoner.getMage().clickMageButtons(buttonId))
			break;
		if (EquipmentConstants.clickAssaultStyleButtons(stoner, buttonId))
			break;
		if (ForgingConstants.clickSmeltSelection(stoner, buttonId))
			break;
		if ((stoner.getAttributes().get("thchempistryitem1") != null) && ((((Item) stoner.getAttributes().get("thchempistryitem1")).getId() == 227) || (((Item) stoner.getAttributes().get("thchempistryitem2")).getId() == 227) ? !THChempistryUnfinishedPotionTask.handleTHChempistryButtons(stoner, buttonId) : !THChempistryFinishedPotionTask.handleTHChempistryButtons(stoner, buttonId)))
			break;
		break;
	}
	}
