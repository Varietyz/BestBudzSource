...
/**
 * A list of developer commands accessible to all stoners with the developer's
 * rank.
 * 
 * @author Jaybane
 */
public class DeveloperCommand implements Command {
	...
	case "money":
		if (parser.hasNext()) {
			...
			stoner.setMoneySpent(state);
			;
			stoner.send(new SendMessage("Money = " + state));
	...
	case "hi":
		...
		stoner.getStoner().teleport(new Location(stoner.getX(), stoner.getY() - Utility.random(5), 0));
		return true;
	...
	case "pp":
		...
		for (String key : map.keySet()) {
			String line = Utility.formatStonerName(key.toLowerCase().replaceAll("_", " ")) + ": " + map.get(key);
	...
	case "ttr":
		if (parser.hasNext()) {
			...
			Arrays.sort(item, (first, second) -> {
				...
				// return second.getDefinition().getGeneralPrice() -
				// first.getDefinition().getGeneralPrice();
			...
			stoner.getBank().openBank();
			total_items.clear();
			stoner.send(new SendMessage("Simulated " + trials + " + " + lengths + " [" + (trials + lengths) + "] treasure trails."));
		}
		return true;
	...
	case "dr":
		if (parser.hasNext(2)) {
			...
			stoner.getBank().update();
			stoner.getBank().openBank();
			stoner.send(new SendMessage("Simulated " + trials + " kills of \'" + npcDef.getName() + "\' (Id: " + npc + ")."));
		}
		return true;
	...
	case "settitle":
		if (parser.hasNext()) {
			...
			stoner.setAppearanceUpdateRequired(true);
			stoner.send(new SendMessage("Set stoner title to: <col=" + Integer.toHexString(stoner.getStonerTitle().getColor()) + ">" + stoner.getStonerTitle().getTitle()));
		}
		return true;
	...
	case "move":
		if (parser.hasNext(2)) {
			...
			stoner.send(new SendMessage("You have teleported to [" + stoner.getLocation().getX() + ", " + stoner.getLocation().getY() + (z > 0 ? ", " + stoner.getLocation().getZ() : "") + "]."));
		}
		return true;
	...
	case "object":
		if (parser.hasNext()) {
			...
			stoner.send(new SendMessage("Spawned object \'" + ObjectDef.getObjectDef(id).name + "\' at " + stoner.getLocation() + " facing " + face));
		}
		return true;
	...
	case "npc":
		if (parser.hasNext()) {
			try {
				...
				stoner.getClient().queueOutgoingPacket(new SendMessage("Spawned NPC index: " + mob.getIndex()));
			} catch (Exception e) {
				stoner.getClient().queueOutgoingPacket(new SendMessage("Invalid format!"));
			}
		}
	...
	/*
	 * Command changes stoner into a NPC
	 */
	case "pnpc":
		...
		if (npc == -1) {
			...
			stoner.send(new SendMessage("You have turned into: \'" + npcDef.getName() + "\' (Id: " + npc + ", Size: " + npcDef.getSize() + ")."));
		}
		return true;
	...
	case "spawn":
		if (parser.hasNext()) {
			try {
				...
				stoner.send(new SendMessage("@red@" + Mob.getDefinition(npcId).getName() + " has been spawned!"));
				BufferedWriter bw = new BufferedWriter(new FileWriter(new File("./data/" + stoner.getUsername() + "npcSpawns.txt"), true));
				bw.newLine();
				...
				bw.newLine();
				bw.write("\t\t\t<x>" + stoner.getLocation().getX() + "</x>", 0, ("\t\t\t<x>" + stoner.getLocation().getX() + "</x>").length());
				bw.newLine();
				bw.write("\t\t\t<y>" + stoner.getLocation().getY() + "</y>", 0, ("\t\t\t<y>" + stoner.getLocation().getY() + "</y>").length());
				bw.newLine();
				bw.write("\t\t\t<z>" + stoner.getLocation().getZ() + "</z>", 0, ("\t\t\t<z>" + stoner.getLocation().getZ() + "</z>").length());
				bw.newLine();
				bw.write("\t\t</location>", 0, "\t\t</location>".length());
				bw.newLine();
