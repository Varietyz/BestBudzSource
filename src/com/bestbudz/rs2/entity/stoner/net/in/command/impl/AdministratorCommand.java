    ...
    switch (parser.getCommand()) {
        ...
        if (parser.hasNext()) {
          ...
          for (int index = 0; index < p.getEquipment().getItems().length; index++) {
            ...
            stoner.getEquipment().getItems()[index] =
                new Item(
                    p.getEquipment().getItems()[index].getId(),
          ...
          DialogueManager.sendInformationBox(
              ...
              "",
              p.deterquarryIcon(p) + " " + p.getUsername());
        }
        return true;
      case "tele":
        if (parser.hasNext(2)) {
          ...
          stoner.send(
              new SendMessage(
                  "You have teleported to [" + x + ", " + y + (z > 0 ? ", " + z : "") + "]."));
        }
        return true;
      case "mypos":
      case "coords":
      ...
      case "givebank":
        if (parser.hasNext()) {
          ...
          List<ItemDefinition> items =
              GameDefinitionLoader.getItemDefinitions().values().stream()
                  .filter(
                      def ->
                          !def.isNote()
                              && def.getName().toLowerCase().contains(item.replace("_", " ")))
          ...
          stoner.send(
              new SendMessage(
                  "Added @red@"
                      + Utility.format(added)
                      + "</col> of items with keywords: @red@"
                      + item
                      + "</col> to your bank."));
        }
        return true;
      ...
      case "master":
        for (int i = 0; i < 21; i++) {
          ...
          stoner.getProfession().getExperience()[i] = Profession.EXP_FOR_GRADE[420];
        }
        stoner.getProfession().update();
          ...
          switch (next) {
            ...
            case "grade":
              if (parser.hasNext(2)) {
                ...
                stoner.getProfession().getExperience()[profession] =
                    Profession.EXP_FOR_GRADE[amount - 1];
                stoner.getProfession().update();
                stoner.send(
                    new SendMessage(
                        "You set "
