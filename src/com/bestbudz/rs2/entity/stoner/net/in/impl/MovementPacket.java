...
public class MovementPacket extends IncomingPacket {
	...
	if ((stoner.isDead()) || (stoner.getMage().isTeleporting()) || (!stoner.getController().canMove(stoner)) || (StonerConstants.isSettingAppearance(stoner))) {
	...
	if (steps > 0) {
		if ((Math.abs(path[(steps - 1)][0] - stoner.getLocation().getX()) > 21) || (Math.abs(path[(steps - 1)][1] - stoner.getLocation().getY()) > 21)) {
			stoner.getMovementHandler().reset();
		}

	} else if ((Math.abs(firstStepX - stoner.getLocation().getX()) > 21) || (Math.abs(firstStepY - stoner.getLocation().getY()) > 21)) {
		stoner.getMovementHandler().reset();
		return;
	}
	if (steps > 0)
