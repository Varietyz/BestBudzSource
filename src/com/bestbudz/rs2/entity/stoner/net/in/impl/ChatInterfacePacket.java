...
public class ChatInterfacePacket extends IncomingPacket {
	...
	else if (stoner.getDialogue().getNext() > -1)
		stoner.getDialogue().execute();
	}
	...
	case 3917:
		...
		stoner.send(new SendProfessionGoal(professionId, (int) stoner.getProfession().getExperience()[professionId], amount, type - 1));
		break;
	case 2700:
	...
	case 3900:
		...
		break;
	case 15460:
		stoner.getStonerShop().onSetPrice(stoner, amount);
		break;
	case 1743:
		FoodieTask.attemptFoodie(stoner, stoner.getAttributes().getInt("foodieitem"), stoner.getAttributes().getInt("foodieobject"), amount);
		break;
	case 4429:
		THChempistryUnfinishedPotionTask.attemptToCreateUnfinishedPotion(stoner, amount, (Item) stoner.getAttributes().get("thchempistryitem1"), (Item) stoner.getAttributes().get("thchempistryitem2"));
		break;
	case 4430:
	...
	case 5064:
		...
		if (stoner.getInterfaceManager().hasBankOpen())
			stoner.getBank().deposit(id, amount, slot);
	...
	case 3322:
		if (stoner.getTrade().trading())
			stoner.getTrade().getContainer().offer(id, amount, slot);
