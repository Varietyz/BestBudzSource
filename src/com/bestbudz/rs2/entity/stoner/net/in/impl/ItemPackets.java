...
public class ItemPackets extends IncomingPacket {
	...
	case 145:
		...
		if (BestbudzConstants.DEV_MODE) {
			stoner.send(new SendMessage("InterfaceId: " + interfaceId + " | Interface Manager: " + stoner.getInterfaceManager().getMain()));
		}

		if ((interfaceId != 1688 && interfaceId != 59813 && interfaceId != 56503) && (!stoner.getInterfaceManager().verify(interfaceId))) {
			return;
		}
	...
	case 117:
		...
		if ((interfaceId != 1688 && interfaceId != 56503) && (!stoner.getInterfaceManager().verify(interfaceId)))
			return;
		...
		case 1688:
			...
			if (itemId == 1704) {
				stoner.getClient().queueOutgoingPacket(new SendMessage("<col=C60DDE>This amulet is all out of charges."));
				return;
			}
		...
		case 3322:
			if (stoner.getTrade().trading())
				handleTradeOffer(stoner, slot, itemId, 5);
			else if (stoner.getDueling().isStaking()) {
	...
	case 43:
		...
		case 5064:
			...
			if (stoner.getInterfaceManager().hasBankOpen())
				bankItem(stoner, slot, itemId, 10);
		...
		case 3322:
			if (stoner.getTrade().trading())
				handleTradeOffer(stoner, slot, itemId, 10);
			else if (stoner.getDueling().isStaking()) {
	...
	case 129:
		...
		case 3322:
			if (stoner.getTrade().trading())
				handleTradeOffer(stoner, slot, itemId, 2147483647);
			else if (stoner.getDueling().isStaking()) {
	...
	case 87:
		...
		for (int index = 0; index < BestbudzConstants.ITEM_DISMANTLE_DATA.length; index++) {
			if (itemId == BestbudzConstants.ITEM_DISMANTLE_DATA[index][0]) {
				...
				stoner.getBox().addOrCreateGroundItem(BestbudzConstants.ITEM_DISMANTLE_DATA[index][1], 1, true);
				stoner.getBox().addOrCreateGroundItem(BestbudzConstants.ITEM_DISMANTLE_DATA[index][1], 1, true);
				stoner.send(new SendMessage("You have dismantled your " + GameDefinitionLoader.getItemDef(itemId).getName() + "."));
		...
		if (!Item.getDefinition(itemId).isTradable() || Item.getDefinition(itemId).getName().contains("Clue scroll")) {
			stoner.start(new OptionDialogue("</col>Drop and loose forever", p -> {
				stoner.getBox().remove(itemId, 1);
				stoner.send(new SendMessage("Your " + GameDefinitionLoader.getItemDef(itemId).getName() + " has been dropped and lost forever."));
				stoner.send(new SendRemoveInterfaces());
	...
	case 53:
		...
		if ((!stoner.getBox().slotHasItem(firstSlot)) || (!stoner.getBox().slotHasItem(secondSlot))) {
			return;
		}
	...
	case 122:
		...
		if ((stoner.getConsumables().consume(itemId, slot, ConsumableType.FOOD)) || (stoner.getConsumables().consume(itemId, slot, ConsumableType.POTION))) {
			return;
		}
		...
		case 4155:// Mercenary gem
			...
			DialogueManager.sendStatement(stoner, "You have been tasked to kill:", stoner.getMercenary().getAmount() + " " + stoner.getMercenary().getTask());
			return;
	...
	case 16:
		...
		if (itemId == 11283) {
			stoner.getClient().queueOutgoingPacket(new SendMessage("Your shield has " + stoner.getMage().getDragonFireShieldCharges() + " charges."));
			return;
		}
		...
		case 11808:// zgs
			...
			for (int i = 0; i < items.length; i++) {
				if (itemId == items[i][0] && stoner.getBox().hasItemAmount(items[i][0], 1)) {
					...
					DialogueManager.sendItem2zoom(stoner, "You carefully attempt to dismantly your godsword...", "@dre@You were successful!", items[i][1], 11798);
					break;
				}
			}
			break;
