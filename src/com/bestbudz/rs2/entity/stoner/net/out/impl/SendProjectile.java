...
public class SendProjectile extends OutgoingPacket {
	...
	public SendProjectile(Stoner stoner, Projectile p, Location pos, int lock, byte offsetX, byte offsetY) {
	super();
	this.p = p;
	this.lock = lock;
	...
	if (p.getSize() > 1) {
		sendAlt = new SendAltCoordinates(new Location(pos.getX() + (p.getSize() / 2), pos.getY() + (p.getSize() / 2)), stoner);
	} else {
		sendAlt = new SendAltCoordinates(pos, stoner);
	}
	}
