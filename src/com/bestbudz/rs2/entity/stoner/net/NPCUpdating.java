...
public class NPCUpdating {
  ...
  public static void addNewNpc(StreamBuffer.OutBuffer out, Stoner stoner, Mob mob) {
    ...
    Location delta = Utility.delta(stoner.getLocation(), mob.getLocation());
  ...
  public static void appendState(StreamBuffer.OutBuffer block, MobUpdateFlags npc, int faceDir) {
    ...
    if (npc.isTransformUpdate()) {
      if (npc.getTransformId() != -1) {
        block.writeShort(
            npc.getTransformId(), StreamBuffer.ValueType.A, StreamBuffer.ByteOrder.LITTLE);
      }
    }
    if (npc.isFaceToDirection() || faceDir != -1) {
      Location pos = null;
      if (npc.isFaceToDirection()) {
        ...
        pos =
            new Location(
                npc.getLocation().getX() + GameConstants.DIR[faceDir][0],
                npc.getLocation().getY() + GameConstants.DIR[faceDir][1]);
      }
      if (pos == null) {
  ...
  public static void update(Client client, StonerUpdateFlags pFlags, MobUpdateFlags[] nFlags) {
    ...
    for (Iterator<Mob> i = client.getNpcs().iterator(); i.hasNext(); ) {
      ...
      if (mob.getLocation().isViewableFrom(pFlags.getLocation())
          && mob.isVisible()
          && flags != null
          && !flags.isPlacement()) {
    ...
    for (int i = 0; i < World.getNpcs().length; i++) {
      ...
      if (mob.getLocation().isViewableFrom(pFlags.getLocation()) && mob.isActive()) {
        addNewNpc(out, client.getStoner(), mob);
