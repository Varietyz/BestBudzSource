...
public class Equipment {
	...
	if ((items[slot] != null) && (items[slot].getId() == item.getId()) && (items[slot].getDefinition().isStackable())) {
		...
		items[slot] = new Item(items[slot].getId(), items[slot].getAmount() + stoner.getBox().getItems()[clickSlot].getAmount());
	...
	stoner.getClient().queueOutgoingPacket(new SendEquipment(slot, items[slot].getId(), items[slot].getAmount()));
	...
	for (int i = 0; i < 14; i++) {
		if (items[i] != null) {
			...
			stoner.getClient().queueOutgoingPacket(new SendEquipment(i, items[i].getId(), items[i].getAmount()));
		} else {
			stoner.getClient().queueOutgoingPacket(new SendEquipment(i, 0, 0));
		}
	}
	...
	if (!stoner.getBox().hasSpaceFor(items[slot])) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("You do not have enough box space to unequip that."));
		return false;
	}
	...
	public void update() {
	for (int i = 0; i < items.length; i++)
		stoner.getClient().queueOutgoingPacket(new SendEquipment(i, items[i] != null ? items[i].getId() : 0, items[i] != null ? items[i].getAmount() : 0));
	}
	...
	stoner.getClient().queueOutgoingPacket(new SendEquipment(slot, items[slot] != null ? items[slot].getId() : 0, items[slot] != null ? items[slot].getAmount() : 0));
	}
	...
	stoner.getClient().queueOutgoingPacket(new SendConfig(43, EquipmentConstants.getAssaultStyleConfigId(weaponId, assaultStyle)));
	}
	...
	if ((items[3] != null) && (items[3].getWeaponDefinition() != null)) {
		assaultAnimation = items[3].getWeaponDefinition().getAssaultAnimations()[assaultStyle.ordinal()];
	...
	stoner.getCombat().getMelee().setAssault(new Assault(hitDelay, assaultSpeed), new Animation(assaultAnimation, 0));
	}
	...
	if (ToxicBlowpipe.hasBlowpipe(stoner)) {
		...
		stoner.getCombat().getSagittarius().setAssault(new Assault(hitDelay, assaultSpeed), new Animation(assaultAnimation, 0), SagittariusConstants.getDrawbackGraphic(stoner), SagittariusConstants.getEndGraphic(), proj);
	}
	}
