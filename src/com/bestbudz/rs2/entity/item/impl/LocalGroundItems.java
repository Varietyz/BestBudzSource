...
public class LocalGroundItems {
  ...
  public void drop(int id, int slot) {
    if ((stoner.getController().equals(ControllerManager.WILDERNESS_CONTROLLER))
  ...
  public void pickup(final int x, final int y, final int id) {
    GroundItem g =
        GroundItemHandler.getGroundItem(
            id, x, y, stoner.getLocation().getZ(), stoner.getUsername(), false);
    if ((g == null) || (!GroundItemHandler.exists(g))) {
      stoner.getMovementHandler().reset();
      return;
    }
    TaskQueue.queue(
        new WalkToTask(stoner, g) {
          @Override
  ...
  public boolean stack(GroundItem g) {
    ...
    GroundItem onGround =
        GroundItemHandler.getNonGlobalGroundItem(
            g.getItem().getId(),
            g.getLocation().getX(),
            g.getLocation().getY(),
            g.getLocation().getZ(),
