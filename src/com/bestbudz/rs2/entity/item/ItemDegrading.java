...
public class ItemDegrading {
  ...
  public static final int[][] WEAPON_DEGRADING = {
    {13899, 13901},
    {13902, 13904},
    {13901, 13901},
    {13904, 13904},
  ...
  public static final int[][] EQUIPMENT_DEGRADING = {
    {13858, 13860},
    {13861, 13863},
    {13864, 13866},
    {13870, 13872},
  ...
  public void degradeEquipment(Stoner p) {
    for (int i = 0; i < 14; i++)
      if (i != 3) {
            ...
            if (index != -1) {
              if (item.getId() != EQUIPMENT_DEGRADING[index][1]) {
                p.getClient()
                    .queueOutgoingPacket(
                        new SendMessage(
                            "Your " + item.getDefinition().getName() + " has degraded."));
              }
          ...
          if (ref != null) {
            ...
            if (isFullDegraded(ref)) {
              p.getClient()
                  .queueOutgoingPacket(
                      new SendMessage(
                          "Your " + item.getDefinition().getName() + " has degraded to dust."));
  ...
  public void degradeWeapon(Stoner p) {
        ...
        if (isFullDegraded(ref)) {
          ...
          if (lastStage == -1) {
            p.getClient()
                .queueOutgoingPacket(
                    new SendMessage(
                        "Your " + weapon.getDefinition().getName() + " has degraded to dust."));
