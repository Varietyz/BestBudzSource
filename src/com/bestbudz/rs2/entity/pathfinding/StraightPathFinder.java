...
public class StraightPathFinder {
  ...
  private static boolean canAssaultOver(int x, int y, int z, int pX, int pY) {
    if (x == pX && y == pY) {
      return true;
    }
  ...
  public static boolean isInteractionPathClear(
      final int x0, final int y0, final int z, final int x1, final int y1) {
    int deltaX = x1 - x0;
    int deltaY = y1 - y0;

    double error = 0;
    final double deltaError =
        Math.abs((deltaY) / (deltaX == 0 ? ((double) deltaY) : ((double) deltaX)));
  ...
  public static boolean isProjectilePathClear(
      final int x0, final int y0, final int z, final int x1, final int y1) {
    int deltaX = x1 - x0;
    int deltaY = y1 - y0;
