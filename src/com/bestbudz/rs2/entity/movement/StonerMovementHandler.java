...
public class StonerMovementHandler extends MovementHandler {
	...
	@Override
	public void process() {
	if ((stoner.isDead()) || (stoner.isFrozen()) || (stoner.isStunned()) || (stoner.getMage().isTeleporting()) || ((stoner.getDueling().isDueling()) && stoner.getDueling().getRuleToggle()[DuelingConstants.NO_MOVEMENT])) {
		reset();
		return;
	}
	...
	if ((walkPoint != null) && (walkPoint.getDirection() != -1)) {
		...
		if ((!forceMove) && (!BestbudzConstants.WALK_CHECK) && (!Region.getRegion(stoner.getLocation()).canMove(stoner.getLocation(), walkPoint.getDirection()))) {
			reset();
			return;
		}

		stoner.getMovementHandler().getLastLocation().setAs(stoner.getLocation());
		stoner.getLocation().move(com.bestbudz.rs2.GameConstants.DIR[walkPoint.getDirection()][0], com.bestbudz.rs2.GameConstants.DIR[walkPoint.getDirection()][1]);
	...
	if (stoner.getRunEnergy().isRunning()) {
		if (stoner.getRunEnergy().canRun()) {
			...
			if ((runPoint != null) && (runPoint.getDirection() != -1)) {
				if ((!forceMove) && (!BestbudzConstants.WALK_CHECK) && (!Region.getRegion(stoner.getLocation()).canMove(stoner.getLocation(), runPoint.getDirection()))) {
				...
				stoner.getMovementHandler().getLastLocation().setAs(stoner.getLocation());
				stoner.getLocation().move(com.bestbudz.rs2.GameConstants.DIR[runPoint.getDirection()][0], com.bestbudz.rs2.GameConstants.DIR[runPoint.getDirection()][1]);
				secondaryDirection = runPoint.getDirection();

				stoner.getRunEnergy().onRun();
			}
		} else {
			stoner.getClient().queueOutgoingPacket(new SendMessage("You don't have enough run energy to do that."));
	...
	if (stoner.inMultiArea())
		stoner.getClient().queueOutgoingPacket(new SendMultiInterface(true));
	else {
