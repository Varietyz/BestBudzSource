...
import com.bestbudz.rs2.content.dialogue.impl.GamblerDialogue;
...
public class WalkToActions {
	...
	TaskQueue.queue(new FollowToEntityTask(stoner, mob) {
		@Override
	...
	public static void clickObject(final Stoner stoner, final int option, final int id, final int x, final int y) {
	if (stoner.getMage().isTeleporting()) {
		return;
	}
	...
	if ((id == 1815)) {
		TaskQueue.queue(new PullLeverTask(stoner, x, y, length[0], length[1]) {
			@Override
			public void onDestination() {
			stoner.getUpdateFlags().sendFaceToDirection(length[0] >= 2 ? x + length[0] / 2 : x, length[1] >= 2 ? y + length[1] / 2 : y);
			stoner.getMage().teleportNoWildernessRequirement(2561, 3311, 0, MageProfession.TeleportTypes.SPELL_BOOK);
			}
		});
		return;
	} else if ((id == 1814)) {
		TaskQueue.queue(new PullLeverTask(stoner, x, y, length[0], length[1]) {
			...
			public void onDestination() {
			stoner.getUpdateFlags().sendFaceToDirection(length[0] >= 2 ? x + length[0] / 2 : x, length[1] >= 2 ? y + length[1] / 2 : y);
			stoner.getMage().teleportNoWildernessRequirement(3153, 3923, 0, MageProfession.TeleportTypes.SPELL_BOOK);
			}
		});
		return;
		...
		TaskQueue.queue(new PullLeverTask(stoner, x, y, length[0], length[1]) {
			...
			public void onDestination() {
			stoner.getUpdateFlags().sendFaceToDirection(length[0] >= 2 ? x + length[0] / 2 : x, length[1] >= 2 ? y + length[1] / 2 : y);
			stoner.getMage().teleportNoWildernessRequirement(id == 5960 ? 3090 : 2539, id == 5960 ? 3956 : 4712, 0, MageProfession.TeleportTypes.SPELL_BOOK);
			}
		});
		return;
	} else if ((id == 9706)) {
		TaskQueue.queue(new PullLeverTask(stoner, x, y, length[0], length[1]) {
			...
			public void onDestination() {
			stoner.getUpdateFlags().sendFaceToDirection(length[0] >= 2 ? x + length[0] / 2 : x, length[1] >= 2 ? y + length[1] / 2 : y);
			stoner.getMage().teleportNoWildernessRequirement(3105, 3951, 0, MageProfession.TeleportTypes.SPELL_BOOK);
			}
		});
		return;
	} else if ((id == 9707)) {
		TaskQueue.queue(new PullLeverTask(stoner, x, y, length[0], length[1]) {
			...
			public void onDestination() {
			stoner.getUpdateFlags().sendFaceToDirection(length[0] >= 2 ? x + length[0] / 2 : x, length[1] >= 2 ? y + length[1] / 2 : y);
			stoner.getMage().teleportNoWildernessRequirement(3105, 3956, 0, MageProfession.TeleportTypes.SPELL_BOOK);
			}
		});
		return;
	}

	TaskQueue.queue(new WalkToTask(stoner, x, y, length[0], length[1]) {
	...
	case 1:
		...
		case 2801:
			...
			if (stoner.getBox().getFreeSlots() == 0) {
				DialogueManager.sendStatement(stoner, "You do not have any free box spaces to do this!");
				return;
			}
		...
		default:
			...
			DialogueManager.sendNpcChat(stoner, id, Emotion.DEFAULT, Utility.randomElement(DEFAULT_DIALOGUES));
		...
		case 5419:
			...
			for (int i = 53516; i < 53716; i++) {
				...
				if (i - 53516 < available.size()) {
					...
					stoner.getClient().queueOutgoingPacket(new SendString("Shops Exchange | @gre@" + available.size() + "</col> Active " + (available.size() == 1 ? "Shop" : "Shops"), 53505));
					stoner.getClient().queueOutgoingPacket(new SendString(p.getUsername(), i));
					if (stoner.getShopMotto() != null) {
						stoner.getClient().queueOutgoingPacket(new SendString(p.getShopMotto(), i + 200));
					} else {
						stoner.getClient().queueOutgoingPacket(new SendString("No shop description set.", i + 200));
					}
				} else {
		...
		for (int[] id : Barrows.BROKEN_BARROWS) {
			if (item == id[1]) {
				if (stoner.getBox().contains(new Item(995, 250_000))) {
					...
					DialogueManager.sendNpcChat(stoner, 954, Emotion.DEFAULT, "Your " + GameDefinitionLoader.getItemDef(id[0]).getName() + " has been repaired.");
				} else {
					DialogueManager.sendNpcChat(stoner, 954, Emotion.EVIL_LAUGH_SHORT, "You need 250k to repair your barrows piece!");
				}
				break;
			}
		}
	...
	if (StonerConstants.isOwner(stoner)) {
		...
		if (o != null) {
			stoner.getClient().queueOutgoingPacket(new SendMessage("Object option: " + option + " id: " + id + " x: " + x + " y: " + y + " face: " + o.getFace() + " type: " + o.getType()));
		} else {
			stoner.getClient().queueOutgoingPacket(new SendMessage("clicking id with no reference: " + id));
		}
	}
	...
	case 1:
		...
		if (Exercisement.SINGLETON.fireObjectClick(stoner, stoner.getLocation(), Region.getObject(x, y, stoner.getLocation().getZ()))) {
			return;
		}
		...
		if (ObjectConstants.isSummoningObelisk(id)) {
			if (stoner.getGrades()[21] == stoner.getMaxGrades()[21]) {
				stoner.getClient().queueOutgoingPacket(new SendMessage("You already have full Summoning points."));
			} else {
				stoner.getClient().queueOutgoingPacket(new SendSound(442, 1, 0));
				stoner.getClient().queueOutgoingPacket(new SendMessage("You recharge your Summoning points at the obelisk."));
		...
		case 26762:
			...
			TaskQueue.queue(new Task(stoner, 1, false) {
				...
				@Override
				public void onStop() {
				if (stoner.getLocation().equals(new Location(3233, 3938)) || stoner.getLocation().equals(new Location(3232, 3938))) {
					stoner.teleport(new Location(3233, 10332));
				} else if (stoner.getLocation().equals(new Location(3233, 3950)) || stoner.getLocation().equals(new Location(3232, 3950))) {
					stoner.teleport(new Location(3232, 10351));
				} else if (stoner.getLocation().equals(new Location(3242, 3948)) || stoner.getLocation().equals(new Location(3243, 3948))) {
			...
			if (stoner.getLocation().getX() == 3184 && stoner.getLocation().getY() == 3945) {
				if (stoner.isPouchPayment()) {
					if (stoner.getMoneyPouch() < 7500) {
						DialogueManager.sendStatement(stoner, "You need 7,500 bestbucks to enter the arena.");
						return;
					}
				} else {
				...
				stoner.send(new SendMessage("You have paid 7,500 bestbucks and entered the resource arena."));
		...
		case 2073:
			if (stoner.getBox().hasSpaceFor(new Item(1963))) {
				...
				stoner.getClient().queueOutgoingPacket(new SendMessage("You pick the banana from the tree."));
			}
			break;
		case 2557:
		case 2558:
			...
			if (stoner.getLocation().getY() < 3917) {
				TaskQueue.queue(new WalkThroughDoorTask(stoner, x, y, z, new Location(2998, 3931, z)) {
					...
					if (stage == 0) {
						...
						balance.execute(stoner, null, stoner.getLocation(), new Location(2998, 3930), 0);
					} else if (stage == 5) {
						p.getClient().queueOutgoingPacket(new SendSound(326, 0, 0));
			...
			if (stoner.getLocation().getY() >= 3931) {
				TaskQueue.queue(new WalkThroughDoubleDoorTask(stoner, x, y, z, new Location(2998, 3931, z)) {
					...
					if (stage == 0) {
						...
						balance.execute(stoner, null, stoner.getLocation(), new Location(2998, 3916), 0);
					} else if (stage == 5) {
						p.getClient().queueOutgoingPacket(new SendSound(326, 0, 0));
		...
		case 37929:
			if (stoner.getLocation().getX() < 2926) {
				if (stoner.getLocation().getX() <= 2918)
					stoner.teleport(new Location(2921, 4384));
				else {
		...
		case 27661:
			if (stoner.getGrades()[5] == stoner.getMaxGrades()[5]) {
				stoner.getClient().queueOutgoingPacket(new SendMessage("You already have full Necromance."));
			} else {
				stoner.getClient().queueOutgoingPacket(new SendSound(442, 1, 0));
		...
		case 2104:
			...
			if (stoner.getLocation().getY() >= 3556) {
				TaskQueue.queue(new WalkThroughDoubleDoorTask(stoner, x, y, z, new Location(x, y - 1, z)));
			} else {
				TaskQueue.queue(new WalkThroughDoubleDoorTask(stoner, x, y, z, new Location(x, y + 1, z)));
			}
		...
		case 2100:
			...
			if (stoner.getLocation().getY() <= 3554) {
				TaskQueue.queue(new WalkThroughDoorTask(stoner, x, y, z, new Location(3445, 3555, z)));
			} else {
				TaskQueue.queue(new WalkThroughDoorTask(stoner, x, y, z, new Location(3445, 3554, z)));
			}
			break;
		case 1596:
		case 1597:
			if (stoner.getLocation().getY() <= 9917)
				stoner.teleport(new Location(3131, 9918, 0));
			else {
		...
		case 1557:
		case 1558:
			if (stoner.getLocation().getY() == 9944)
				stoner.teleport(new Location(3105, 9945, 0));
			...
			break;
		case 2623:
			if (stoner.getLocation().getX() >= 2924)
				stoner.teleport(new Location(2923, 9803, 0));
			else {
				stoner.teleport(new Location(2924, 9803, 0));
			}
			break;
		case 8960:
			if (stoner.getLocation().getX() <= 2490)
				stoner.teleport(new Location(2491, 10131, 0));
			else {
		...
		case 8959:
			if (stoner.getLocation().getX() <= 2490)
				stoner.teleport(new Location(2491, 10146, 0));
			else {
		...
		case 8958:
			if (stoner.getLocation().getX() <= 2490)
				stoner.teleport(new Location(2491, 10163, 0));
			else {
				stoner.teleport(new Location(2490, 10163, 0));
			}
			break;
		case 5103:
			if (stoner.getLocation().getX() >= 2691)
				stoner.teleport(new Location(2689, 9564, 0));
			else {
				stoner.teleport(new Location(2691, 9564, 0));
			}
			break;
		case 5104:
			if (stoner.getLocation().getY() <= 9568)
				stoner.teleport(new Location(2683, 9570, 0));
			else {
		...
		case 5111:
			...
			break;
		case 5106:
			if (stoner.getLocation().getX() <= 2674)
				stoner.teleport(new Location(2676, 9479, 0));
			else {
				stoner.teleport(new Location(2674, 9479, 0));
			}
			break;
		case 5107:
			if (stoner.getLocation().getX() <= 2693)
				stoner.teleport(new Location(2695, 9482, 0));
			else {
				stoner.teleport(new Location(2693, 9482, 0));
			}
			break;
		case 5105:
			if (stoner.getLocation().getX() <= 2672)
				stoner.teleport(new Location(2674, 9499, 0));
			else {
		...
		case 4309:
			for (Item i : stoner.getBox().getItems()) {
				if (i != null && Spinnable.forId(i.getId()) != null) {
					...
					stoner.getClient().queueOutgoingPacket(new SendItemOnInterface(1746, 170, spinnable.getOutcome().getId()));
					return;
				}
			}
			stoner.getClient().queueOutgoingPacket(new SendMessage("You do not have anything to spin!"));
			break;
		default:
	...
	case 2:
		...
		case 2558:
			if (stoner.getProfession().getGrades()[17] < 52) {
				stoner.getClient().queueOutgoingPacket(new SendMessage("You need a accomplisher grade of 52 to pick the lock on this door."));
				return;
			}
			if (!stoner.getEquipment().isWearingItem(6575)) {
				DialogueManager.sendStatement(stoner, "You must be wearing a tool ring to be able to open this door.");
				return;
			}
			stoner.getClient().queueOutgoingPacket(new SendMessage("You attempt to pick the lock..."));
			Task task = new Lockpick(stoner, (byte) 2, id, x, y, z);
			stoner.getAttributes().set("lockPick", task);
			TaskQueue.queue(task);
			break;
	...
	public static void itemOnObject(final Stoner stoner, final int itemId, final int objectId, final int x, final int y) {
	if (BestbudzConstants.DEV_MODE) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("Using item " + itemId + " on object " + objectId));
	}
	...
	if ((object == null) && (!StonerConstants.isOverrideObjectExistance(stoner, objectId, x, y, z))) {
		return;
	}
	...
	TaskQueue.queue(new WalkToTask(stoner, x, y, length[0], length[1]) {
		...
		if (objectId == 11744) {
			...
			if (def.getNoteId() == -1) {
				stoner.send(new SendMessage(def.getName() + " cannot be " + (def.isNote() ? "un-noted" : "noted") + "."));
				return;
			}
			...
			if (def.isNote()) {
				...
				stoner.send(new SendMessage("You have un-noted " + amount + " of " + def.getName() + "."));
				return;
			}

			stoner.getBox().remove(def.getId(), amount);
			stoner.getBox().add(def.getNoteId(), amount);
			stoner.send(new SendMessage("You have noted " + amount + " of " + def.getName() + "."));
			return;
		...
		if ((DwarfMultiCannon.hasCannon(stoner)) && (!DwarfMultiCannon.getCannon(stoner).construct(itemId)))
