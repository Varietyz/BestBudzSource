...
public class GelatinnothMother extends Mob {
  ...
  public long getAffectedDamage(Hit hit) {
    ...
    if ((hit.getAssaulter() != null) && (!hit.getAssaulter().isNpc())) {
      ...
      if (p != null) {
        if (getId() == STAGES[0]) {
          if ((p.getCombat().getCombatType() == CombatTypes.MAGE)
              && (MageConstants.getSpellTypeForId(p.getMage().getSpellCasting().getCurrentSpellId())
                  == MageConstants.SpellType.WIND)) {
            return hit.getDamage();
          }
