...
public class Kreearra extends Mob {
	...
	for (Stoner stoner : getCombatants()) {
		Location b = stoner.getLocation();
		if ((Math.abs(a.getX() - b.getX()) <= 1) && (Math.abs(a.getY() - b.getY()) <= 1))
			stoner.hit(new Hit(Utility.randomNumber(20)));
	}
	}

	@Override
	public void doAliveMobProcessing() {
	if ((getCombat().getAssaulting() != null) && (!tornados) && (Utility.randomNumber(20) == 0)) {
		tornados = true;
		initTornados();
	}
	}
