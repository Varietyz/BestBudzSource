...
public final class MobConstants {
  ...
  public static final int[] LOGGED_NPCS =
      new int[] {
        4315, 6618, 6619, 6610, 6612, 6615, 5779, 6609, 2054, 494, 2265, 2266, 2267, 319, 6342, 8,
        415, 4005, 2042, 239, 2215, 3162, 2205, 3129, 3127
      };
  ...
  public static final int[] FLYING_MOBS = {
    3176, 3166, 3167, 3174, 2042, 2043, 2044, 3165, 3162, 3163, 3164
  };
  private static final Logger logger = Logger.getLogger(MobConstants.class.getSimpleName());
  ...
  public static List<Mob> getGodWarsBossMob(Allegiance npc) {
    return GODWARS_BOSSES.stream()
        .filter(
            mob ->
                GodWarsData.forId(mob.getId()) != null
                    && GodWarsData.forId(mob.getId()).getAllegiance() == npc)
  ...
  public static boolean isAgressiveFor(Mob mob, Stoner stoner) {
    ...
    return (stoner.getController().canAssaultNPC())
        && (stoner.getProfession().getCombatGrade() <= mob.getDefinition().getGrade() * 2 + 1);
  }
  ...
  public static boolean noFollow(Mob mob) {
  ...
  public enum MobDissapearDelay {
    ...
    MobDissapearDelay(int id, byte delay) {
      this.id = id;
      this.delay = delay;
    }

    public static final void declare() {
      for (MobDissapearDelay i : values()) data.put(Integer.valueOf(i.id), Byte.valueOf(i.delay));
    }
