...
public class MapConstants {
  ...
  public static final RSObject getDoubleDoor(
      final int id, final int x, final int y, final int z, final int face) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      if (o != null
          && (d = ObjectDef.getObjectDef(o.getId())) != null
          && d.name != null
          && d.name.equals(name)
          && o.getFace() == face) {
        return o;
  ...
  public static final int getAlternateId(final ObjectDef main, boolean flag) {
    ...
    loop:
    for (int i = type - 200; i < type + 200; i++) {
      try {
        ...
        if (a != null) {
          for (int k = 0; k < def.anIntArray773.length; k++) {
            if (a.length - 1 < k || def.anIntArray773[k] != a[k]) continue loop;
          }
        }

        if (b != null) {
          for (int k = 0; k < def.anIntArray776.length; k++) {
            if (b.length - 1 < k || def.anIntArray776[k] != b[k]) continue loop;
          }
        }

        if (c != null) {
          for (int k = 0; k < def.modifiedModelColors.length; k++) {
            if (c.length - 1 < k || def.modifiedModelColors[k] != c[k]) continue loop;
          }
        }

        if (d != null) {
          for (int k = 0; k < def.originalModelColors.length; k++) {
            if (d.length - 1 < k || def.originalModelColors[k] != d[k]) continue;
          }
        }
