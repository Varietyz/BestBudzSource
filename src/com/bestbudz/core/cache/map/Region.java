...
public class Region {
  ...
  public void appendDoubleDoor(int oId, int x, int y, int z) {
    ...
    getDoubleDoors(door.getX2(), door.getY2())[z][door.getX2() - regionAbsX][
            door.getY2() - regionAbsY] =
        null;
  ...
  public boolean canMove(int x, int y, int z, int direction) {
	...
	public boolean canMoveStraightLine(int x0, int y0, int x1, int y1, int z) {
		...
		while (true) {
			if (!canMove(x0, y0, z, GameConstants.getDirection(sx, sy))) {
				return false;
			}
  ...
  public boolean canShoot(int x, int y, int z, int direction) {
    if (direction == 0) {
      ...
      return !shotBlockedNorthEast(x, y, z)
          && !shotBlockedNorth(x, y, z)
      ...
      return !shotBlockedSouthWest(x, y, z)
          && !shotBlockedSouth(x, y, z)
          && !shotBlockedWest(x, y, z);
    } else if (direction == 6) {
      return !shotBlockedSouth(x, y, z);
    } else if (direction == 7) {
      return !shotBlockedSouthEast(x, y, z)
          && !shotBlockedSouth(x, y, z)
          && !shotBlockedEast(x, y, z);
    }
    return false;
