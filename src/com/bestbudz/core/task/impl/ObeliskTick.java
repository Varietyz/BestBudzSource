...
public class ObeliskTick extends Task {
  ...
  private static Location[] getObeliskLocations(Obelisk obelisk) {
    ...
    for (int xMod = 0; xMod <= 4; xMod += 4) {
      for (int yMod = 0; yMod <= 4; yMod += 4) {
        locations[i++] =
            new Location(location.getX() + xMod, location.getY() + yMod, location.getZ());
      }
    }
    return locations;
  }
  ...
  public void execute() {
    ...
    while (true) {
      ...
      if (Obelisk.values()[random] == obelisk) continue;
    ...
    for (Stoner stoner : World.getStoners()) {
      ...
      if (obelisk
          .getArea()
          .contains(new Point(stoner.getLocation().getX(), stoner.getLocation().getY()))) {
        teleport(stoner, to);
      }
    }
  ...
  private void teleport(Stoner stoner, Obelisk to) {
    ...
    stoner
        .getMage()
        .doWildernessTeleport(
            to.Location.getX() + deltaX,
            to.Location.getY() + deltaY,
            to.Location.getZ(),
            TeleportTypes.OBELISK);
  }
  ...
  public enum Obelisk {
    ...
    Obelisk(int pillarId, Location Location) {
      ...
      this.gfxArea =
          Area.areaFromCorner(
              new com.bestbudz.rs2.entity.Location(
                  Location.getX() + 1, Location.getY() + 1, Location.getZ()),
              2,
              2);
