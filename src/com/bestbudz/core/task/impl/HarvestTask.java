...
public class HarvestTask extends Task {
	...
	super(stoner, 2, false, StackType.NEVER_STACK, BreakType.ON_MOVE, TaskIdentifier.CURRENT_ACTION);
	this.stoner = stoner;
	this.object = object;
	this.item = item;
	this.x = x;
	...
	if (stoner.getBox().getFreeSlots() == 0) {
		stoner.getClient().queueOutgoingPacket(new SendMessage("You do not have enough box space to collect this."));
		stop();
	} else {
	...
	if (item == 1779) {
		...
		message = "You hate picking " + Utility.getAOrAn(name) + " " + name + ", yet you are doing so.";
	}
	}
