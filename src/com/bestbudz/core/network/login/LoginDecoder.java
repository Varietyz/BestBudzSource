...
public class LoginDecoder extends FrameDecoder {
	...
	for (String retard : BestbudzConstants.BAD_USERNAMES) {
	...
	client.getStoner().setUid(uid);
	client.getStoner().setUsername(name);
	client.getStoner().setDisplay(name);
	client.getStoner().setPassword(pass);
