...
import com.mayhem.rs2.content.challenges.DailyChallenges;
...
public final class PlayerSave {
	...
	public static final class PlayerContainer {
		...
		public static boolean loadDetails(Player player) throws Exception {
			...
			try {
				...
				if (details.lootingBag != null) {
                    player.getLootingBag().setItems(details.lootingBag);
                }
		...
		public PlayerContainer(Player player) {
			...
			lootingBag = player.getLootingBag().getItems();
		}
	...
	public static final class PlayerDetails {
		...
		public static boolean loadDetails(Player player) throws Exception {
			...
			try {
				...
				player.setHardcoreIron(details.isHardcoreIron);
				player.setHardcoreDeath(details.HardcoreDeath);
				player.setMember(details.isMember);
				player.setIron(details.isIron);
				player.setExtreme(details.isExtreme);
				player.setClassic(details.isClassic);
				...
				if (details.dailyChallenge != null) {
					player.getDailyChallenges().setChallenge(details.dailyChallenge);
					player.getDailyChallenges().setAmount(details.challengeAmount);
					player.getDailyChallenges().setLastChallenge(details.lastChallenge);
					player.getDailyChallenges().setSkiller(details.challengeSkiller);
				}
		...
		private final boolean isHardcoreIron;
		...
		private boolean challengeSkiller;
		...
		public PlayerDetails(Player player) {
			...
			isClassic = player.isClassic();
			isIron = player.isIron();
			isHardcoreIron = player.isHardcoreIron();
			HardcoreDeath = player.HardcoreDeath();
