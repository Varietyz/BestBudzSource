...
import com.mayhem.core.util.chance.WeightedObject;
...
public class MysteryBoxMinigame {
	 ...
	 */
	private final static int CREDITS_REQUIRED = 65;
	...
	public static Chance<Item> available = new Chance<Item>(Arrays.asList(
			...
			new WeightedChance<Item>(WeightedChance.COMMON, new Item(4099, 1)), //Mystic hat (dark)
			new WeightedChance<Item>(WeightedChance.COMMON, new Item(4089, 1)), //Mystic hat
	...
	public static void open(Player player) {
		...
		player.send(new SendString("</col>FalconExpress Bucks: @gre@" + Utility.format(player.getCredits()), 17006));
		player.send(new SendString("Mystery Box is a gambling minigame where you can bet @gre@" + CREDITS_REQUIRED + " </col>FalconExpress Bucks.", 17008));
	...
	public static boolean can(Player player) {
		...
		if (player.getCredits() < CREDITS_REQUIRED) {
			player.send(new SendMessage(MESSAGE_COLOR + "You do not have enough FalconExpress Bucks to do this!"));
			return false;
		}
