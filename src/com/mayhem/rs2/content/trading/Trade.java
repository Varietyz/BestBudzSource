...
public class Trade {
	...
	public void request(Player requested) {
		...
		if (player.ironPlayer() && !(requested.getRights() == 4) && (!(requested.getRights() == 4))) {
			player.send(new SendMessage("You are an Iron player and cannot trade!"));
			return;
		}
		
		if (requested.ironPlayer() && !(player.getRights() == 4) && (!(player.getRights() == 4))) {
			player.send(new SendMessage(requested.getUsername() + " is an Iron player and cannot trade!"));
			return;
		}
		
		if (player.getRights() == 15) {
			player.send(new SendMessage("Why even try m8? ;)"));
			return;
		}
		
		if (requested.getRights() == 15) {
			player.send(new SendMessage(requested.getUsername() + " Doesn't want to trade with you."));
			return;
		}
