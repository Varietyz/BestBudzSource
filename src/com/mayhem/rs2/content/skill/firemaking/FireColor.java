...
public class FireColor {
	...
	public static final void open(Player player) {
		...
		player.send(new SendString("Help Clan: " + fires.getColor()[1] + "" + FIRE_STARTER_CLAN, 49760));
	...
	public static final boolean can(Player player) {
		...
		if (player.getCredits() < 10) {
			DialogueManager.sendStatement(player, "You need 10 FalconExpress Bucks to do this!");
			return false;
		}
		return true;
	}
	...
	public static final void main(Player player, int buttonId) {
		...
		for (Location position : FIRE_POSITIONS) {
			...
			ObjectManager.registerObject(new GameObject(FIRE, position.getX(), position.getY(), position.getZ(), 10, 1));
		}

		player.setCredits(player.getCredits() - 10);
		World.sendGlobalMessage(fires.getColor()[1] + "<img=8>" + player.getUsername()
				+ " has just changed all fire colors to " + fires.getName() + "!");
		// AchievementHandler.activate(player,
		// AchievementList.CHANGE_FIRE_COLOR_5_TIMES, 1);
