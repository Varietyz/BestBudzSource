...
import com.mayhem.rs2.content.challenges.DailyChallengeHandler;
...
public class Player extends Entity {
	...
	public static final int timePlayed = 0;

	public boolean isInCutScene = false;

	private DailyChallengeHandler dailyChallenges = new DailyChallengeHandler(this, null);
	...
	private boolean isHardcoreIron = false;
	...
	public boolean ironPlayer() {
		...
		if (this.isHardcoreIron()) {
			return true;
		}
		if (this.getRights() == 15) {
			return true;
		}
		return false;
	}
	...
	public boolean login(boolean starter) throws Exception {
		...
		if (starter) {
			ChangeAppearancePacket.setToDefault(this);
			World.sendGlobalMessage("<img=13>@dre@" + username + " has logged in for the first time!");
			send(new SendMessage("<img=13>@dre@ TYPE ::DISCORD TO JOIN OUR ACTIVE GROUP CHAT " + username + "!!!"));
		...
		send(new SendString("</col>FalconExpress Bucks: @gre@" + Utility.format(this.getCredits()), 52504));
		...
		send(new SendMessage("<img=8>@blu@Welcome to FalconExpress!"));
		send(new SendMessage("<img=8>@blu@Forums are under construction!"));
		send(new SendMessage("<img=8>@dre@::Discord to join our Group chat! || Only Donate To Qwerty159063!. "));
		
		
		if (World.getActivePlayers() > Constants.MOST_ONLINE) {
			Constants.MOST_ONLINE = World.getActivePlayers();
			FileHandler.saveMaxPlayers();
			World.sendGlobalMessage("[<col=910D0D>FalconExpress</col>] We have broken our most players online record! New record: <col=910D0D>" + Constants.MOST_ONLINE + "</col>!");
		}
		...
		if (getDailyChallenges().getChallenge() != null) {
			send(new SendMessage("Your daily challenge is to " + getDailyChallenges().getChallenge()
					.getDescription(getDailyChallenges().getAmount())));
		} else {
			send(new SendMessage("You've already completed your daily challenge."));
		}
		...
		if (PlayerConstants.isStaff(this)) {
			...
			send(new SendString("", 29413));//oh, when it broke i deleted everything i guess and put the original back or something o
		}
	...
	//handles getting game modes for highscores to read
	public String getGameMode() {
		if (isIron() && (isExtreme()))
			return "Extreme Ironman";
		if (isIron() && (isClassic()))
			return "Classic Ironman";
		if (isExtreme() && (!isIron())) 
			return "Extreme";
		if (isClassic() && (!isIron()))
			return "Classic";
		if (isIron() && (!isExtreme() && (!isClassic())))
			return "Ironman";
		if (isUltimateIron() && (isExtreme()))
			return "Extreme Ultimate Ironman";
		if (isUltimateIron() && (isClassic()))
			return "Classic Ultimate Ironman";
		if (isUltimateIron() && (!isExtreme() && (!isClassic())))
			return "Ultimate Ironman";
		if (isHardcoreIron() && (!isExtreme() && (!isClassic())))
			return "Hardcore Ironman";
		if (isHardcoreIron() && (isExtreme() == true))
			return "Extreme Hardcore Ironman";
		if (isHardcoreIron() && (isClassic() == true))
			return "Classic Hardcore Ironman";
	...
	public void logout(boolean force) {
		if (isActive()) {
			if (force) {
				...
				com.everythingrs.hiscores.Hiscores.update("ttc4K5R0Ctw0LMGlzplS6V32gEkxsu5ZikxL0vKo4Bc5NjiMQZmxuqP099bgliRg8zAVe7Nl",  "Normal Mode", this.getUsername(),
			...
			for (int i = 0; i < this.getSkill().getExperience().length; i++)
				playerXp[i] = (int) this.getSkill().getExperience()[i];
			String key = "ttc4K5R0Ctw0LMGlzplS6V32gEkxsu5ZikxL0vKo4Bc5NjiMQZmxuqP099bgliRg8zAVe7Nl";
			
			if (this.getRights() == 2 || this.getRights() == 3 || this.getRights() == 4) {
				System.err.println("Didn't save hiscores for " + this.getUsername());
			} else com.everythingrs.hiscores.Hiscores.update(key, getGameMode(), this.getUsername(), this.getRights(),
					playerXp, debugMessage);

			// this is as good as its guna get lel, disgusting ik
			/*
			 * if (isIron()) { new Thread(new IronmanHiscores(this)).start(); }
			 * 
			 * else if (isUltimateIron()) { new Thread(new
			 * UltimateHiscores(this)).start(); }
			 * 
			 * else if (isExtreme()) { new Thread(new
			 * ExtremeHiscores(this)).start(); }
			 * 
			 * else { new Thread(new Highscores(this)).start(); }
			 */
	...
	public boolean isHardcoreIron() {
		return isHardcoreIron;
	}

	public void setHardcoreIron(boolean isHardcoreIron) {
		this.isHardcoreIron = isHardcoreIron;
	}
	...
	public DailyChallengeHandler getDailyChallenges() {
