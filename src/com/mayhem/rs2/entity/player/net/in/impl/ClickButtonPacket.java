...
import com.mayhem.rs2.content.GenieLamp;
import com.mayhem.rs2.content.GenieReset;
...
import com.mayhem.rs2.content.SkillsChat;
import com.mayhem.rs2.content.StarterKit;
import com.mayhem.rs2.content.TeleportHandler;
import com.mayhem.rs2.content.achievements.AchievementButtons;
...
import com.mayhem.rs2.content.interfaces.impl.AchievementTab;
import com.mayhem.rs2.content.interfaces.impl.BossInterface;
import com.mayhem.rs2.content.interfaces.impl.MinigameInterface;
import com.mayhem.rs2.content.interfaces.impl.OtherInterface;
import com.mayhem.rs2.content.interfaces.impl.PointsInterface;
import com.mayhem.rs2.content.interfaces.impl.PvPInterface;
import com.mayhem.rs2.content.interfaces.impl.QuestTab;
import com.mayhem.rs2.content.interfaces.impl.oldQuestTab;
import com.mayhem.rs2.content.interfaces.impl.SkillingInterface;
...
import com.mayhem.rs2.content.profiles.ProfileLeaderboard;
import com.mayhem.rs2.content.skill.SkillGoal;
...
import com.mayhem.rs2.content.skill.crafting.Crafting;
...
import com.mayhem.rs2.content.skill.magic.Autocast;
...
import com.mayhem.rs2.content.skill.summoning.SummoningCreation;
import com.mayhem.rs2.entity.Animation;
import com.mayhem.rs2.entity.Graphic;
import com.mayhem.rs2.entity.ReportHandler;
import com.mayhem.rs2.entity.World;
import com.mayhem.rs2.entity.ReportHandler.ReportData;
...
import com.mayhem.rs2.entity.player.net.out.impl.SendOpenTab;
...
public class ClickButtonPacket extends IncomingPacket {
	...
	public void handle(Player player, StreamBuffer.InBuffer in, int opcode, int length) {
		...
		/* Quest Tab */
		if (QuestTab.handle(player, buttonId)) {
			return;
		}
		...
		if(HouseManager.clickButton(player, buttonId)) {
			return;
		}
		...
		case 136201:
			...
			break;
		case 136204:
			InterfaceHandler.writeText(new AchievementInterface(player, AchievementDifficulty.MEDIUM));
			AchievementInterface.sendInterfaceForAchievement(player, AchievementList.BURN_500_LOGS);
			player.setAchievement(AchievementDifficulty.MEDIUM);
			player.send(new SendInterface(35_000));
			break;
		case 136207:
			InterfaceHandler.writeText(new AchievementInterface(player, AchievementDifficulty.HARD));
			AchievementInterface.sendInterfaceForAchievement(player, AchievementList.ANSWER_80_TRIVIABOTS_CORRECTLY);
			player.setAchievement(AchievementDifficulty.HARD);
			player.send(new SendInterface(35_000));
			break;
		...
		case 66108:
			...
			break;
			/** Fix Link with current server   
		case 66115:
			player.send(new SendString("http://www.mayhem.org/store/", 12000));
			break;
		*/ 
		...
		case 84237:
			...
			if (player.inWilderness() && player.getWildernessLevel() > 21) {
		...
		/** Equipment screen */
		case 59097:
			player.send(new SendString("</col>Melee Max Hit: @red@" + MeleeFormulas.calculateBaseDamage(player), 15116));
			player.send(new SendString("</col>Range Max Hit: @gre@" + RangeFormulas.getRangedMaxHit(player) + ".0", 15117));
			player.send(new SendString("</col>Magic Max Hit: @blu@" + MagicFormulas.magicMaxHit(player) + ".0", 15118));
		...
		case 140186:
			...
			player.send(new SendInterface(28500));
		...
		case 110245:
			...
			player.send(new SendOpenTab(11));
			player.send(new SendMessage("@or1@Your settings have been saved!"));
			break;
		case 110248:
		...
		case 201053:
			...
			player.send(new SendMessage("@gre@You have enabled your privacy settings.</col>"));
			break;
		case 201052:
		case 201054:
			player.send(new SendConfig(1032, 2));
			player.setProfilePrivacy(false);
			player.send(new SendMessage("@red@You have disabled your privacy settings.</col>"));
			break;
		case 201055:
		...
		case 189194:
			...
			break;
		case 189124:
			player.send(new SendMessage("@cr1@Coming soon!"));
			break;
			
		/** Experience lock */	
		case 59206:
			player.start(new OptionDialogue("@or2@Lock experience</col>", p -> {
				player.getSkill().setExpLock(true);
				player.send(new SendMessage("You have @red@locked</col> your experience."));
				player.send(new SendRemoveInterfaces());
			}, "Unlock experience", p -> {
		...
		case 115078:
			...
			for (String key : alphabetical) {
				String line = Utility.formatPlayerName(key.toLowerCase().replaceAll("_", " ")) + ": @red@" + map.get(key);
				player.send(new SendString("@red@Player Log Panel | " + alphabetical.size() + " Logs", 8144));
		...
		case 154052:
			...
			break;
		case 115070:
			/*player.send(new SendString("@dre@Mayhem's Latest Updates (</col> " + GameSettings.LATEST_UPDATE.length + " @dre@)", 8144));
			for (int i = 0; i < 30; i++) {
		...
		case 250005:
			...
			player.send(new SendString("Description: @red@Free", 61032));
		...
		case 250008:
			...
			player.send(new SendString("Description: @red@Free", 62032));
		...
		case 250011:
			...
			player.send(new SendString("Description: @red@Free", 63032));
			player.send(new SendString("Requirement: @red@None", 63033));
			player.send(new SendString("Wildy Level: @red@None", 63034));
			break;
		case 238086:// Boss
		case 246038:
		case 253246:
		...
		case 242062:
			...
			player.send(new SendString("Description: @red@Free", 64032));
		...
		case 242065:
			...
			player.send(new SendString("Description: @red@Free", 65032));
		...
		case 246044:
			...
			player.send(new SendString("Description: @red@Free", 61532));
		...
		default:
			...
			if (Fletching.SINGLETON.clickButton(player, buttonId))
				break;
			if (com.mayhem.rs2.content.skill.craftingnew.Crafting.SINGLETON.clickButton(player, buttonId))
				break;
