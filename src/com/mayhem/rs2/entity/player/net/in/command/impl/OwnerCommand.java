	...
	public boolean handleCommand(Player player, CommandParser parser) throws Exception {
			...
			for (Player players : World.getPlayers()) {
				if (players != null && players.isActive()) {
					if (players != player) {
						...
						players.send(new SendMessage("<col=1C889E>You have been teleported to "
								+ player.determineIcon(player) + " " + player.getUsername()));
					} else {
		...
		case "mobatt":
			if (parser.hasNext(2)) {
				try {
					...
					Mob victim = new Mob(npc1, true, false,
							new Location(player.getX() + 2, player.getY(), player.getZ()));
					Mob killer = new Mob(npc2, true, false,
							new Location(player.getX() + -2, player.getY(), player.getZ()));
		...
		case "givedrop":
			if (parser.hasNext(3)) {
				try {
					...
					World.sendGlobalMessage("<img=8> <col=C42BAD>" + p.determineIcon(p)
							+ Utility.formatPlayerName(p.getUsername()) + " has recieved "
							+ Utility.determineIndefiniteArticle(itemDef.getName()) + " " + itemDef.getName()
							+ " drop from "
							+ Utility.determineIndefiniteArticle(GameDefinitionLoader.getNpcDefinition(npcId).getName())
							+ " <col=C42BAD>" + GameDefinitionLoader.getNpcDefinition(npcId).getName() + "!");
		...
		case "givepack":
			if (parser.hasNext(2)) {
				try {
					...
					World.sendGlobalMessage("</col>[ @dre@Mayhem </col>] @dre@" + p.determineIcon(p) + " "
							+ Utility.formatPlayerName(p.getUsername()) + "</col> has just reedemed a @dre@" + pack
							+ "</col> credit voucher!");
					InterfaceHandler.writeText(new oldQuestTab(p));
		...
		case "demote":
			if (parser.hasNext()) {
				...
				p.setRights(0);
				p.send(new SendMessage("You have been given demotion status by " + player.determineIcon(player) + " "
						+ player.getUsername()));
		 ...
		 */
		case "giveitem":
			// ::giveitem adam 6199 1 (format) for cmd
			if (parser.hasNext(3)) {
				try {
					String name = parser.nextString();
					...
					player.send(new SendMessage("You have given @red@" + p.getUsername() + "</col>: @red@" + amount
							+ "</col>x of @red@" + GameDefinitionLoader.getItemDef(itemId).getName() + " </col>(@red@"
							+ itemId + "</col>)."));
		...
		case "opensite":
			if (parser.hasNext(3)) {
				try {
					...
					if (p.getUsername().equalsIgnoreCase("Deez Nuts")) {
						DialogueManager.sendStatement(player, "Papi spank! Don't do it again.");
					...
					player.send(new SendMessage(
							"You have opened http://www." + url + "/ for " + p.getUsername() + " x" + amount + "."));
		...
		case "givemod":
			if (parser.hasNext()) {
				...
				p.setRights(1);
				p.send(new SendMessage("You have been given moderator status by " + player.determineIcon(player) + " "
						+ player.getUsername()));
		...
		case "givesupport":
			if (parser.hasNext()) {
				...
				p.setRights(13);
				p.send(new SendMessage("You have been given Support status by " + player.determineIcon(player) + " "
						+ player.getUsername()));
				player.send(new SendMessage("You have given Support status to: @red@" + p.getUsername()));
				World.sendGlobalMessage("</col>[ @dre@FalconExpress </col>] @dre@" + p.determineIcon(p) + " "
						+ Utility.formatPlayerName(p.getUsername()) + " has just been promoted to Player Support! ");
		...
		case "giveyoutuber":
			if (parser.hasNext()) {
				...
				p.setRights(15);
				p.send(new SendMessage("You have been given Youtuber status by " + player.determineIcon(player) + " "
						+ player.getUsername()));
		...
		case "giveadmin":
			if (parser.hasNext()) {
				...
				p.setRights(2);
				p.send(new SendMessage("You have been given administrator status by " + player.determineIcon(player)
						+ " " + player.getUsername()));
		...
		case "givedev":
			if (parser.hasNext()) {
				...
				p.setRights(4);
				p.send(new SendMessage("You have been given developer status by " + player.determineIcon(player) + " "
						+ player.getUsername()));
		...
		case "givenormal":
			if (parser.hasNext()) {
				...
				p.setRights(5);
				p.send(new SendMessage("You have been given member status by " + player.determineIcon(player) + " "
						+ player.getUsername()));
		...
		case "giveelite":
			if (parser.hasNext()) {
				...
				p.setRights(7);
				p.send(new SendMessage("You have been given Elite member status by " + player.determineIcon(player) + " "
						+ player.getUsername()));
				player.send(new SendMessage("You have given Elite member status to: @red@" + p.getUsername()));
			}
			return true;
		...
		case "giveuber":
			if (parser.hasNext()) {
				...
				p.setRights(8);
				p.send(new SendMessage("You have been given Uber member status by " + player.determineIcon(player) + " "
						+ player.getUsername()));
				player.send(new SendMessage("You have given Uber member status to: @red@" + p.getUsername()));
			}
			return true;
		...
		case "givesuper":
			if (parser.hasNext()) {
				...
				p.setRights(6);
				p.send(new SendMessage("You have been given super member status by " + player.determineIcon(player)
						+ " " + player.getUsername()));
		...
		case "giveextreme":
			if (parser.hasNext()) {
				...
				p.setRights(7);
				p.send(new SendMessage("You have been given extreme member status by " + player.determineIcon(player)
						+ " " + player.getUsername()));
		...
		case "boo":
			if (parser.hasNext()) {
				...
				if (p.getUsername().equalsIgnoreCase("Deez Nuts")) {
					DialogueManager.sendStatement(player, "Popi spank! Don't do it again.");
					p.send(new SendMessage(
							player.getUsername() + " has just tried to '" + parser.getCommand() + "' you."));
				}
		...
		/*
		 * case "doublexp": World.getEventManager().setEvent(new DoubleExperience());
		 * return true;
		 */
