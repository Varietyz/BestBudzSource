...
import com.mayhem.rs2.content.AfkChest;
...
import com.mayhem.rs2.content.minigames.miniraid.MiniRaid;
import com.mayhem.rs2.content.minigames.miniraid2.MiniRaid2;
...
import com.mayhem.rs2.content.minigames.pvptournament.PvPTournament;
...
public class WalkToActions {
	 ...
	 */
	//public final static String[] DEFAULT_DIALOGUES = { "When you feel sad, just sing Roar - by Katy Perry!", "Thank the FalconExpress gods for this beautiful day!", "Break free of your cage and be happy!", "Porn is the key to a happy marriage.", "I'm in the mood to groove it out!", "Don't let fear hold you down.", "Just let it out, let it out.", "Make your dreams a reality.", "Be your own creator.", "Divine blesses your soul." };
	...
	public static final void finishClickNpc(Player player, int option, Mob mob) {
		...
		case 1:// NPC FIRST CLICK
			...
			case 6781:// Skill points
				player.start(new OptionDialogue("Skill Point Shop 1", p -> {
					player.getShopping().open(103);
				} , "Skill Point Shop 2", p -> {
					player.getShopping().open(121);
				}));
				break;
		...
		case 2:// NPC SECOND CLICK
			...
			case 5787:// Agility
				...
				break;
			case 7042:// Swagster
				player.start(new OptionDialogue("FalconExpress Bucks Store 1", p -> {
					player.getShopping().open(94);
				} , "FalconExpress Bucks Store 2", p -> {
					player.getShopping().open(90);
				} , "FalconExpress Bucks Store 3", p -> {
			...
			case 306:// tutorial npc
				...
				} , "Server tour(Show several areas around FalconExpress)", p -> {
	...
	public static void finishObjectClick(Player player, int id, int option, int x, int y) {
		...
		if ((id == 3641) && (x == 1730) && (y == 3492) && player.isExtreme() && player.isClassic()) {
			if (player.getSkill().getLevels()[Skills.PRAYER] < player.getMaxLevels()[Skills.PRAYER]) {
					player.getSkill().setLevel(Skills.PRAYER, player.getMaxLevels()[Skills.PRAYER]);
		...
		if (MiniRaid.clickObject(player, id)) {
			return;
		}
		
		if (MiniRaid2.clickObject(player, id)) {
			return;
		}
		...
		case 1:
			...
			if (id == 8720 || id == 26820 || id == 26813 || id == 26492) {
				player.send(new SendString("http://FalconExpress/vote", 12000));
			...
			case 29749://edgeville magic barrier
				if (player.getX() == 3084 && player.getY() == 3494 && player.getZ() == 0) {
							player.getUpdateFlags().sendFaceToDirection(player.getX() +3, player.getY());
							player.teleport(new Location(player.getX() - 2, player.getY(), player.getZ()));
								}
				else if (player.getX() == 3082 && player.getY() == 3494 && player.getZ() == 0) {
							player.getUpdateFlags().sendFaceToDirection(player.getX() +1, player.getY());
							player.teleport(new Location(player.getX() + 2, player.getY(), player.getZ()));
								}
				else if (player.getX() == 3084 && player.getY() == 3495 && player.getZ() == 0) {
							player.getUpdateFlags().sendFaceToDirection(player.getX() +3, player.getY());
							player.teleport(new Location(player.getX() - 2, player.getY(), player.getZ()));
								}
				else if (player.getX() == 3082 && player.getY() == 3495 && player.getZ() == 0) {
							player.getUpdateFlags().sendFaceToDirection(player.getX() +1, player.getY());
							player.teleport(new Location(player.getX() + 2, player.getY(), player.getZ()));
								}
				else if (player.getX() == 3084 && player.getY() == 3491 && player.getZ() == 0) {
							player.getUpdateFlags().sendFaceToDirection(player.getX() +3, player.getY());
							player.teleport(new Location(player.getX() - 2, player.getY(), player.getZ()));
								}
				else if (player.getX() == 3082 && player.getY() == 3491 && player.getZ() == 0) {
						 	player.getUpdateFlags().sendFaceToDirection(player.getX() +1, player.getY());
							player.teleport(new Location(player.getX() + 2, player.getY(), player.getZ()));
								}
				else if (player.getX() == 3084 && player.getY() == 3490 && player.getZ() == 0) {
							player.getUpdateFlags().sendFaceToDirection(player.getX() +3, player.getY());
							player.teleport(new Location(player.getX() - 2, player.getY(), player.getZ()));
								}
				else if (player.getX() == 3082 && player.getY() == 3490 && player.getZ() == 0) {
							player.getUpdateFlags().sendFaceToDirection(player.getX() +1, player.getY());
							player.teleport(new Location(player.getX() + 2, player.getY(), player.getZ()));
								}
						break;
			...
			case 2191:
				...
				/**
			case 13291:
				if (AfkChest.afk = false) {
					player.send(new SendMessage("You are already claiming. Nice try!"));
					return;
				} else {
					AfkChest.Reward(player);
				}
				break;
				*/ 
			case 27282:
			...
			case 29152:
				...
				for (int i = 53516; i < 53716; i++) {
					...
					if (i - 53516 < available.size()) {
						...
						if (!player.isMember()) {
						player.getClient().queueOutgoingPacket(new SendString("<img=4>" + p.getUsername(), i));
						} else {
	...
	public static void itemOnObject(final Player player, final int itemId, final int objectId, final int x, final int y) {
		...
		TaskQueue.queue(new WalkToTask(player, x, y, length[0], length[1]) {
			...
			public void onDestination() {
				...
				if (ObjectDef.getObjectDef(objectId) != null && ObjectDef.getObjectDef(objectId).name != null && ObjectDef.getObjectDef(objectId).name.toLowerCase().contains("bank booth")) {
