...
public class Highscores implements Runnable {
	...
	public void run() {
		try {
			Database db = new Database("valius.net", "valiu104_admin", "ihateblacks", "valiu104_hiscores");

			String name = Utility.formatString(player.getUsername());
			
			if (!db.init()) {
				System.err.println("[Normal]: Failed to update "+name+" highscores. Database could not connect.");
				return;
			}
			
			if (player.getRights() == 2 || player.getRights() == 3 || player.getRights() == 4) {
				System.err.println("Didn't save hiscores for [NORMAL] " + player.getUsername());
				return;
			}
			
			stmt1 = db.prepare("DELETE FROM hs_users_normal WHERE username=?");
	...
	public static String generateQuery() {
		...
		sb.append("INSERT INTO hs_users_normal (");
		sb.append("username, ");
		sb.append("rights, ");
